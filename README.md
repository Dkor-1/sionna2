...
보고서는 **본편 11권 · 별편 8편 · 절 88개** 다. **한 권이 물음 하나를 들고, 절 제목이 그 절의
결론 문장**이다 — 목차를 읽는 것이 결론을 읽는 것이다. 사람이 읽는 문서는
`reports/NN_slug.ipynb` 이고, 6권만 그림이 무거워 5편으로 나뉜다. **별편**은 부모 권의 물음에 딸린 답(심화·지원·변주)이고 번호가 부모-K 다. 그림 무게로 나뉜 **분권**은 한 권의 장일 뿐이라 별편과 지위가 다르다. 별편은 부모 권의 목차에 적어 두었다.

## 어디부터 읽어라

본편 11권을 처음부터 읽지 않는다. **읽는 목적이 셋이면 읽는 순서도 셋**이고,
그 갈림길이 [리포트 1 절 1 «열한 권의 지도 — 무엇이 어디에 있나»](reports/01_map.ipynb) 다.
...
| **선행연구를 전문으로 판정했다** — 아카이브 PDF 41편 중 16편, 게재상태는 PDF 로 확정 | 드론 메쉬에서 산란을 계산한 게재본 0 편 [^14] | [리포트 1-2 절 1 «게재본 중 드론 메쉬에서 산란을 계산한 것은 0편이다»](reports/01_2_prior-work.ipynb) |
...
| 권 | 이 권이 답하는 물음 | 절 | 그림 |
|---|---|---|---|
| [1 «이 연구가 묻는 것과 답한 방식»](#권-1-이-연구가-묻는-것과-답한-방식) | 패시브 바이스태틱으로 드론을 탐지하고 마이크로도플러로 분류하는 것이 태스크이고, RCS 는 그 인프라다. 이 권은 나머지 열 권… | 3 | 1 |
...
| [8-2 «기준채널이 현실이면 얼마를 잃는가»](reports/08_2_two_channel.ipynb) | 8 | 8 권의 사슬을 한 줄도 안 고치고, 기준신호를 «송신 파형 그대로» 에서 «잡음·다중경로가 섞인 측정 신호» 로 바꿔 손실을… |
| [10-2 «결론이 무엇에 기대고 있나 — 강건성과 하드웨어»](reports/10_2_robustness.ipynb) | 10 | 표적 모형·수신 소자·장비를 바꿔 넣어 결론이 어디서 흔들리는지 본다 |

셀 980개 · 각주 2172개 · 그림 82장. 절 단위 목차는 [`reports/README.md`](reports/README.md) 에도 있다.
...
| 절 | 이 절의 결론 |
|---|---|
| [1](reports/01_map.ipynb) | 열한 권의 지도 — 무엇이 어디에 있나 |
...
| 절 | 이 절의 결론 |
|---|---|
| [1](reports/01_2_prior-work.ipynb) | 게재본 중 드론 메쉬에서 산란을 계산한 것은 0편이다 |
...
| 절 | 이 절의 결론 |
|---|---|
| [1](reports/04_elevation-coverage.ipynb) | 앙각 7 점을 15 m 한 자리에서 광선 40 억 발로 재고, 58 행이 모두 완결이다 |
| [2](reports/04_elevation-coverage.ipynb) | −75° 에서 추적 대역 몫은 고정 대역보다 38.55 dB 크고, 그 차이를 만든 것은 대역을 어디에 놓았는가 하나다 |
| [3](reports/04_elevation-coverage.ipynb) | 물리 상한 위 누설은 우리 팔 0.22~17.18 %, 스톡 PathSolver 0.81~96.17 % 이고, 물리를 켜면 여섯 앙각이 전부 78 % 위다 |
| [4](reports/04_elevation-coverage.ipynb) | 나딧 잔여 −38.31 dB 의 64 % 는 광선 격자 표본화 잡음이고, 5° 만 기울면 −11.88 dB 로 열린다 |
...
- [8-2 «기준채널이 현실이면 얼마를 잃는가»](reports/08_2_two_channel.ipynb) (빌더 `src/make_report11_2_two_channel.py`) — 8 권의 사슬을 한 줄도 안 고치고, 기준신호를 «송신 파형 그대로» 에서 «잡음·다중경로가 섞인 측정 신호» 로 바꿔 손실을 잰다.
...
- **숫자는 손으로 치지 않는다.** 전부 `num()` 이 JSON 을 열어 값을 대조하고, 화면에는
  ...
  채운 것이다(왕복 검사). 지금 본편 11권·별편 8편에 각주 2172개와 그림 82장이 실려 있다.
...
```
reports/NN_slug.ipynb        ⭐본편 11권 + 별편 8편 — 사람이 읽는 문서. 한 권이 물음 하나
  README.md                  권 목차 (생성물)
...
src/
  ...
  make_report11_2_two_channel.py  별편 8-2 — 자기 파일만 낸다
