...
보고서는 **본편 12권 · 별편 7편 · 절 88개** 다. **한 권이 물음 하나를 들고, 절 제목이 그 절의
...
본편 12권을 처음부터 읽지 않는다. **읽는 목적이 셋이면 읽는 순서도 셋**이고,
그 갈림길이 [리포트 1 절 1 «열두 권의 지도 — 무엇이 어디에 있나»](reports/01_map.ipynb) 다.
...
| 권 | 이 권이 답하는 물음 | 절 | 그림 |
|---|---|---|---|
| [1 «이 연구가 묻는 것과 답한 방식»](#권-1-이-연구가-묻는-것과-답한-방식) | 패시브 바이스태틱으로 드론을 탐지하고 마이크로도플러로 분류하는 것이 태스크이고, RCS 는 그 인프라다. 이 권은 나머지 열한… | 3 | 1 |
...
셀 1009개 · 각주 2424개 · 그림 84장. 절 단위 목차는 [`reports/README.md`](reports/README.md) 에도 있다.

### 권 1 «이 연구가 묻는 것과 답한 방식»

패시브 바이스태틱으로 드론을 **탐지하고 마이크로도플러로 분류**하는 것이 태스크이고, RCS 는 그 인프라다. 이 권은 나머지 열한 권과 거기 딸린 별편들의 지도다.

→ [`reports/01_map.ipynb`](reports/01_map.ipynb)

| 절 | 이 절의 결론 |
|---|---|
| [1](reports/01_map.ipynb) | 열두 권의 지도 — 무엇이 어디에 있나 |
...
- **숫자는 손으로 치지 않는다.** 전부 `num()` 이 JSON 을 열어 값을 대조하고, 화면에는
  ...
  채운 것이다(왕복 검사). 지금 본편 12권·별편 7편에 각주 2424개와 그림 84장이 실려 있다.
...
```
reports/NN_slug.ipynb        ⭐본편 12권 + 별편 7편 — 사람이 읽는 문서. 한 권이 물음 하나
  README.md                  권 목차 (생성물)
