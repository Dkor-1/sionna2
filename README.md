...
`reports/NN_slug.ipynb` 이고, 그림이 무거워 나눈 권은 6권(5편) · A권(10편) 이다. **별편**은 부모 권의 물음에 딸린 답(심화·지원·변주)이고 번호가 부모-K 다. 그림 무게로 나뉜 **분권**은 한 권의 장일 뿐이라 별편과 지위가 다르다. 별편은 부모 권의 목차에 적어 두었다.
...
| **CFAR 를 경험 Pfa 로 교정했다** — GPU 몬테카를로로 오경보 셀을 직접 세었다 | 2,717 s [^10], 명목 1e-4 에서 배율을 형상마다 다시 잰다. ⚠**실내 통제 기하**(`benchmark/geometry.py`)에서 잰 값이다 — 실외 판에서 다시 내기 전에는 인용하지 않는다 | [리포트 8 절 3 «실내 통제 기하에서 경험 Pfa 를 재니 명목값의 1…»](reports/08_detector.ipynb) |
...
| [12 «실외 장면 — 지면과 벽이 서면 무엇이 달라지나»](#권-12-실외-장면--지면과-벽이-서면-무엇이-달라지나) | 자유공간에서 세운 판정이 지면과 건물이 있는 자리에서도 서는지 묻는다. 빗살 하모닉 SNR 로 다시 재면 el −30° 한 칸에… | 0 | 2 |
| [A «도감 — 원장의 모든 팔을 그림으로»](#권-a-도감--원장의-모든-팔을-그림으로) | 앙각 스윕 원장이 담은 팔 전부를 STFT 맵과 대역 에너지로 펴 둔 재고 목록이다. 판정하지 않는다 — 어느 팔이 무엇을 담고… | 14 | 1834 |
...
셀 4607개 · 각주 2592개 · 그림 1918장. 절 단위 목차는 [`reports/README.md`](reports/README.md) 에도 있다.
...
| [2](reports/01_map.ipynb) | 판정한 후보 12편 중 네 관문을 동시에 통과한 게재본은 0편이고, 최근접 3편은 각각 다른 관문에서 걸린다 |
...
| [2](reports/02_kernel.ipynb) | 스톡 솔버와 맞대면 «면이 많아서 에코가 커진다» 가설은 반증되고, 우리 커널 런타임의 96.9% 는 호스트가 쓴다 |
...
| [7](reports/02_2_stock-engine.ipynb) | 완전파는 정확도의 과녁이고, 표를 만들 수 있는 비용대에서 우리가 고른 것은 SBR+PO 다 |
...
| [3](reports/03_2_size-law.ipynb) | 모양의 유무는 부호가 확실하고(자릿수는 우리 격자가 정한다), 모양의 정밀도는 한 자릿수 dB 안에서 논다 |
...
| [6](reports/06_3_pattern.ipynb) | matrice4e 배 쪽 3.5 GHz 한 칸에서 프로펠러 채널은 레벨이 20.70 dB 낮고 변조 깊이는 0.73 → 5.93 dB 로 깊다 |
...
| 절 | 이 절의 결론 |
|---|---|
| [1](reports/07_illuminators.ipynb) | 상관에 걸 상시 기준신호는 표준마다 여럿이고, 대역이 가장 넓은 것을 하나씩 골랐다 |
...
| [2](reports/08_detector.ipynb) | 탭을 1~96 으로 늘려도 소거 깊이가 포화하고, 그 대가가 0-도플러 노치다 |
| [3](reports/08_detector.ipynb) | 실내 통제 기하에서 경험 Pfa 를 재니 명목값의 1.52~2.66 배였다 — 실외 판은 아직 없다 |
| [4](reports/08_detector.ipynb) | 잡음 맵까지의 배율은 셀 상관이 만들고, 나머지 +0.87 dB 는 아직 못 갈랐다 — 교정표는 형상마다 다시 잰다 |
...
| [3](reports/11_measurement.ipynb) | 두 기체(Matrice 4E · Mini 5 Pro)를 함께 한 거리빈에 넣는 최대 대역은 200 MHz 다 |
...
자유공간에서 세운 판정이 **지면과 건물이 있는 자리**에서도 서는지 묻는다. 빗살 하모닉 SNR 로 다시 재면 **el −30° 한 칸에서** 솔버 낙차 자세를 메운 뒤 값이 자유공간과 같아진다. el −60° 는 못 미치고, el 0° 는 바닥 아래라 이 원장이 말하지 않는다. 그 자세에서 빠지는 경로는 지면 정반사로 특정했다 — ⚠실외 한 칸의 자세 하나를 덤프한 결과다.
...
| | |
|---|---|
| Python | `/workspace/.venvs/py312/bin/python` (3.12) — 이 한 env 로 전부 실행 |
| 핵심 (지금 설치) | Sionna / Sionna RT 2.1.0 · Mitsuba 3.9.1 · drjit 1.5.0 (OptiX GPU) · torch 2.11.0+cu128 · numpy 2.5.2 · scipy 1.18.0 · trimesh 5.0.0 + manifold3d 3.5.2 (2026-09-16 확인) |
...
- **숫자는 손으로 치지 않는다.** 전부 `num()` 이 JSON 을 열어 값을 대조하고, 화면에는
  ...
  채운 것이다(왕복 검사). 지금 본편 13권·별편 7편에 각주 2592개와 그림 1918장이 실려 있다.
