...
def r51():
    return [
        ...
        md("## 이 사슬 위에서 무엇이 결정되나", "",
           ...
           "세 파형이 같은 코드 경로를 지나므로 그 격차에 구현 차이는 섞이지 않는다. 다만 "
           "프레임 수·문턱·거리창 같은 **규약**은 파형마다 다른 값을 만든다 — "
           + ref("eca", "같은 프레임 수에서 갈리는 T_CPI") + " · "
           + ref("shared-threshold", "세 밴드가 공유하는 문턱")
           + ". 그러므로 뒤 편들이 재는 격차는 파형 차이와 규약 차이를 갈라 읽는다. "
           "⛔«격차는 사슬 차이가 아니라 파형 차이다» 로 잘라 적었던 줄은 내렸다"
           "(2026-09-06 — 코드 경로가 하나라는 사실은 구현 차이만 배제하는데, 규약이 만드는 "
           "몫까지 파형 차이로 넘겨 편 52 가 ⛔로 부인한 읽기를 앞 편이 만들어 주고 있었다).", "",
...
def r52():
    return [
        header(
            num=52,
            #: ⛔2026-09-04 — 전 제목은 «탭을 늘리면 **환경이 정한 바닥**에서 멈추고» 였다.
            #  그 «바닥» 을 떠받치던 열은 실내 통제 기하에서 낸 것이라 내렸다
            #  (`archive/chamber_0903/`). 실외 원장으로 받칠 수 있는 관측만 남긴다.
            title="탭을 1~96 으로 늘려도 소거 깊이가 포화하고, 그 대가가 0-도플러 노치다",
            ...
            results=[
                ...
                #: ⛔2026-09-04 — 이 자리에 「다중경로를 다중경로를」 낱말 겹침과 닫지 않은
                #  «가 남아 ⛔가 무엇을 철회하는지 안 읽혔다. 독자 눈에는 뒤의 수만 남았다.
                f"한계까지 내려간다 — 5G {DEEP_DPI['5G NR 100MHz']}. ⛔여기 함께 적었던 "
                f"«다중경로를 넣으면 {DEEP_FULL['5G NR 100MHz']} 에서 멈춘다» 는 **실내 통제 "
                f"기하에서 낸 열이라 내렸다**(`archive/chamber_0903/`) — 인용하지 않는다.",
                f"기존 합성에서 탭 1~96 스윕의 소거 깊이가 "
                ...
                f"검사한 기준신호·지연 기저의 통제 모델에서는 클러터를 "
            ...
            method=[
                ...
                ("점유 격자",
                 ...
                 f"신호가 선다"),
                ("깊이 스윕",
                 f"탭 수 1~96 × (직접파만 / 다중경로 포함) 두 조건. ⛔다중경로 열은 실내 통제 기하에서 "
        ...
        md("## 직접파를 얼마나 지울 수 있는가", "",
           ...
           "",
           "왼쪽은 float64 산술의 한계다. ⛔**오른쪽 열은 실내 통제 기하에서 낸 것이라 "
           "내렸다**(`archive/chamber_0903/`) — 그 수는 인용하지 않는다. ⛔여기 함께 적었던 "
           "「두 열의 간격이 곧 환경이 정하는 몫이다」도 **그 열이 떠받치던 결론이라 뺀다** — "
           "실외 판에서 다시 내기 전에는 이 장면의 바닥을 말하지 않는다.", "",
        ...
        md("## 통제 모델에서는 클러터 세기 변화에 SCR이 둔감했다", "",
           f"클러터 세기를 {num(None, f'{ECA}:{_clutter_max()}.scale', '{:.0f}')}배까지 키워도 "
           f"SCR 변화폭은 {num(None, f'{ECA}:S5_clutter_dead.scr_span_db', '{:.1e}', 'dB')} 다. "
           f"이 통제 모델의 클러터는 소거기가 쓰는 기준신호 지연 기저에 포함된다.", "",
           f"이 결과의 적용 범위는 기존 통제 기하와 기준신호 모델이다. 기준채널 불일치·탭 밖 지연·채널 변동·유한 동적범위는 별도 대조가 필요하다. "
...
def r53():
    return [
        header(
            num=53,
            #: ⛔2026-09-04 — «운용 형상» 이라고만 적혀 있어 실외로 읽혔다. 이 값을 낸
            #  `benchmark/verify_cfar.py:58` 은 `geometry.chamber_window` 를 부르고
            #  `benchmark/geometry.py` 첫 줄이 «semi-anechoic 챔버 내부 바이스태틱 배치» 다.
            #  같은 편의 셀이 이미 「그 표는 실내 통제 기하가 읽던 것이라 내렸다」고 적는다.
            #  제목이 기하를 밝히지 않으면 첫 화면까지 그대로 퍼진다.
            title=f"실내 통제 기하에서 경험 Pfa 를 재니 명목값의 {RATIO_LO}~{RATIO_HI} "
                  f"배였다 — 실외 판은 아직 없다",
...
def r54():
    return [
        header(
            num=54,
            # ⛔옛 제목 「그 배율의 원인은 셀 상관이고, 교정표는 형상마다 다시 재야 한다」는
            #   2026-09-06 에 내렸다 — 대조군 둘이 확정한 구간은 «잡음 맵까지» 인데 제목이
            #   배율 전체의 원인을 단정했고, 제목은 목차·README 에서 본문 한정어와 떨어져
            #   인용된다. 남은 몫은 GAP_REST(위 168 줄 — `outputs/verify_cfar.json` 의
            #   ratio 넷에서 계산)이고 어떤 대조군도 그 구간을 끄지 않는다.
            title=f"잡음 맵까지의 배율은 셀 상관이 만들고, 나머지 {GAP_REST} dB 는 아직 "
                  f"못 갈랐다 — 교정표는 형상마다 다시 잰다",
            ...
            method=[
                ...
                ("형상 의존",
                 ...
                 f"크기로 적는다"),
                ("두 형상의 쓰임",
                 "⛔운용 형상의 표는 실내 통제 기하가 읽던 것이라 내렸다. 자유공간 기하는 "
    ...
    return [
        header(
            ...
            results=[
                ...
                f"수신기를 하나 더 놓으면 등속 6상태 그램행렬이 관측창 {T_OBS} 에서 국소 랭크 "
                f"{RANK2} · 위치 CRLB 하한 {RMS2} 가 된다 — 그 절대값은 측위 세션 옵션인 PRS "
                f"셀({REF_CFG}) · 기저선 "
        ...
        md("## 조명원별 셀 크기와 CRLB", "",
           ...
           f"오른쪽 두 열은 CRLB 이고, 그 안의 SNR 은 선언값이 아니라 **실내 통제 동작점에서 잰 "
        ...
        md("## 관측가능성 — 수신기 2대면 국소 랭크가 6 이 된다", "",
           ...
           table_from(f"{OBS}:fixes",
                      [("형상", None),
                       ...
                       ("관측가능 부분공간의 위치 CRLB(rms)", "pos_rms_m")],
        ...
        md("## 이 표를 읽는 법 — 국소 랭크와 하한", "",
           f"표의 랭크는 표적이 **등속으로 움직인다고 놓고** 관측창 {T_OBS} 를 누적한 "
           f"그램행렬에서 센 값이고, 수신기를 더 놓은 자리는 "
           f"({num(None, f'{OBS}:fixes._rx2[0]', '{:.0f}', 'm')}, "
           f"{num(None, f'{OBS}:fixes._rx2[1]', '{:.1f}', 'm')}, "
           f"{num(None, f'{OBS}:fixes._rx2[2]', '{:.1f}', 'm')}) 한 점이다 — «2RX 부터 6» 은 "
           f"기하만의 성질이 아니라 등속 가정 · 이 관측창 · 이 배치가 함께 만든 값이다.", "",
           "이 랭크는 **국소** 값이다 — 송신기와 두 수신기는 세 점이라 한 평면 위에 있고, 그 "
           "평면에 대해 위치와 속도를 함께 뒤집은 궤적은 모든 시각에서 같은 $(R_b, f_d)$ 를 "
           "낸다. 전역에서 하나로 정하는 일은 알려진 고도 · 비행 가능 공간 · 평면의 어느 쪽인지 "
           "같은 사전조건이 맡는다.", "",
           "오른쪽 열은 CRLB — 불편추정기의 오차가 내려갈 수 있는 **하한**이고, 추적기가 실제로 "
           "내는 위치 오차는 추적기를 돌려 따로 잰다.", "",
           f"`benchmark/verify_observability.py:615` 는 `pinv(G, rcond=1e-13)` 으로 영공간을 "
           f"버린 뒤 남은 **관측가능 부분공간**에서 이 열을 낸다 — 1RX 행의 값이 그 부분공간 "
           f"안의 하한인 이유이고, 랭크 열이 쓰는 허용오차 {PTOL} 로 잘라 다시 내면 1RX 행이 "
           f"2RX 행보다 작아진다. 이 대조는 `docs/AUDIT_REPORTS_0901.md` 가 든다."),

        md("## 이 표의 절대값이 매달린 것", "",
           f"랭크는 셀을 갈아도 그대로지만, 위치 CRLB 는 셀마다 달라진다. 표의 절대값은 기준 셀 "
           f"{REF_CFG} — 위 표의 마지막 줄인 **PRS**, 즉 측위 세션에서만 켜지는 신호에서 푼 "
           f"값이다. 상시 기준신호 셀은 같은 표에서 $\\sigma_{{R_b}}$ 가 더 크므로 위치 CRLB 도 "
           f"그만큼 커진다.", "",
           f"기하도 함께 매달려 있다 — 기저선 "
        ...
        next_steps([
            ("수신기 2대 형상으로 검출 실험을 재설계한다",
             f"CRLB 하한 {RMS2} 와 추적기가 내는 위치 오차가 따로 서고, 평면 거울상 짝이 허용 "
             f"비행 공간에 남는지가 정해진다",
