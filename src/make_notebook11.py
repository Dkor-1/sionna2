...
_front = provenance_cells(
    ...
    tldr=[
        ...
        "**그러나 '어디에 있나'는 다른 문제다.** 한 순간에 재는 것은 거리·도플러 **둘뿐**인데 위치는 "
        f"**3차원**이라 **{OB['summary']['verdict']}** (FIM 랭크 "
        f"{OB['summary']['snapshot_fim_rank']}/{OB['summary']['state_dim_position']}). 이 모호함은 근사가 "
        "아니라 **송수신 축 둘레 회전이라는 엄밀한 대칭**(기계정밀도로 검증)이다. **수신기 2개**면 위치 "
        f"오차가 {OB['fixes']['1RX (baseline)']['pos_rms_m']:.0f} m → "
        f"**{OB['fixes']['2RX']['pos_rms_m']*100:.0f} cm** 로 풀린다(§5).",
    ],
    roadmap=[
        ...
        dict(sec="§5", what="관측가능성 — 정확도≠분해능, 그리고 한 쌍으론 위치 불가",
             why="⭐ 탐지 다음의 '어디에 있나'. 수신기 2개 또는 각도가 필수인 이유"),
    ],
    sources=[
...
cells.append(md(
    ...
    "",
    "> 🔍 **여기서 하는 일** — 지금까지는 **탐지**(거리·속도 칸에 표적이 있나)를 다뤘습니다. 이 절은 그 "
    "다음 질문, **위치를 알 수 있나**를 봅니다. 답은 놀랍습니다: **송·수신기 한 쌍으로는 원리적으로 불가능**합니다.",
    "",
    "### 5.1 정확도 ≠ 분해능 — 흔한 오해부터",
    "",
    "**분해능(resolution)** 은 *두 표적을 가를 수 있나*(§2 의 거리 빈 폭)이고, **정확도(accuracy)** 는 "
    "*표적 하나의 위치를 얼마나 정밀히 찍나*(CRLB, 신호대잡음비에 좌우)입니다. **둘은 완전히 다른 축**인데 "
    "빈 폭을 정확도로 오해하기 쉽습니다. 실측하면 정확도가 분해능보다 **수십~수백 배 미세**합니다:",
    "",
...
cells.append(md(
    ...
    "이것이 이 프로젝트가 탐지를 먼저 하고 추적을 다음 일로 미룬 이유입니다.",
))
