...
cells.append(md(
    ...
    f"창 안에서 곁봉우리는 표적보다 최소 {abs(max(r['psl'] for r in RES.values())):.0f} dB 낮아 무해하다.",
    "",
...
cells.append(md(
    ...
    "",
    "<sub>여기 **σ 는 이 챔버의 특정 바이스태틱 기하**(TX·RX·표적 배치, 단일 반송파, 8° 창 국소평균)에서 "
    "잰 값이다 — 그래서 report08 §2 의 **360° 방위평균·밴드평균 σ 와 값이 다르다**(RCS 는 자세·기하에 "
    "따라 수 dB 출렁인다). 링크버짓에는 이 기하에서의 실제 밝기를 그대로 넣는다.</sub>",
    "",
...
cells.append(md(
    ...
    *[_obrow(c) for c in _cells_ob],
    ...
    "쪼갤 뿐 신호 대역이 넓어지는 게 아니니까요. 위 표의 '유효 셀 크기' 는 max(FFT 빈 그리드, 주엽폭) 이라 "
    "§2 의 −3 dB 주엽폭 ΔR_b 와는 정의가 다릅니다.)</sub>",
    "",
