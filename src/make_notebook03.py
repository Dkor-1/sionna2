...
cells += provenance_cells(
    ...
    spine=dict(
        core=(f"우리 타깃 DJI 드론은 **공개 CAD 가 없다**(제조사 미공개). 그래서 **방법 자체**를 검증한다 — "
              ...
              f"**우리와 똑같은 파라메트릭 방식으로 다시 만들어** 원본과 겹쳐 본다. 평균 밝기(σ)가 네 기체 모두 "
              f"**±1 dB 이내**로 맞고(Phantom 4 실측 스캔은 투영면적 {PH4['d_area_db']:+.2f} dB·σ {PH4['d_sigma_db']:+.2f} dB), "
              f"방향별 뾰족값({rms_lo:.0f}~{rms_hi:.0f} dB)만 인용하지 않는다. 같은 방식이면 타깃 드론 메쉬도 그만큼 믿을 수 있다."),
        ...
        verify=(f"제조사 실물 CAD(Yuneec Typhoon H480)·커뮤니티 메쉬(M100·M600) 오버레이 + 주요 "
                f"치수(대각·프롭경) 대조. 평균 σ 차이 {TY['d_sigma_db']:+.2f}·{M100['d_sigma_db']:+.2f}·"
...
cells.append(md(
    ...
    "**외부 3D 모델(실물 CAD·커뮤니티 메쉬)**과 겹쳐 봄으로써 그 빠진 절차를 채운다(선행이 이 기준을 어디서 찾는지는 §4).",
))
...
cells.append(md(
    ...
    "형상 라이브러리 **trimesh** 로 이들을 읽어, 그 기체를 우리 방식으로 다시 만든 메쉬와 같은 좌표계에 정렬해 겹쳐 본다.",
    "",
    "| 잣대 | 기체 | 성격 | 규모 |",
    "|---|---|---|---|",
    f"| **A. 실물 CAD** | {TY['name_real']} | 제조사(Yuneec) 헥사콥터의 **실물 3D 도면**, 오픈 라이선스 | 면 {TY['n_faces_real']:,} |",
...
cells.append(md(
    ...
    "이 리포트는 바로 그 표준을 따라, 우리와 아무 관계 없는 외부 실물 CAD·커뮤니티 메쉬에 겹쳐 본다.",
))
...
cells.append(md(
    ...
    "| 확인 | 방법 | 결과 |",
    "|---|---|---|",
    "| 형상 충실도 | 실물 CAD·커뮤니티 메쉬·**Phantom 4 실물 스캔** 오버레이 | 겹침 확인 |",
    f"| 평균 밝기 눈금 | 네 기체 평균 σ 차이 | **{TY['d_sigma_db']:+.2f}·{M100['d_sigma_db']:+.2f}·{M600['d_sigma_db']:+.2f}·{PH4['d_sigma_db']:+.2f} dB** (±1 dB) |",
...
cells.append(md(
    ...
    "trimesh 로 겹쳐 보고, 밝기는 선행이 쓰는 SBR+PO 로 비교했다. 프로펠러는 Holybro 1345 실물 CAD 로 보조 점검했다.",
    "",
    "| 주장 | 근거 | 값 |",
    "|---|---|---|",
    f"| 평균 밝기 눈금이 맞다 | 네 기체 평균 σ 차이 | **{TY['d_sigma_db']:+.2f} · {M100['d_sigma_db']:+.2f} · {M600['d_sigma_db']:+.2f} · {PH4['d_sigma_db']:+.2f} dB** (모두 ±1 dB) |",
    ...
    "",
    "**믿으면 안 되는 것** (앞머리 8️⃣ 참조): 방향별 σ 뾰족값(널·글린트 위치) · 대조 기체가 신형 타깃과 같다는 착각(대부분 구형/타사) · "
    "투영면적을 실측 밝기로 오해하기.",
    "",
