...
cells += provenance_cells(
    ...
    spine=dict(
        core=(f"우리 타깃 DJI 드론은 **공개 CAD 가 없다**(제조사 미공개). 그래서 **방법 자체**를 검증한다 — "
              ...
              f"**대략 ±1 dB**로 맞고(Phantom 4 만 −1.25 dB 로 살짝 벗어남)(Phantom 4 실측 스캔은 투영면적 {PH4['d_area_db']:+.2f} dB·σ {PH4['d_sigma_db']:+.2f} dB), "
    ...
    caveats=[
        ...
        f"**방향별(각도별) σ 뾰족값은 인용 금지.** 평균은 대략 ±1 dB 로 맞지만 각도별 RMS 는 {rms_lo:.0f}~{rms_hi:.0f} dB 어긋난다. "
...
cells.append(md(
    ...
    "",
    "다운로드한 실기체 **네 대**를 우리 방식으로 다시 만들어 원본과 겹쳐 본다. 위 둘 — **DJI Phantom 4 실물 "
    ...
    f"다시 만든 우리 메쉬 · **오른쪽** 형상 일치도. 평균 밝기 σ 는 네 기체 모두 **약 ±1 dB**"
    ...
    f"−3 dB 안팎 갈리지만(그림 오른쪽 Δarea), 평균 σ 는 {M100['d_sigma_db']:+.2f}·{M600['d_sigma_db']:+.2f} dB 로 역시 ±1 dB 안이다. "
    f"바로 이 거칢 — 공개 모델은 눈요기용이라 치수·형상을 못 믿는다 — 이 **우리 신형 타깃을 정밀 파라메트릭 메쉬로 직접 짓는 이유**다.",
    "",
    "> ⚠ **우리 메쉬는 원본 CAD 에 형상을 맞추지(fitting) 않는다 — 일부러.** 원본을 보고 깎으면 '닮았다'는 건 "
    "순환논리(그냥 베낀 것)라 검증이 안 된다. 그래서 **공개 제원표 숫자(대각·프롭경·엔벨로프)만으로 독립적으로** "
    "짓고, 그래도 맞는지 본다. 세부 형상은 당연히 다르다 — 우리는 매끈한 파라메트릭 동체라 시각적으로 더 통통해 "
    "보이고, 원본은 실물의 얇은 프레임·잔디테일을 담는다. **그런데도 평균 σ 가 맞는 건, RCS 평균이 세부 곡률이 "
    "아니라 전체 크기(투영면적 규모)가 지배하고 우리가 제원으로 맞추는 게 바로 그 전체 치수이기 때문이다.** "
    "'통통함'은 정직하게 드러나 있다 — 그 세부 차이가 **각도별 RMS(6~12 dB, 인용 안 함)** 와 **Δ투영면적(커뮤니티 "
    "메쉬 대비 −3 dB — 회전원판 프롭·안테나 돌기까지 그린 원본이 오히려 실루엣이 넓다)** 로 나타난다.",
    "",
...
cells.append(md(
    ...
    "가는 돌출부를 덜 그린다 → 투영면적이 조금 작다 → 그 방향의 밝기도 그만큼 달라질 수 있다(방향에 따라 크게도·작게도 — Typhoon 은 +1.83 dB 로 오히려 크다). "
