...
def blocks_32() -> list:
    ...
    return [
        header(
            num=32,
            #: ⛔2026-09-04 — 전 제목은 «모양의 유무는 **수십 dB** 를 가르고» 였다.
            #  그 수십 dB 의 출처인 −54.44 dB 를 이 편 본문이 스스로 막아 뒀다 —
            #  「자릿수는 우리 격자가 정한다 … ⛔이 값은 **부호까지만** 인용한다」.
            #  제목이 그 금지를 어기고 있었다. 부호만 말하게 좁힌다.
            title="모양의 유무는 부호가 확실하고(자릿수는 우리 격자가 정한다), "
                  "모양의 정밀도는 한 자릿수 dB 안에서 논다",
