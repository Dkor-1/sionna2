...
cells.append(md(
    ...
    "",
    "> 🔧 **독립 교차검증 계획(오픈소스).** 이 SBR σ 는 현재 해석해(평판·구)로 검증돼 있다. 신뢰성을 더 "
    "높이기 위해 **RadarSimPy**(GPLv3, 3D STL 메쉬 RCS·광선추적)로 같은 드론 메쉬의 σ 를 **독립 재계산**해 "
    "대조하는 것이 다음 단계다 — 두 도구가 ±2 dB 내 일치하면 권위 σ 소스를 오픈소스로 이관해 우리 코드의 "
    "정당성 부담을 외부 검증 도구가 나눠 지게 한다(‘검증 후 대체’ 원칙, `OPENSOURCE.md`·prior_work/pw02).",
    "",
    "---",
