...
cells.append(md(
    ...
    "",
    "> 📚 **이 SBR+PO 는 선행 연구가 쓰는 방식 중 하나다.** ISAC 문헌에서 표적 RCS 는 세 갈래로 처리된다 — "
    "① 상용 full-wave EM 솔버(CADFEKO 등)로 계산해 채널에 주입(예: LAMBDA, arXiv:2607.03826), "
    "② **자작 SBR+PO**(기하광학 광선전송 + 광선튜브 위 물리광학 표면적분 — 예: BVH 가속 SBR+PO, "
    "arXiv:2604.09243), ③ 점산란체·UTD 근사(Costa & Thomä, IEEE J-STEAP 2025; Ziganshin, arXiv:2604.05991). "
    "우리는 **②(자작 SBR+PO)** 를 택했다 — 상용툴 없이 재현 가능하고, 공개 SBR 도구(RaytrAMP, GPL-3.0)가 "
    "못 하는 **바이스태틱 + 내부 산란체(배터리·PCB) 다중재질**까지 다루기 때문이다. 검증은 해석해(평판 "
    f"{_plate_err:+.2f}·구 {_sphere_err:+.2f} dB)로 하고, 절대값은 **실측 문헌 RCS 로 앵커링**한다(report08). "
    "근거·출처: `prior_work/pw01`(선행 RCS 처리 분류).",
    "",
    "---",
