...
cells += provenance_cells(
    ...
    caveats=[
        "**절대 RCS 는 실측 포락선에 '겹치나' 밝은 상단이다 — 신뢰의 중심에 두지 않는다.** SBR 은 "
        "해석해(구·평판)로 검증되고, 드론 σ 는 소형드론 실측 포락선(0.3~0.5 m 쿼드 방위평균 대략 "
        "−28~−16 dBsm)에 들며 크기·자세·주파수 트렌드를 재현한다(문헌 대조: DJI Mavic Pro 실측 "
        "−17 dBsm@15 GHz 등). 다만 few-λ(공진영역)에서 PO 는 절대레벨을 밝은 쪽으로 잡는 경향이 있어 "
        "수 dB 낙관적일 수 있다. 이 리포트가 지지하는 것은 방법의 정합성·상대 패턴이지 절대 dBsm 의 정밀값이 "
        "아니며, 검출 결과는 σ 밴드로 제시해 상대 결론의 robust 함을 보인다.",

        "**격자 위상 진동은 지터평균으로 잡는다.** 곡면은 광선격자를 어디에 맞추느냐에 절대레벨이 "
        f"±1.5 dB 흔들리는데(λ/8 산포 {_lam8['spread']:.1f} dB), 서브셀 오프셋 격자 여러 개를 평균하면 "
        "구가 해석해에 ±0.1 dB 로 붙는다(직접 검증). 그래도 절대 dBsm 은 측정 앵커에 맡긴다 — §4.",
