...
cells.append(md(
    ...
    "대고 재보는 것**입니다 — 커스텀 산란 add-on 을 얹는 선행 연구(예: Ziganshin, arXiv:2604.05991)도 "
    "구·원통 같은 정준체를 해석해·상용 솔버(FEKO)·실측과 대조해 검증합니다. 레이더 교과서는 두 물건의 "
    "밝기를 폐형식(닫힌 공식)으로 줍니다: 정면으로 조사한 **금속 평판**은 σ = 4πA²/λ², **금속구**는 "
    "σ = πr².",
    "",
    ...
    *[f"| {tag} 방위평균 [dBsm] | " + " | ".join(f"{m:+.2f}" for m in ser["mean_dbsm"]) + " |"
    ...
    "",
    "정확히 하면 이 수렴은 **촘촘한 방위 샘플(180개)의 평균**에서 성립합니다. **개별 방위각 하나의 값**은 "
    "격자를 λ/12→λ/24 로 반절해도 최대 ~6 dB 흔들리고(p95 ~3.9 dB), 방위 샘플이 24개뿐이면 평균조차 "
    "~0.5 dB 움직입니다(`report_mesh/outputs/mesh_verify.json` I_sbr_subdiv). 그러므로 **개별각 σ 값은 "
    "수렴한 숫자가 아니며 인용하지 않습니다** — 우리가 밖으로 내보내는 것은 촘촘한 방위평균뿐입니다.",
    "",
    "덧붙여 삼각형을 4배(25,676→102,704면) 잘게 쪼개도 σ 변화는 사실상 0(최대 ~10⁻⁴ dB, 같은 파일 "
    "I_sbr_subdiv subdivision_invariance)입니다 — 결과가 메쉬 테셀레이션의 산물이 아니라 **표면 형상 "
    "그 자체**에서 나온다는 뜻입니다.",
    "",
...
cells.append(md(
    ...
    "그렇습니다. 반면 SBR 은 광선을 실제로 쏘므로, 뒤에 숨은 면은 광선이 앞면에서 멈춰 애초에 닿지 "
    "못합니다. 이 가림 처리는 상용 EM 솔버의 SBR 과 같은 성질입니다.",
    "",
    ...
    "면만** 밝기에 넣으니, 뒤에 숨은 면까지 세는 순수 PO 보다 값이 낮아집니다.",
))
