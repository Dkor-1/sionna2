...
검증 (이 모듈이 맞다는 근거)
  ...
  * 큰 구(반지름 r≫λ): σ = πr² (기하광학) — 일치 확인
  검증이 맞으면, 같은 코드로 드론 RCS(θ, 주파수)를 뽑을 수 있습니다.

정확도 한계(솔직히)
  * 이 PO 는 **자기차폐(self-shadowing)·다중반사(multi-bounce)를 무시**합니다: 시선을 향한
    면(n̂·û>0)이면 앞쪽 구조에 가려지는 뒷면도 그대로 coherent 합산됩니다. 볼록한 평판·구
    검증엔 이 오차가 없지만, **오목한 드론(암·로터·짐벌)의 절대 RCS 는 과대계상**될 수 있습니다.
