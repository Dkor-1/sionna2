...
def build_08_2():
    ...
    c = [
        ...
        md("## 세 엔진을 같은 격자에, 거리 3 / 8 / 15 m", "",
           ...
           f"±{TM['rpm_spread_frac']*100:.2f} %(PX4 실측 중간값). 기하는 **진짜 모노스태틱**"
           "(TX=RX, 기선 0)이다.", "",
           ...
           f"그림의 절반이다. 8 m 는 원거리장 경계({FARFIELD_M:.1f} m) 바로 곁이다.", "",
           "| 엔진 | 변조 p-p | 무늬 |", "|---|---|---|",
           f"| Sionna PathSolver | {TRI['ptp_db']['sionna']:.1f} dB | 무늬가 성기고 날개끝 근처에서 잦아든다 |",
           f"| Ours (SBR+PO, 기본) | {TRI['ptp_db']['sbr']:.1f} dB | ⚠능선이 대역을 채우고 날개끝 밖에도 남는다 |",
           f"| Ours w/o occlusion | {TRI['ptp_db']['po']:.1f} dB | 능선이 몇 가닥에 그친다 |",
           "",
           "⚠ **깊이는 p-p 로 읽지 마라.** 고해상도(4,096 자세)에서 Sionna 의 p-p 가 거리마다 "
           "32.6 / 2.5 / 50.1 dB 로 요동쳤는데, 그 요동은 물리가 아니라 **광선 표본화**다. "
           "경로를 거의 못 찾은 자세 하나가 −186 dB 까지 떨어져 전폭을 혼자 정한다. "
           "강건한 p5~p95 로 보면 7.5 / 1.1 / 20.2 dB 이고, 우리 팔은 p-p 7.9 ↔ p5~p95 3.6 dB "
           "로 간격이 좁다 — **그 간격 자체가 안정성의 척도**다.", "",
        ...
        md("## 블레이드 플래시를 세 엔진으로 나란히 (60 ms 확대)", "",
           ...
           f"{CEN['n_with_prop_specular']} 칸**이다(광선 2.56 억 발). "
           "남은 것은 확산 산란뿐이라 아치가 뭉개진다. "
           "⚠ 단 이 «0» 은 앙각 60° 이하 격자에서의 값이다 — 더 가파른 앙각에서는 잡힌다.", "",
           "⭐ **시간 분해능을 주파수 분해능보다 앞세운다.** 조각을 한 주기보다 길게 잡으면 "
           "플래시는 그 안에서 평균돼 사라지고 가로 능선만 남는다. 조각 길이 0.45 주기는 "
           "**실측으로 고른 값**이다 — 플래시 대비(날개끝 띠의 시간축 변동 폭)가 "
           "0.15주기 2.6 dB · 0.20 7.4 · 0.30 16.1 · **0.45 19.1(최대)** · 0.60 17.6 이다.", "",
           "⚠ 세 패널은 각자 최대로 정규화했다 — 이 그림은 **구조를 비교하지 레벨을 비교하지 "
           "않는다**. 레벨 차이(Sionna 가 −60 dB)는 위 표가 담는다."),
...
def build_08_3():
    ...
    c = [
        ...
        md("## 회전수가 같으면 무늬가 시간에 안 변한다", "",
           ...
           "PX4 텔레메트리 실측(모터 간 0.07~0.29 %)의 중간값이다. "
