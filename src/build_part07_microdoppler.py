    ...
    return [
        header(
            ...
            results=[
                ...
                f"모두 같은 부호·같은 크기 도플러를 받는다 — 최대 "
                f"{_n(P + '.rigid_prop_velocity.doppler_max_hz', PRB, '{:.1f}', 'Hz')}, "
                f"강체 예측 "
                f"{_n(P + '.rigid_prop_velocity.predicted_rigid_hz', PRB, '{:.1f}', 'Hz')}.",
        ...
        md("## 자유도를 세면 답이 정해진다", "",
           ...
           f"프롭 전체에 강체속도 하나를 주면 프롭 경유 경로가 전부 같은 부호·같은 크기 도플러를 "
           f"받는다 — 실측 최대값이 강체 예측과 같은 자리에 선다. 블레이드 전진/후퇴가 "
           f"갈라지려면 부호가 반대인 두 값이 나와야 하는데, 이 표의 도플러는 한 부호로 몰린다."),
    ...
    return [
        header(
            ...
            results=[
                ...
                f"로터마다 회전수를 "
                f"{_n('_meta.rpm_spread_frac', MDB, '{:.0%}')} 흩뜨리면 상관이 "
            ...
            method=[
                ("단일축",
                 ...
                 "다르다"),
                ("흩뜨림 규약",
                 "네 로터에 ±2 % 를 패턴 [1, −1, −0.55, 0.55] 로 준다 — 무게중심 치우침과 요 "
                 "토크 균형이 만드는 실제 비대칭을 흉내 낸 것이다"),
                ("판정량",
                 "창을 반으로 갈라 두 스펙트럼의 상관을 잰다. 완전 주기함수면 1 에 붙는다"),
                ("무엇이 선언인가",
                 "흩어짐 폭 2 % 는 우리 표적의 측정값이 아니라 선언된 가정이다 — "
                 "`_meta.spread_is_declared_ko` 에 그 사실을 적었다"),
            ],
        ...
        md("## 흩어짐 폭은 선언이고, 측정이 그 자리에 들어온다", "",
           "실제 기체는 무게중심 치우침과 요 토크 균형 때문에 네 모터가 서로 다른 추력을 내고 "
           "그만큼 회전수가 갈린다. 그 폭은 우리 표적에서 측정 대기 상태다.", "",
           f"근거는 `{MDB} : _meta.spread_is_declared_ko` 에 적었다. 선배가 준 PX4 SITL 로그는 "
           f"모터간 산포가 0.07~0.29 % 뿐이라 이 질문에 쓰기에 좁다.", "",
           "⭐ 그래서 이 편이 말하는 것은 «흩어짐이 2 % 다» 가 아니라 "
           "**«무늬가 시간에 변하려면 흩어짐이 필요하다»** 는 구조다."),
    ...
    return [
        header(
            ...
            results=[
                ...
                f"헤드라인 기체의 블레이드 통과율 {_L('physics.f_flash', '{:.1f}', 'Hz')} 는 "
                f"LTE·WiFi 의 나이퀴스트 안에 든다. 5G 쪽 두 모드에서는 그 통과율마저 접힌다.",
        ...
        md("## 그래서 무엇이 남는가", "",
           ...
           f"**블레이드 통과율** 이다 — 헤드라인 기체에서 "
           f"{_L('physics.f_flash', '{:.1f}', 'Hz')} 이고 LTE·WiFi 의 나이퀴스트 안에 든다.", "",
