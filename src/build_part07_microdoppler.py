...
def blocks_42() -> list:
    ...
    return [
        header(
            num=42,
            title="예산 사다리에서 판정 통계가 경로수에 따라 변했다 — "
            ...
            results=[
                ...
                f"{_n(BL + '.mini2_no_modulation_cells_now[0]', ATK)} 계열의 해석에는 예산 의존성을 "
                f"고려해야 한다. 메쉬와 예산의 기여는 미분리 상태다.",
            ],
            method=[
                ("예산 사다리",
                 ...
                 "추적했다"),
                ("기울기",
                 "판정 통계 [dB] 를 평균 경로수 [dB] 에 회귀한 기울기. 이 사다리의 예산 의존성을 재며 "
                 "신호 세기·확률적 확신도로 해석하려면 별도 교정이 필요하다"),
                ("기체 짝 보정",
        ...
        md("## 검사한 사다리의 예산 관계", "",
           table(["무엇을", "값"], [
               ...
               ["관측 기울기로 외삽한 Mini 2 상승 예측",
                _n(BL + ".implied_mini2_boost_if_budget_matched_db", ATK, "{:+.2f}", "dB")],
           ]), "",
           "관측 기울기는 이 통계의 예산 의존성을 보여준다. 확률적 확신도로 해석하려면 별도 교정이 필요하다. "
        ...
        md("## 꼬리의 관측값과 예산 해석을 구분한다", "",
           ...
           "가장 적게 어긋난 두 기체 중 하나다 — 이 차이에서 메쉬와 예산의 기여를 나누려면 맞춘 대조가 필요하다. "
