    ...
    return [
        ...
        md("## 널과 신호가 같은 문턱 아래에서 갈린다", "",
           *_fig(1, "report15_f1",
           ...
           "**널 팔 넷 중 가장 시끄러운 것**이다. 가장 시끄러운 널이 조용하면 나머지도 "
           "조용하므로 나머지 세 줄은 수치로 접었다(접힌 값은 `outputs/md_map_audit.json` 의 "
           "`null_rows_folded`).", "",
           "색눈금은 각 패널 자신의 정적 반사를 0 dB 로 잡은 값이다. ⭐**신호 줄은 블레이드 "
           "박자로 띠가 뛰고, 널 줄은 시간에 전혀 안 변한다** — 표시 규약이 0 도플러를 "
           "지우지 않으므로 널 줄에도 정지 반사 그 자체는 0 dB 로 남는다. 즉 «검다» 가 아니라 "
           "«안 움직인다» 가 널의 표시다. 판마다 붙은 뱃지가 정지 반사 대비 최대 측대역인데, "
           "신호 줄은 몇 dB 안쪽이고 널 줄은 60~80 dB 아래다."),
