...
리포트 07 은 전부 **모노스태틱**이다(송신국=수신국). 우리 실증은 **패시브 바이스태틱**이라
«모노에서 본 무늬가 바이스태틱에서 그대로인가» 가 열린 채로 남아 있었다. 이 편이 그 자리를
채운다 — 리포트 07 의 세팅을 그대로 두고 **수신 방향 û_s 하나만** 연다.
...
cells = [
    md("# 리포트 07b — 마이크로도플러: **바이스태틱**", "",
       "> **도플러는 이등분선이 정하고, 플래시는 방위가 정한다. 둘은 같이 안 움직인다.**", "",
       "*(파일 이름 — 사용자가 «마이크로도플러_bistatic.ipynb 이런 식» 이라고 했고 저장소가 "
       "ASCII 파일명 규약과 15 권 편성에 맞춰 **8 권의 다섯째 편**"
       "(`reports/08_5_bistatic.ipynb`)으로 둔다. "
       "`07b` 는 리포트 07 의 짝이라는 뜻이다.)*", "",
       "리포트 07 의 맵은 전부 **모노스태틱**이다 — 송신국과 수신국이 같은 자리에 있다. "
       "그런데 우리 실증은 **패시브 바이스태틱**이다: 조명은 남의 기지국이 하고 우리는 떨어진 "
       "곳에서 받는다. 그래서 «모노에서 본 무늬가 바이스태틱에서 그대로인가» 는 리포트 07 이 "
       "답하지 않은 질문으로 남아 있었다. 이 편이 그것만 한다.", "",
       "| 무엇 | 값 |",
       "|---|---|",
       f"| 표적 | {M['name']} · 호버 |",
       f"| 조명 시선 û_i | 방위 {M['az_deg']:.0f}° · 앙각 {M['el_deg']:.0f}° (리포트 07 그대로, **고정**) |",
    ...
    md("## 절 0. 시나리오 — 무엇을 바꿨고 무엇을 안 바꿨나", "",
       ...
       "리포트 07 의 판과 사과-대-사과가 된다.", "",
       "**바꾼 것**: 수신 방향 û_s 하나. û_i 는 리포트 07 의 시선에 고정한 채, û_s 를 정해진 "
       "평면에서 β 만큼 연다.", "",
       "| 평면 | 무엇인가 | 왜 |",
       "|---|---|---|",
       ...
       "",
       f"세 줄 모두 **{M['n']:,} 프레임 전부 비트 단위로 같다**. 즉 이 편의 β=0 칸은 리포트 07 의 "
    ...
    md("## 그림 2 — β 를 열었을 때 실제로 무엇이 나오나", "",
       ...
       "β=0 칸 — 리포트 07 의 그 원장이다. 각 패널의 흰 파선이 **예측 B**, 검은 점선이 "
       "**예측 A** 다. 아랫줄 다섯 칸이 수치 판정이다.", "",
       "⭐ 맵의 표시 규약은 `src/md_mapstyle.py` 가 강제한다 — 리포트 07 과 **같은 설정**이다"
       "(조각 0.45 블레이드 주기 · hop 2 · 8배 제로패딩 · jet · 0~−40 dB). 다섯 칸을 각자 "
       "최대로 정규화했으므로 **무늬**를 비교하는 그림이고, **레벨**은 제목의 숫자와 (m) 칸이 "
       "맡는다.", "",
       "⚠ 맵만 보고 축척을 눈으로 재려 하면 안 된다. 이 표시 규약의 주파수 분해능은 281 Hz 인데 "
    ...
    md("## 절 1. 도플러 축척 — 이등분선이 이긴다", "",
       ...
       "(회귀 ③ 이 β=0 에서 리포트 07 의 PO 팔과 비트동일임을 확인한다).", "",
    ...
    md("## 절 2. 광선 팔로는 이 축척을 못 잰다 — 그리고 그것은 바이스태틱 탓이 아니다", "",
       ...
       "줄어서, 상대적으로 더 두드러진다.", "",
       "⭐ **이것은 바이스태틱의 성질이 아니다.** 리포트 07 의 모노 원장에 있는 세 팔을 "
       "**같은 추정기**로 재면 바로 보인다.", "",
    ...
    md("## 절 3. 레벨 — 게이트 두 개가 겹치는 만큼만 돌아온다", "",
       ...
       f"(자세 {len(J['census']) and M['n'] and 24} 개 평균, 장 계산 없이 넓이만).", "",
    ...
    md("## 절 4. 플래시 — 주기는 로터가, 시각은 방위가 정한다", "",
       ...
       f"· `el60` — 도플러는 {R['el60']['po_ratio']:.3f} 로 이미 11 % 줄었는데 플래시열은 "
    ...
    md("## 절 5. 정직 — 이 편이 딛고 선 근사와, 안 한 일", "",
       ...
       "물을 수 있다. 그것이 다음 편의 질문이다."),

    md("## ⭐ 모노 ↔ 바이스태틱 대조표", "",
       "리포트 07 의 어느 수치가 어떻게 달라지나. β 는 방위면 기준이다.", "",
       "| 무엇 | 리포트 07 (모노) | 이 편 β 30° | β 60° | β 90° | β 120° | 무엇이 그렇게 만드나 |",
       "|---|---|---|---|---|---|---|",
    ...
    md("## 부록 — 그림별 데이터 이력", "",
       ...
       f"✅ 후 (커널 기본값) | ±{M['rpm_spread_frac']*100:.2f} %(PX4 실측 중간값 — 리포트 07 상속) |",
       f"| 2 의 회귀·바닥 비교 | + `outputs/report07_three_engines.npz` | "
       f"{TRI['_meta']['generated'][:16]} | ✅ 후 | 같음 |",
       "",
       "이 편의 원장은 **리포트 07 의 원장에서 파생**된다 — `_meta` 를 읽어 세팅을 맞추고, "
       "β=0 열을 그 원장과 대조한다(절 0 회귀 표). 리포트 07 의 원장이 다시 계산되면 이 편도 "
       "다시 돌려야 한다."),
    ...
    md("## 부록 — 그림을 만든 설정 전부", "",
       ...
       f"| 조명 시선 û_i | 방위 {M['az_deg']:.0f}° · 앙각 {M['el_deg']:.0f}° | 리포트 07 상속 |",
       f"| 로터별 회전수 | {', '.join(f'{x:.0f}' for x in M['rpm_per_rotor'])} rpm "
       f"(산포 ±{M['rpm_spread_frac']*100:.2f} %) | 리포트 07 상속 (PX4 실측 중간값) |",
       f"| 로터 회전 방향 | {M['rotor_dirs']} · 블레이드 {M['prop_blades']}장 | 기체 CAD |",
       f"| PRF | {M['prf_hz']:.0f} Hz | 리포트 07 상속 (16×f_tip) |",
       f"| 표본 수 | {M['n']:,} = {M['n']/M['prf_hz']*1e3:.0f} ms = "
       f"{M['blade_periods']:.1f} 블레이드 주기 | 리포트 07 상속 |",
       ...
       "",
       "**표시 — `src/md_mapstyle.py` 가 강제한다 (리포트 07 과 같음)**", "",
       "| 항목 | 값 |",
       "|---|---|",
    ...
    md("## 부록 — 재현", "",
       ...
       "| 무엇 | 왜 미해결인가 |",
       "|---|---|",
       "| 광선 팔의 광대역 슬로타임 바닥 | 리포트 07 의 «날개끝 밖 능선» 과 같은 것. 격자를 "
       ...
       "| 2채널(ECA·CAF) 처리 | 이 편의 설계상 제외. 다음 편 |"),
]
