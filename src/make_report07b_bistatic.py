...
#: 셀 9 대조표의 «8 권 모노 편» 열이 무엇인가 — 빌드 시점의 계보로 적는다.
_MONO_COL_NOTE = (
    f"이 스윕의 β=0 칸이고 현재 모노 원장과 비트동일이다({_MONO_HIT}/{MONO_LIN['n']} 프레임)"
    if MONO_LIN["live_matches"] else
    "이 스윕의 β=0 칸이다 — 8 권 모노 편이 **그 스윕과 같은 격자 규약으로 냈던** 판이고, "
...
cells = [
    ...
    md("## ⭐ 모노 ↔ 바이스태틱 대조표", "",
       ...
       "| 무엇 | 팔 | 8 권 모노 편 | 이 편 β 30° | β 60° | β 90° | β 120° | 무엇이 그렇게 만드나 |",
       "|---|---|---|---|---|---|---|---|",
       ...
       "두 행을 한 줄로 이어 읽지 않는다.", "",
       f"⚠ **«8 권 모노 편» 열의 정체.** f_tip·f_flash 칸은 운동학 공칭값이고, 나머지 칸은 "
    ...
    md("## 부록 — 그림별 데이터 이력", "",
       ...
       f"±{M['rpm_spread_frac']*100:.2f} %(PX4 **SITL**(시뮬) 로그 중간값 — 8 권 모노 편 상속) |",
