...
6 권의 모노 편(6-1~6-4)은 전부 **모노스태틱**이다(송신국=수신국). 우리 실증은 **패시브 바이스태틱**이라
...
#: ⭐세 팔의 이름은 6 권 전체가 한 자리에서 받는다 — 6-2 와 6-5 의 같은 표가 같은 이름을
...
# ⭐2026-08-10 — 15 권 구조(당시 편성)에 자리를 잡는다. 6 권이 네 편(06_1~06_4)이므로 바이스태틱 판은
#   그 다섯째 편이다. 저장소 루트가 아니라 `reports/` 에 산다(사람이 읽는 문서가 사는 곳).
OUT = os.path.join(_ROOT, "reports", "06_5_bistatic.ipynb")
...
if MONO_LIN["live_matches"]:
    ...
    _MONO_NOTE = [
        "⚠ **회귀 ② 가 가리키는 모노 열은 «얼리기 전» 사본이다.** 6 권 모노 편의 SBR 열은 "
        "그 뒤 [얼린 광선 격자](06_2_engines.ipynb)로 다시 났고, 이 편의 β 스윕은 아직 "
...
# ── ⭐팔 표시에 쓰는 것들 — 전부 원장·코드에서 읽는다(손으로 안 적는다) ────────────
def _sweep_arm_keys() -> list[str]:
    """이 편의 β 스윕이 실제로 낸 열 이름 — 어느 팔이 돌았는지의 정본."""
    import numpy as _np
    with _np.load(f"{_ROOT}/outputs/report07b_bistatic_md.npz", allow_pickle=True) as z:
        return sorted(z.files)


SWEEP_KEYS = _sweep_arm_keys()
#: PathSolver 열이 이 스윕 원장에 있는가 — 표지의 경계 문단이 이 값으로 갈린다.
HAS_SIONNA_ARM = any("sionna" in k.lower() for k in SWEEP_KEYS)

#: 커널 계약 검사 원장 — 상반성 위반의 **크기**를 여기서 읽는다.
_VBF_PATH = f"{_ROOT}/outputs/verify_bistatic_field.json"
VBF = json.load(open(_VBF_PATH)) if os.path.exists(_VBF_PATH) else None
_RECIP_LINE = (
    "⭐ 위 둘째 줄(상반성 부분성립)의 **크기는 숫자로 있다** — 커널 계약 검사에서 "
    f"E(û_i, û_s) ↔ E(û_s, û_i) 의 최대 차가 **{VBF['reciprocity']['max_abs_d_db']:.1f} dB** 다"
    f"(`outputs/verify_bistatic_field.json : reciprocity.max_abs_d_db` = "
    f"{VBF['reciprocity']['max_abs_d_db']:.3f} · {VBF['reciprocity']['n']} 칸)."
    if VBF else
    "⚠ 위 둘째 줄(상반성 부분성립)의 **크기를 내는 원장이 없다** — 어긋남의 dB 를 여기 적지 "
    "않는다.")

#: 이 스윕 **자신의** SBR 바닥 — 절 1 이 인용하는 모노 원장 바닥과 다른 잣대다.
_SBR_FLOORS = [r["sbr_floor_rel_db"] for r in ROWS]

#: 순수 PO 팔의 플래시 관측량이 원장에 있는가 — 절 4 의 «한 팔 안 확인» 이 이것에 걸린다.
_PO_FLASH_KEYS = [k for k in ROWS[0] if k.startswith("po_") and "flash" in k]
_PO_FLASH_LINE = (
    "⚠ **순수 PO 팔의 플래시 관측량은 이 원장에 없다** — `rows[*]` 에 그 열이 "
    f"{len(_PO_FLASH_KEYS)} 개이고, 플래시는 SBR 팔의 `E` 하나로만 쟀다"
    "(`benchmark/report07b_bistatic_md.py:513`). 그래서 «한 엔진 안에서도 도플러와 "
    "플래시가 갈리는가» 는 **원장 없음**이다 — 아래 «남긴 문제» 로 넘긴다."
    if not _PO_FLASH_KEYS else
    "⭐ 순수 PO 팔의 플래시 열이 원장에 있다 — "
    f"{', '.join(f'`{k}`' for k in _PO_FLASH_KEYS)}. 한 팔 안의 갈림을 그 열로 확인한다.")

#: 셀 9 대조표의 «6 권 모노 편» 열이 무엇인가 — 빌드 시점의 계보로 적는다.
_MONO_COL_NOTE = (
    f"이 스윕의 β=0 칸이고 현재 모노 원장과 비트동일이다({_MONO_HIT}/{MONO_LIN['n']} 프레임)"
    if MONO_LIN["live_matches"] else
    "이 스윕의 β=0 칸이다 — 6 권 모노 편이 **그 스윕과 같은 격자 규약으로 냈던** 판이고, "
    f"현재 모노 원장과는 진폭 상관 {MONO_LIN['live']['amp_corr']:.3f} 로 다른 시계열이다")

#: 두 팔의 격자 간격 — SBR 은 광선 λ/12(`rcs_sbr.DEFAULT_DIV`), PO 는 점구름 λ/11
#  (`benchmark/report07b_bistatic_md.py:449` `SPACING_PO = rsb.C0 / FC / 11.0`).
_LAM_MM = 299792458.0 / M["fc_hz"] * 1e3
...
cells = [
    md("# 리포트 6-5 — 마이크로도플러: **바이스태틱**", "",
       f"> **도플러 축척은 이등분선의 수평투영을 따르고(순수 PO 대조팔 · 슬롯당 −20 dB 판 · "
       f"{len(ROWS)} 칸 상대 rms 오차 {DS['po_rms_rel_err_vs_pred_B'] * 100:.1f} %), "
       f"플래시 주기는 β 에 안 변한다({FR['n_total']} 칸 전부 공칭 "
       f"{FR['f_flash_nominal_hz']:.1f} Hz 격자 위 · 최대 이탈 "
       f"{FR['max_abs_dev_from_harmonic_hz']:.2f} Hz).**", "",
       f"> ⛔ **2026-09-06 — 「둘은 같이 안 움직인다」 는 이 자리에서 내렸다.** 두 관측량이 따로 "
       f"움직인다는 판정은 절 4 의 ⛔ 가 막았다 — 도플러 축척 열은 임계값 판을 15·20·25 dB 로 "
       f"바꾸면 갈리고, 플래시 쪽 자유 손잡이(띠 계수 · 조각 길이)를 흔든 판은 이 원장에 없다. "
       f"플래시 쪽에서 서는 것은 «이등분선 방위가 "
       f"{abs(R['az60']['bisector_az_deg'] - R['el60']['bisector_az_deg']):.1f}° 돈 한 짝"
       f"(`az60`↔`el60`)에서 플래시열 상관이 갈린다» 까지다(절 4).", "",
       "6 권의 앞 네 편(6-1~6-4)의 맵은 전부 **모노스태틱**이다 — 송신국과 수신국이 같은 자리에 있다. "
       "그런데 우리 실증은 **패시브 바이스태틱**이다: 조명은 남의 기지국이 하고 우리는 떨어진 "
       "곳에서 받는다. 그래서 «모노에서 본 무늬가 바이스태틱에서 그대로인가» 는 6 권 모노 편이 "
       ...
       "|---|---|",
       f"| 표적 | {M['name']} · 호버 |",
       f"| 조명 시선 û_i | 방위 {M['az_deg']:.0f}° · 앙각 {M['el_deg']:.0f}° (6 권 모노 편 그대로, **고정**) |",
       ...
       f"| 커널 — **SBR 팔** | `rcs_sbr.sbr_field_bistatic` — 바이스태틱 **복소장** E(û_i, û_s) · "
       f"가림 · 각도의존 Γ(θ) · 광선 격자 λ/12 ({_LAM_MM/12:.2f} mm) |",
       f"| 커널 — **순수 PO 대조팔** | `src/microdoppler.py::microdoppler_series` — 점구름 면적분 · "
       f"가림 없음 · Γ(θ) 꺼짐 · 점 간격 λ/11 ({_LAM_MM/11:.2f} mm) |",
       "",
       "⭐ **한 줄 요약 세 개** — 요약마다 어느 팔이 냈는지를 앞에 박았다.", "",
       f"1. (**순수 PO 대조팔**) **도플러는 줄어든다 — 그런데 교과서의 cos(β/2) 로는 아니다.** 날개끝 도플러의 축척은 "
       ...
       f"2. (플래시는 **SBR 팔** · 맞대는 도플러는 **순수 PO 대조팔** — 두 팔에 걸친 관측이다) "
       f"**플래시는 도플러와 따로 논다.** 플래시 **주기**는 β 가 어떻든 로터가 정한다"
       ...
       f"3. (**SBR 팔** · 면적은 그 팔의 광선 격자 센서스) **에코는 약해진다.** "
       f"조명게이트 ∩ 수신게이트에 남는 투영 유효면적이 β 120° 에서 "
       ...
       "⭐ **이 편이 돌린 팔은 둘이다** — SBR 팔(스윕 원장 npz 의 `E` 열)과 순수 PO 대조팔"
       "(`E_po` 열). "
       + ("Sionna PathSolver 열도 같은 스윕 원장에 있다. "
          if HAS_SIONNA_ARM else
          "Sionna PathSolver 가 나오는 자리는 절 2 의 **모노 원장 표** 하나이고, 이 β 스윕의 "
          "원장에는 그 열이 없다. ")
       + "절마다 어느 팔이 냈는지는 표 제목과 절 첫 문장에 박아 두었다.", "",
    ...
    md("## 절 0. 시나리오 — 무엇을 바꿨고 무엇을 안 바꿨나", "",
       ...
       "6 권 모노 편의 판과 사과-대-사과가 된다.", "",
       "**바꾼 것**: 수신 방향 û_s 하나. û_i 는 6 권 모노 편의 시선에 고정한 채, û_s 를 정해진 "
       "평면에서 β 만큼 연다.", "",
       "| 평면 | 무엇인가 | 왜 |",
       "|---|---|---|",
       ...
       f"{MONO_LIN['n']:,} 프레임 전부다. 즉 이 편의 β=0 칸은 6 권 모노 편이 "
    ...
    md("## 그림 2 — β 를 열었을 때 실제로 무엇이 나오나", "",
       ...
       "β=0 칸 — 6 권 모노 편의 그 원장이다. 각 패널의 흰 파선이 **예측 B**, 검은 점선이 "
       "**예측 A** 다. 아랫줄 다섯 칸이 수치 판정이다.", "",
       "⭐ 맵의 표시 규약은 `src/md_mapstyle.py` 가 강제한다 — 6 권 모노 편과 **같은 설정**이다"
       ...
       "안 갈린다. 그래서 판정은 (k) 칸이 맡는다.", "",
       "⭐ **맵 열 칸과 (k) 칸은 팔이 다르다.** 위 두 줄의 맵 열 칸은 **SBR 팔**(`E`)이 냈고, "
       "(k) 칸의 도플러 축척은 **순수 PO 대조팔**(`E_po`)이 냈다. 같은 잣대(시간분해 최대 도플러)를 "
       "두 팔에 각각 댄 것이 (k)·(l) 두 칸이고, 영문 캡션이 그 둘을 «two arms» 로 적는다."),
    ...
    md("## 절 1. 도플러 축척 — 이등분선이 이긴다", "",
       ...
       "**어느 팔에서 재나.** 판정은 **순수 PO 대조팔**에서 한다 — 그 팔은 날개끝에서 "
       f"스펙트럼이 절벽처럼 잘린다(바닥 {OOBE['po']['new_floor_rel_db_raw_mean']:.0f} dB · "
       "**모노 원장 기준** `outputs/outofband_power.json`). 같은 원장에서 광선 팔(SBR)의 바닥은 "
       f"{OOBE['sbr']['new_floor_rel_db_raw_mean']:.0f} dB 이고, **이 스윕 자신의** SBR 바닥은 "
       f"{max(_SBR_FLOORS):.1f} ~ {min(_SBR_FLOORS):.1f} dB 다(`rows[*].sbr_floor_rel_db`). "
       "두 잣대 어느 쪽으로 재도 광선 팔의 날개끝 가장자리는 바닥에 묻힌다 — 절 2 가 그것을 "
       "계량한다.", "",
       "⚠ 위 두 출처는 **판이 다르다** — 모노 원장은 얼린 광선 격자, 이 스윕은 자세마다 다시 "
       "정의하는 격자다. 그래서 두 숫자에 각각 출처를 박았고, 절대값을 이어 붙여 읽지 않는다.", "",
       ...
       "(회귀 ③ 이 β=0 에서 6 권 모노 편의 PO 팔과 비트동일임을 확인한다).", "",
    ...
    md("## 절 2. 광선 팔로는 이 축척을 못 잰다 — 그리고 그것은 바이스태틱 탓이 아니다", "",
       ...
       "⭐ **이것은 바이스태틱의 성질이 아니다.** 6 권 모노 편의 모노 원장에 있는 세 팔을 "
       "**같은 추정기**로 재면 바로 보인다.", "",
       ...
       "(그 전후 비교는 [리포트 6-2 «광선 격자를 어디에 매나»](06_2_engines.ipynb)), "
       ...
       "**광선을 쓰는 두 팔은 둘 다** 그 너머로 새어 나간다. 6 권 모노 편이 «다음에 할 것» 으로 "
       ...
       "**이 표의 출처는 `outputs/outofband_power.json` 이다** — 세 팔을 같은 잣대로 다시 "
       f"잰 원장이고(생성 {OOB['_meta']['generated'][:16]}), 이 편이 인용하는 바닥·대역밖 몫은 "
       "전부 그 파일의 `three_engines[*]` 에서 온다.", "",
       ...
       "[리포트 2 «우리 커널 — 무엇이고, 무엇이 아닌가»](02_kernel.ipynb) 가 잰다. "
       "이 편의 스윕은 아직 얼린 격자를 안 쓴다.", "",
       "⚠ 그렇다고 광선 팔의 맵이 틀렸다는 말이 아니다. 레벨(절 3)·플래시(절 4)·가림은 광선 팔이 "
       "맡는 것이고 이 편의 그 결과들은 전부 광선 팔에서 나온다. 못 하는 것은 **날개끝 "
       "가장자리의 정밀 측정** 하나다."),

    md("## 절 3. 레벨 — 게이트 두 개가 겹치는 만큼만 돌아온다 (**SBR 팔**)", "",
       "**어느 팔인가.** 이 절의 두 표는 전부 **SBR 팔**에서 나온다 — 에코 레벨은 `E` 의 "
       "슬로타임 평균 전력이고(`benchmark/report07b_bistatic_md.py:518`), 게이트 센서스는 "
       "그 팔의 광선 격자를 다시 쏘아 센 기하 계수다.", "",
       ...
       "",
       "**측정 에코는 그 면적만큼 곧이곧대로 줄지 않는다** (SBR 팔).", "",
       "| 라벨 | β | 에코 레벨 (모노 대비 · SBR 팔) | 진폭 p-p (SBR 팔) |",
       "|---|---|---|---|",
    ...
    md("## 절 4. 플래시 — 주기는 로터가, 시각은 방위가 정한다", "",
       "⭐ 이 절이 무엇으로 서는가 — **플래시열**이다. 주기는 β 에 안 변하고, 시각은 "
       "이등분선의 방위를 따라 밀린다. ⚠ 표에 나란히 실리는 «도플러 축척» 열은 **다른 "
       "팔**(순수 PO 대조팔)이 내고 임계값 판마다 값이 갈린다 — 두 열을 칸 대 칸으로 빼서 "
       "읽는 것은 아래 ⛔ 가 막는다. 아래 표의 열 제목에 팔을 박았고, 조건 차이는 표 밑에 "
       "적었다.", "",
       "**플래시 주기는 β 에 안 변한다** (SBR 팔).", "",
       ...
       "**플래시 시각은 이등분선의 방위가 정한다** (SBR 팔).", "",
       "| 라벨 | β | 이등분선 방위 | 도플러 축척 — **순수 PO 팔** | "
       "플래시열 상관 (β=0 대비) — **SBR 팔** | 플래시 밀림 — **SBR 팔** |",
       "|---|---|---|---|---|---|",
       "\n".join(f"| `{k}` | {R[k]['beta_deg']:.0f}° {R[k]['plane']} | "
       ...
       "",
       "⚠ **두 열은 조건이 다르다.**", "",
       "| 열 | 팔 | 반송파 | 격자 | 가림 | Γ(θ) | 코드 |",
       "|---|---|---|---|---|---|---|",
       f"| 도플러 축척 | 순수 PO 대조팔 | fc·cos(β/2) (이등분선 시선의 모노 등가) | "
       f"점구름 λ/11 ({_LAM_MM/11:.2f} mm) | 없음 | 꺼짐 | "
       "`microdoppler_series` (`report07b_bistatic_md.py:465`) |",
       f"| 플래시열 상관 · 밀림 | SBR 팔 | {M['fc_hz']/1e9:.1f} GHz 고정 | "
       f"광선 λ/12 ({_LAM_MM/12:.2f} mm) | 있음 | 켜짐 | "
       "`flash_envelope(E[:,j])` (같은 파일 `:513`) |",
       "",
       "두 팔이 같이 쓰는 것은 슬로타임 격자·자세·로터별 회전수다. 커널·격자·재질 규약은 "
       "위 표대로 갈린다.", "",
       f"⚠ 플래시 띠의 폭은 각 β 의 **예측 축척 B** 로 따라간다(`:512`) — 절 1 의 판정이 이 절의 "
       "관측량에 이미 들어가 있다.", "",
       _PO_FLASH_LINE, "",
       "⭐ 표를 가로로 읽지 말고 **두 세로줄을 비교**하라 — 다만 비교가 서는 세로줄은 "
       "**플래시 쪽 하나**다.",
       f"· 두 칸은 β 가 같고({R['az60']['beta_deg']:.0f}°), 다른 것은 이등분선의 "
       f"**방위가 돌았는가** 뿐이다 — `az60` 은 {R['az60']['bisector_az_deg']:.1f}°, "
       f"`el60` 은 {R['el60']['bisector_az_deg']:.1f}° 다.",
       f"· 플래시열 상관은 `az60` **{R['az60']['flash_lag_corr']:.3f}** 대 "
       f"`el60` **{R['el60']['flash_lag_corr']:.3f}** 로 갈린다. **이 두 칸을 가르는 것은 "
       f"이등분선이 {abs(R['az60']['bisector_az_deg'] - R['el60']['bisector_az_deg']):.1f}° "
       "돌았는가 뿐이다.** ⚠ 다만 같은 표의 β "
       f"{R['az30']['beta_deg']:.0f}° 짝은 이등분선이 "
       f"{abs(R['az30']['bisector_az_deg'] - R['el30']['bisector_az_deg']):.1f}° 돌았는데도 "
       f"안 갈린다({R['az30']['flash_lag_corr']:.3f} 대 "
       f"{R['el30']['flash_lag_corr']:.3f}) — **얼마나 돌아야 플래시열이 무너지는지는 "
       "이 원장으로 못 정한다.**",
       f"· ⛔ **도플러 축척 열로는 두 칸이 안 갈린다.** 같은 임계값 판에서 맞대면 15 dB 판은 "
       f"{R['az60']['po_ratio_hi']:.4f} 대 {R['el60']['po_ratio_hi']:.4f} 로 **비트동일**이고, "
       f"25 dB 판은 {R['az60']['po_ratio_lo']:.4f} 대 {R['el60']['po_ratio_lo']:.4f} 로 "
       f"{abs(R['az60']['po_ratio_lo']-R['el60']['po_ratio_lo']):.4f} 차다"
       "(`rows[*].po_ratio_hi` · `po_ratio_lo`). 표에 인쇄된 20 dB 판의 차 "
       f"{abs(R['az60']['po_ratio']-R['el60']['po_ratio']):.4f} 는 **원장이 가진 세 판 가운데 "
       "한 판에만 있다.** 두 칸을 서로 빼서 읽지 마라.", "",
       "⚠ **그 열은 자유 파라미터 하나가 정한다.** 추정기 `inst_max_doppler` 는 슬롯당 낙차 "
       "`drop_db` 를 **20 dB 로 못 박고** 부른다(`benchmark/report07b_bistatic_md.py:216` · "
       "호출 `:506`). 그 값이 자유롭다는 것은 원장이 이미 안다 — 같은 행에 15 dB 판"
       "(`po_ratio_hi`)과 25 dB 판(`po_ratio_lo`)이 나란히 있다"
       f"(`:537`~`:540`). 세 판을 15·20·25 dB 순으로 늘어놓으면 `az60` 은 "
       f"{R['az60']['po_ratio_hi']:.3f}·{R['az60']['po_ratio']:.3f}·"
       f"{R['az60']['po_ratio_lo']:.3f} 이고 `el60` 은 {R['el60']['po_ratio_hi']:.3f}·"
       f"{R['el60']['po_ratio']:.3f}·{R['el60']['po_ratio_lo']:.3f} 이다 — `az60` 의 20 dB 값만 "
       "제 행의 두 이웃 판보다 위로 튄다.", "",
       "⚠ **왜 그 판에서만 한 칸이 튀는지 — 기전은 모른다.** 이 추정기는 슬롯마다 «그 슬롯의 "
       "최고 |f|» 를 STFT 격자에서 한 칸 골라 세고 그 시계열의 99 분위를 쓰므로, 값이 격자 "
       "위로 양자화되어 **계단으로** 움직인다. 어느 임계값에서 어느 칸이 살아남는지를 가르는 "
       "것이 무엇인지는 이 원장으로 못 정한다. 원장에는 15 · 20 · 25 dB 세 판밖에 없어 그 "
       "사이와 바깥이 어떻게 되는지도 **모른다**. **미해결로 둔다**(아래 «남긴 문제»).", "",
       "⭐ 이 절이 기대는 플래시열 상관은 임계값 잣대가 아니다 — 띠 안 에너지 b(t) 의 "
       "정규화 상호상관 최대값이다(`flash_envelope` `:278` → `best_lag` `:297`). "
       "⚠ 다만 그 열의 자유 손잡이(띠 계수 0.35·1.05 `:512` · 조각 길이 0.45 주기)를 흔든 "
       "판은 **이 원장에 없다** — 아래 «남긴 문제» 로 넘긴다.", "",
       "⭐ **주장의 크기.** 위 갈림은 **플래시열 상관 하나**로 서고 그 열은 SBR 팔이 낸다 — "
       "여기서 세우는 것은 «`az60` 과 `el60` 두 칸에서 플래시열 상관이 갈린다» 까지다. "
       "`az60`↔`el60` 의 **도플러 축척 대비**를 근거로 «두 관측량이 따로 움직인다» 를 "
       "세우는 것은 위 ⛔ 가 막았다. "
       "한 커널 안에서 같은 갈림이 서는지는 위 ⚠ 가 적은 대로 이 원장 밖이다.", "",
       f"⭐ **한 칸의 관찰** — `el90` 은 도플러 축척이 세 판 모두 "
       f"{min(R['el90'][_k] for _k in ('po_ratio_hi', 'po_ratio', 'po_ratio_lo')):.3f}~"
       f"{max(R['el90'][_k] for _k in ('po_ratio_hi', 'po_ratio', 'po_ratio_lo')):.3f} 로 "
       f"내려가 있는데 플래시열 상관은 {R['el90']['flash_lag_corr']:.3f} 다. ⚠ 이것은 "
       "**그 한 칸의 관찰**이고, 플래시 쪽 손잡이는 위 ⚠ 대로 안 흔들었다. 두 열이 언제나 "
       "따로 움직이는지는 이 원장으로 **못 정한다**.", "",
    ...
    md("## 절 5. 정직 — 이 편이 딛고 선 근사와, 안 한 일", "",
       "**① SBR 팔의 적용범위 한계** (`rcs_sbr.sbr_field_bistatic` 이 물려받은 것 그대로)", "",
       "\n".join(f"- {s}" for s in M["scope_limits"]), "",
       _RECIP_LINE, "",
       "**② 순수 PO 대조팔의 근사** — 절 1 의 도플러 축척 판정이 이 팔에서 나온다.", "",
       "- 가림 없음: 점구름 전면을 적분한다(설계상 대조군 — 아무것도 안 막는다).",
       "- 각도의존 Γ(θ) 꺼짐: `microdoppler_series` 의 `angle_gamma` 기본값이 False 이고 "
       "이 스윕은 그 기본값으로 부른다(`src/microdoppler.py:39` · `report07b_bistatic_md.py:465`).",
       "- 진폭 게이트·obliquity 를 이등분선 하나로 근사한다 — 두 게이트 (n̂·û_i)·(n̂·û_s) 각각이 "
       "아니라 (n̂·û_b) 하나다(같은 파일 446~448행). 도플러축은 위상이 정하므로 이 팔이 재는 "
       "**축척**에는 안 들어가고, 레벨은 SBR 팔이 맡는다.",
       f"- 점 간격을 λ/11({_LAM_MM/11:.2f} mm)로 **고정**한다(`SPACING_PO`, 같은 파일 449행) — "
       "β 마다 점구름이 같아야 비율이 선다.", "",
    ...
    md("## ⭐ 모노 ↔ 바이스태틱 대조표", "",
       "6 권 모노 편의 어느 수치가 어떻게 달라지나. β 는 방위면 기준이다. "
       "**행마다 낸 팔이 다르므로 «팔» 열을 먼저 읽는다.**", "",
       "| 무엇 | 팔 | 6 권 모노 편 | 이 편 β 30° | β 60° | β 90° | β 120° | 무엇이 그렇게 만드나 |",
       "|---|---|---|---|---|---|---|---|",
       f"| 날개끝 도플러 f_tip (**예측 B**) | 해석(운동학) | **{M['f_tip_mono_hz']:.0f} Hz** | "
       + " | ".join(f"{R[k]['f_tip_pred_B_hz']:.0f} Hz" for k in AZK)
       + " | 이등분선의 수평 성분 |",
       f"| 도플러 축척 (**실측**) | 순수 PO 대조팔 | 1.000 | "
       + " | ".join(f"{R[k]['po_ratio']:.3f}" for k in AZK) + " | 위와 같음 |",
       f"| 플래시율 f_flash | SBR 팔 | **{M['f_flash_hz']:.1f} Hz** | "
       + " | ".join(f"{R[k]['f_flash_meas_hz']/max(1, round(R[k]['f_flash_meas_hz']/M['f_flash_hz'])):.1f} Hz"
                    for k in AZK)
       + " | **안 변한다** — 로터 회전수가 정한다 |",
       "| 플래시열 상관 (β=0 대비) | SBR 팔 | 1.000 | "
       + " | ".join(f"{R[k]['flash_lag_corr']:.3f}" for k in AZK)
       + " | 이등분선 **방위**가 돌면 정렬 시각이 밀린다 |",
       "| 에코 레벨 | SBR 팔 | 0.0 dB (기준) | "
       + " | ".join(f"{R[k]['level_db_rel_mono']:+.1f} dB" for k in AZK)
       + " | 게이트 겹침 + 코히어런트 간섭 |",
       "| 게이트 유효면적 A_eff | SBR 팔 격자 기하 | "
       f"**{cen('azimuth', 0)['A_eff_m2']*1e4:.0f} cm²** | "
       + " | ".join(f"{cen('azimuth', b)['A_eff_rel_mono']*100:.0f} %" for b in BET)
       + " | 두 게이트의 겹침 |",
       "| 거리 관측량 | 기하 | 표적까지 거리 (원) | "
       "← 송·수신 **거리의 합** (타원) → | | | | 초점이 둘 |",
       f"| 슬로타임 격자 | 두 팔 공통 | {M['n']:,} @ {M['prf_hz']:.0f} Hz | "
       "← **같음** → | | | | 설계상 고정 |",
       f"| 표시 규약 | 표시 | flash_spec 0.45 주기 | ← **같음** → | | | | "
       "`md_mapstyle` 가 강제 |",
       "",
       f"⚠ **1 행은 예측, 2 행은 실측이다** — 같은 양의 두 판이고 방위면에서 최대 "
       f"{max(abs(1 - R[k]['po_over_predB']) for k in AZK)*100:.1f} % 어긋난다"
       f"(`{max(AZK, key=lambda k: abs(1 - R[k]['po_over_predB']))}` · `rows[*].po_over_predB`). "
       "두 행을 한 줄로 이어 읽지 않는다.", "",
       "⚠ **2 행(도플러 축척 실측)은 임계값 `drop_db` = 20 dB 판이다.** 같은 원장의 "
       f"15 dB 판(`po_ratio_hi`) · 25 dB 판(`po_ratio_lo`) 폭은 β 30° "
       f"{R['az30']['po_ratio_hi']:.3f}~{R['az30']['po_ratio_lo']:.3f} · β 60° "
       f"{R['az60']['po_ratio_hi']:.3f}~{R['az60']['po_ratio_lo']:.3f} · β 90° "
       f"{R['az90']['po_ratio_hi']:.3f}~{R['az90']['po_ratio_lo']:.3f} · β 120° "
       f"{R['az120']['po_ratio_hi']:.3f}~{R['az120']['po_ratio_lo']:.3f} 다. β 90° 의 "
       "인쇄값은 그 폭 안이고 β 120° 는 폭의 끝에 걸쳐 있는데, β 30°·60° 의 "
       f"{R['az30']['po_ratio']:.3f}·{R['az60']['po_ratio']:.3f} 은 그 **밖**이다.", "",
       "⚠ 그래서 이 행에서 확실한 것은 두 가지다. ① 세 판(15·20·25 dB) 모두에서 β 가 "
       "커질수록 값이 **작아지거나 같다** — 이 세 판 안에서 그 방향은 안 뒤집힌다. "
       f"② β 30° 와 60° 는 15 dB 판({R['az30']['po_ratio_hi']:.4f} 대 "
       f"{R['az60']['po_ratio_hi']:.4f})과 25 dB 판({R['az30']['po_ratio_lo']:.4f} 대 "
       f"{R['az60']['po_ratio_lo']:.4f})에서 **비트동일**이다 — 인쇄된 20 dB 판의 차 "
       f"{abs(R['az30']['po_ratio']-R['az60']['po_ratio']):.4f} 만 보고 그 둘을 갈라 읽지 "
       "않는다.", "",
       f"⚠ **«6 권 모노 편» 열의 정체.** f_tip·f_flash 칸은 운동학 공칭값이고, 나머지 칸은 "
       f"{_MONO_COL_NOTE}. 이 표를 떼어 인용할 때는 이 줄을 같이 가져간다.", "",
       "⭐ **한 줄로**: 바이스태틱이 되면 **도플러축이 눌리고 에코가 약해지지만, 플래시 격자는 "
       "그대로 남는다**. 분류기가 기댈 특징 중 «회전수» 계열은 기하에 강건하고, «날개끝 주파수» "
       "계열은 기하를 알아야 쓸 수 있다는 뜻이다."),

    md("## 부록 — 그림별 데이터 이력", "",
       "⭐ Γ(θ) 칸은 **팔마다 다르다** — 그래서 그림 2 를 팔별로 쪼개 적는다.", "",
       "| 그림 | 팔 | 원장 | 생성 | Γ(θ) | 로터 산포 |",
       "|---|---|---|---|---|---|",
       f"| 1 (기하) | 해석 | 수치 없음 — β·φ·이등분선만 "
       f"`outputs/report07b_bistatic_md.json` 에서 읽는다 | {M['generated'][:16]} | — | — |",
       f"| 2 (맵 열 칸 · (l)~(o)) | SBR 팔 | `outputs/report07b_bistatic_md.{{json,npz}}` 의 `E` | "
       f"{M['generated'][:16]} | ✅ 켜짐 (커널 기본값) | "
       f"±{M['rpm_spread_frac']*100:.2f} %(PX4 **SITL**(시뮬) 로그 중간값 — 6 권 모노 편 상속) |",
       f"| 2 의 (k) 칸 (도플러 축척) | 순수 PO 대조팔 | 같은 원장의 `E_po` | "
       f"{M['generated'][:16]} | ⛔ 꺼짐 (`microdoppler_series` 의 `angle_gamma` 기본값 False) | "
       "같음 |",
       f"| 2 의 회귀·바닥 비교 | 두 팔 + 모노 원장 | + `outputs/report07_three_engines.npz` | "
       + (f"{TRI['_meta']['generated'][:16]} · 재계산 {TRI['_meta']['regenerated'][:16]}"
          if TRI["_meta"].get("regenerated") else TRI["_meta"]["generated"][:16])
       + " | 팔별로 위와 같음 | 같음 |",
       f"| 절 2 의 세 팔 표 | 모노 원장의 세 팔 | `outputs/outofband_power.json` | "
       f"{OOB['_meta']['generated'][:16]} | 팔별로 위와 같음 | 같음 |",
       "",
       "이 편의 원장은 **6 권 모노 편의 원장에서 파생**된다 — `_meta` 를 읽어 세팅을 맞추고, "
       "β=0 열을 그 원장과 대조한다(절 0 회귀 표). 6 권 모노 편의 원장이 다시 계산되면 이 편도 "
       "다시 돌려야 한다."),

    md("## 부록 — 그림을 만든 설정 전부", "",
       "그림 안에는 무늬를 읽는 데 필요한 표시 설정만 한 줄로 적는다. 계산 설정 전부는 여기 있다.", "",
       "**계산 — 두 팔의 설정은 갈린다.** 격자·재질·가림이 팔마다 다르므로 열을 갈라 적는다.", "",
       "| 항목 | **SBR 팔** (`sbr_field_bistatic`) | **순수 PO 대조팔** (`microdoppler_series`) | 근거 |",
       "|---|---|---|---|",
       f"| 기체 | {M['name']} | 같음 | 1차 실측 표적 |",
       f"| 반송파 | {M['fc_hz']/1e9:.1f} GHz 고정 | fc·cos(β/2) — 이등분선 시선의 모노 등가 | "
       "5G n78 · 절 1 의 위상 항등식 |",
       f"| 시선 | 조명 û_i 방위 {M['az_deg']:.0f}° · 앙각 {M['el_deg']:.0f}° + 수신 û_s | "
       "이등분선 û_b 하나 (β 마다 다시 푼다) | 6 권 모노 편 상속 |",
       f"| 로터별 회전수 | {', '.join(f'{x:.0f}' for x in M['rpm_per_rotor'])} rpm "
       f"(산포 ±{M['rpm_spread_frac']*100:.2f} %) | 같음 | 6 권 모노 편 상속 "
       "(PX4 **SITL**(시뮬) 로그 중간값 — 실기 텔레메트리가 아니다) |",
       f"| 로터 회전 방향 | {M['rotor_dirs']} · 블레이드 {M['prop_blades']}장 | 같음 | 기체 CAD |",
       f"| PRF | {M['prf_hz']:.0f} Hz | 같음 | 6 권 모노 편 상속 (16×f_tip) |",
       f"| 표본 수 | {M['n']:,} = {M['n']/M['prf_hz']*1e3:.0f} ms = "
       f"{M['blade_periods']:.1f} 블레이드 주기 | 같음 | 6 권 모노 편 상속 |",
       f"| 수신 방향 수 | {M['n_rx_dirs']} (방위면 5 + 앙각면 4) — 조명은 û_i 로 **한 번만** 쏘고 "
       "수신방향마다 위상합 + 그림자광선 1발만 반복한다 | "
       f"{M['n_rx_dirs']} (β 마다 이등분선 하나) | 스윕 설계 |",
       f"| 격자 | 광선 간격 λ/12 ({_LAM_MM/12:.2f} mm) | 점구름 간격 λ/11 "
       f"({_LAM_MM/11:.2f} mm) · β 마다 **고정** | `rcs_sbr.DEFAULT_DIV` (spacing 미지정) ↔ "
       "`SPACING_PO = rsb.C0 / FC / 11.0` (`benchmark/report07b_bistatic_md.py:449`) |",
       "| jitter | **없음** (단일 격자) | 광선 격자를 안 쓴다 | 위상을 쓰는 하류라 격자 평균이 "
       "의미를 바꾼다 |",
       "| 가림 | **있음** — 출사가시성 + 그림자광선 | **없음** — 점구름 전면 적분(대조군) | "
       "`rsb._exit_visible` ↔ 설계 |",
       "| 재질 Γ | 각도의존 Γ(θ), 국소 입사각 cosθ = n̂·û_i | 꺼짐 (`angle_gamma` 기본값 False) | "
       "모노 경로와 같은 규약 ↔ `src/microdoppler.py:39` |",
       f"| 계산 시간 | {M['seconds_field']:.0f} s = {M['seconds_per_pose']*1e3:.0f} ms/자세 "
       f"(수신방향 {M['n_rx_dirs']}개 동시) | **원장 없음** — `_meta.seconds_field` 는 SBR 팔만 잰다 | "
       f"GPU {M['gpu']} · {M['host']} |",
       f"| 동체선 제외 폭 | \\|f\\| < {M['f_body_cut_hz']:.0f} Hz | 같음 (추정기 쪽 설정) | "
       "0.15 × 모노 f_tip |",
       "",
       "**표시 — `src/md_mapstyle.py` 가 강제한다 (6 권 모노 편과 같음)**", "",
       "| 항목 | 값 |",
       "|---|---|",
       ...
       "| 무엇 | 팔 | 어떻게 |",
       "|---|---|---|",
       f"| 도플러 축척 (주) | 순수 PO 대조팔 | "
       f"{DS['primary_estimator'].replace('순수 PO 대조팔에 ', '')} |",
       "| 플래시율 | SBR 팔 | 날개끝 띠의 시간축 에너지 b(t) 의 지배 선주파수 |",
       "| 플래시 밀림 | SBR 팔 | b(t) 와 β=0 의 b₀(t) 의 정규화 상호상관 최대 자리 (±½주기 안) |",
       "| 게이트 센서스 | SBR 팔 격자 기하 | 커널의 광선 격자를 다시 쏘아 두 게이트 ∩ "
       "출사가시성에 남는 히트 수 × d² |",
       "| 대역밖 전력 (절 2) | 모노 원장의 세 팔 | 평활 없는 Hann 주기도(4배 제로패딩)의 절대 대역 전력 "
       "P_out = Σ\\|X(f)\\|², \\|f\\| > f_tip. 정규화는 **전체 전력** "
       "⟨outputs/outofband_power.json : new_definition.headline⟩ |",
       "| 날개끝 밖 바닥 (절 2) | 모노 원장의 세 팔 | 같은 주기도에서 f_tip 밖 구간의 평균을 "
       "봉우리 대비로 ⟨outputs/outofband_power.json : three_engines[0].new_floor_rel_db_raw_mean⟩ |",
       "| 이 스윕의 SBR 바닥 (절 1) | SBR 팔 | 같은 잣대를 이 편의 β 열에 대어 잰다 "
       "⟨outputs/report07b_bistatic_md.json : rows[*].sbr_floor_rel_db⟩ |"),
    ...
    md("## 부록 — 코드 발췌", "",
       ...
       "(가림 없음 · Γ(θ) 꺼짐 · 점 간격 λ/11)", "",
       "```python",
       "SPACING_PO = rsb.C0 / FC / 11.0                         # 점구름 λ/11 — β 마다 고정",
    ...
    md("## 부록 — 재현", "",
       ...
       "",
       "# ② 그림 둘",
       "PYTHONPATH=src /workspace/.venvs/py312/bin/python benchmark/build_bistatic_geom_fig.py",
       "PYTHONPATH=src /workspace/.venvs/py312/bin/python benchmark/build_bistatic_md_fig.py",
       "",
       "# ③ 이 노트북",
       "PYTHONPATH=src /workspace/.venvs/py312/bin/python src/make_report07b_bistatic.py",
       "```", "",
       "**딸린 파일**", "",
       "| 무엇 | 어디 |",
       "|---|---|",
       ...
       "안 넘긴다 |",
       "| 순수 PO 팔의 플래시 관측량 | 이 원장의 플래시는 SBR 팔의 `E` 하나로만 쟀다 — "
       "«한 커널 안에서도 도플러와 플래시가 갈리는가» 는 **원장 없음**이다. `E_po` 에 "
       "`flash_envelope` 를 한 번 대면 닫힌다(계산은 이미 있는 npz 를 읽는 일이다) |",
       ...
       "| 플래시열 상관의 창 민감도 | 절 4 의 `az60`↔`el60` 갈림이 이 열 **하나**에 걸려 "
       "있는데, 띠 계수(0.35·1.05)와 조각 길이(0.45 주기)를 흔든 판이 이 원장에 없다. "
       "`flash_envelope` 를 계수 격자 위에서 한 번 더 돌리면 닫힌다(이미 있는 npz 를 읽는 "
       "일이다) |",
       "| 도플러 축척 열의 임계값 격자 | 원장에 15 · 20 · 25 dB 세 판만 있다. `drop_db` 를 "
       "촘촘히 흔든 격자가 있어야 절 4 의 «20 dB 판에서만 튄다» 가 세 점이 아니라 곡선으로 "
       "보인다 |",
       "| 2채널(ECA·CAF) 처리 | 이 편의 설계상 제외 — 그 축은 "
       "⛔«기준채널이 현실이면 얼마를 잃는가» 별편(2026-09-03 내림 — 동작점이 실내 통제 기하다, `archive/chamber_0903/`) 가 기준채널 잡음·다중경로로 연다. "
