...
def blocks_67() -> list:
    return [
        ...
        md("## X410 한 대가 기준과 감시를 동시에 든다", "",
           ...
           "돈다. 그 가정 하나를 푼 편이 "
           "⛔«기준채널이 현실이면 얼마를 잃는가» 별편(2026-09-03 내림 — 동작점이 실내 통제 기하다, `archive/chamber_0903/`) 이고, "
    ...
    return [
        header(
            ...
            results=[
                ...
                # ⛔+47.2 % 를 단독으로 적지 않는다(적대 검증 2026-09-01 · docs/AUDIT_REPORTS_0901.md).
                #   그 수는 **한 설정**(생산 격자 div=16·drone-like 합성표적·120 자세)에서 짝지어
                #   잰 비율의 중앙값이고, 원장 자신이 D_cost.measurement_caveat 에
                #   「남의 100 % util 작업과 공유해 단발 wall-clock 이 2.5 배까지 흔들렸다 …
                #   ratio_min<1 인 반복이 남아 있는 것은 경합 잡음이다」 라고 적어 두었다.
                #   모서리항의 per-pose 비용은 거의 상수인데 면적분은 격자² 로 커지므로
                #   **백분율은 격자가 정한다.**
                f"PTD(모서리 회절) 행도 `결판` 이다 — 켠 비용은 생산 격자(div=16)에서 짝지어 잰 "
                f"비율의 중앙값이 "
                f"{PW.num('verdict.cost_increase_pct', fmt='{:+.1f}', unit='%')} 인데, "
                f"⚠**이 백분율은 격자가 정한다** — 모서리항의 per-pose 비용은 거의 상수이고 "
                f"면적분은 격자²로 커진다. 벽시계도 남의 작업과 공유한 GPU 에서 쟀다"
                f"(원장 `D_cost.measurement_caveat`). 그리고 정면입사만 재면 판별력이 0 이라 "
                f"비스듬한 입사를 함께 잰다.",

                # ⛔「밴드평균」이 어느 창인지 안 적으면 못 읽는 수다(적대 검증 2026-09-01).
                #   저 둘은 **앵커 논문의 측정범위 1.8~18.2 GHz** 평균인데, 이 캠페인이 송신하는
                #   창은 1.843~5.21 GHz 뿐이다. 같은 잔차를 1.8~6.0 GHz 로 자르면
                #   v1 −5.78 · v2 −5.16 dB 이고, 전대역 개선폭 1.61 dB 중 대역 안에 남는 것은
                #   0.62 dB 다. 옆 행의 기울기 문턱 1.77 dB 는 이미 밴드정합값을 쓴다.
                #   ⭐v2 잔차는 +0.192 ± 0.039 dB/GHz (5.0σ) 로 주파수 구조가 있다.
                #     v1 은 +0.061 ± 0.071 (0.9σ) 라 v1 단독으로는 유의하지 않다 — 그렇게 적는다.
                f"레벨 행의 눈감기 사전값은 고도정합 실측곡선 대비 **앵커 전대역"
                f"(1.8\u201318.2 GHz)** 평균으로 "
                ...
                f"\u26a0**단 이 캠페인이 송신하는 창은 1.843\u20135.21 GHz 뿐이다** \u2014 같은 잔차를 "
                f"1.8\u20136.0 GHz 로 자르면 v1 \u22125.78 dB \u00b7 v2 \u22125.16 dB 이고, 전대역 개선폭 "
                f"1.61 dB 중 대역 안에 남는 것은 0.62 dB 다. v2 잔차에는 "
                f"+0.192 \u00b1 0.039 dB/GHz (5.0\u03c3)의 주파수 구조가 있어 전대역 평균이 우리 대역의 "
                f"결손을 1.86 dB 낮춰 적는다(v1 은 0.9\u03c3 로 유의하지 않다). 원장 자신이 "
                f"「전대역 평균이 구조를 감춘다」와 「규약 분기가 0.93 dB \u2014 단일값으로 적으면 "
                f"거짓 정밀이다」를 적어 두었다.",
            ],
            method=[
                ("표의 네 번째 열",
        ...
        md("## 모서리 회절 행 — 어느 각도에서 재는가", "",
           ...
           # ⛔같은 이유로 여기도 창을 적는다(적대 검증 2026-09-01)
           f"레벨 행의 눈감기 사전값도 여기 붙는다 — 고도정합 실측곡선 대비 "
           f"**앵커 전대역(1.8\u201318.2 GHz)** 평균으로 "
           f"{P3.num('residual.vs_yuan_theta90_measured_curve.mean_db', fmt='{:+.2f}', unit='dB')} "
           f"(v1 메쉬)이고, 사진 실측 v2 메쉬에서는 "
           f"{PV2.num('v1_vs_v2.level_db.v2', fmt='{:+.2f}', unit='dB')} 다."),
        ...
        next_steps([
            ...
            ("같은 세션에서 모서리가 많은 표준체를 **정면입사 밖의 각도까지** 함께 잰다",
             ...
             # ⛔여기도 격자 의존을 적는다(적대 검증 2026-09-01)
             f"{PW.num('verdict.cost_increase_pct', fmt='{:+.1f}', unit='%')} "
             f"(생산 격자 div=16 · 격자에 따라 달라진다)",
