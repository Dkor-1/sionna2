...
==========================================================================================
⛔⛔**이 빌더는 옛 세대다**(2026-09-04 표시). `src/build_volumes.py` 가 이미 적어 뒀다 —
   「옛 조각 `00_map` 은 «78 편 · 12 부» 라는 **폐지된 구조 자체를 설명하는 글**이라
   재배선으로 살릴 수 없다」. 1 권이 쓰는 조각은 ["13","75"] 뿐이라 이 산출물은 **어느
   권에도 들어가지 않는다.** 아래 «78 편 · 12 부» 는 전부 **옛 편성 기준**이다.
편 00 이 답하는 질문 하나
    **무엇부터 읽어야 하나.**
...
⚠ 이 편은 «신규» 다 — 옛 셀에서 오지 않고 README 와 각 편의 여는 문장에서 조립한다.
   그래서 숫자는 전부 `outputs/reports_index.json`(= ⚠**옛 편성 기준** 편 78개를 세어 만든
   색인)에서
...
def report_00_map():
    ...
    return [
        header(
            ...
            did="⚠**옛 편성 기준**(폐지) — 편 78개를 부 12개로 묶고, 읽는 목적마다 다른 "
                "진입 경로 셋을 깔았다.",
            results=[
            ...
            method=[
                ...
                ("경로", "읽는 목적이 다르면 순서도 다르다. 세 경로가 같은 편들을 서로 다르게 "
                      "가로지른다(⚠개수는 옛 편성 기준이다)"),
            ],
