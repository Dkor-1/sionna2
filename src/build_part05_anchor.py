...
def report_27_box_sphere_control():
    return [
        header(
            num=27,
            #: ⛔2026-09-04 — 전 제목은 «절대 크기가 **아니라** 각도 구조다» 라는 판정형인데,
            #  같은 편 본문이 앞 절반을 깎는다 — 「레벨 축에서 우리 메쉬는 상자 계열을 **전부
            #  이긴다**(정육면체 +6.22 ↔ 우리 −3.30 dB)」 이고 「부피를 맞게 고른 구가
            #  우리보다 낫다(+0.96 dB)」. 갈리는 것은 레벨이 아니라 각도 산포다.
            title="레벨 축에서 상자 계열은 우리 메쉬에 지고, 부피를 맞게 고른 구는 같은 "
                  "자리에 온다 — 갈리는 것은 각도 산포다",
