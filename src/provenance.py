...
GLOSSARY_DEFAULT = [
    ...
    ("SBR", "Shooting-and-Bouncing Rays. **광선(GO)으로 조명면·가림을 찾는 기하 단계** — Sionna 의 Mitsuba 엔진이 하는 부분. 그 위 산란적분(밝기)은 PO 가 낸다. 상용 EM 솔버 SBR+ 의 표준 결합"),
...
def provenance_cells(report: str, what: str, question: str = "",
    ...
    if engines:
        ...
        R += ["",
              ...
              "> - **표적 RCS** 는 🟡 우리가 얹은 **PO(물리광학 표면적분)** 가 냅니다 — Sionna 기본 solver 엔 이 산란적분이 없어 경로 이득만 줄 뿐 RCS 를 못 내기 때문입니다. "
              "광선을 쏴 조명면·가림을 찾는 **SBR** 은 Sionna 의 **Mitsuba 3 엔진을 그대로** 쓰고, 그 위에 **PO 적분만 우리가** 얹습니다(SBR+PO).",
