...
cells.append(md(
    ...
    f"| 바닥 입사각 (법선 기준) | {AP['theta_i_deg']:.0f}° — 스침각이 아니라 꽤 비스듬히(≈45°) 때린다 |",
...
cells.append(mdl([
    ...
    "신호-대-클러터비(SCR)를 다시 재 보면(기준 ×1 = −26 dB 는 RT 실측(−14.7 dB, 최강 −9.8 dB)보다 11~16 dB **약하게 잡은 과소평가**지만, +40 dB 스윕이 실측값까지 덮으므로 결론은 클러터 세기와 무관하다 — "
    "RT 실측 바닥반사 −14.7 dB(§2.1)보다도 약하지만, 이 +40 dB 스윕이 그 실측값까지 전부 덮는다):",
    "",
