...
cells = [
    ...
    md("## §0. 시나리오 — 그림을 보기 전에", "",
       ...
       "",
       "⚠ **3 m 는 근거리장이다** — 이 기체의 원거리장 경계는 2D²/λ = **8.0 m** 다. "
    ...
    md("## 부록 — 그림을 만든 설정 전부", "",
       ...
       f"| 자세 | 방위 {LEAD['az_deg']:.0f}° · 앙각 {LEAD['el_deg']:.0f}° | 지상 레이더는 아래에서 본다 |",
       f"| 거리 | {TM['range_m']:.0f} m (세 엔진 비교) | Sionna 씬 배치 |",
       f"| 호버 회전수 | {HOV['rpm0']:.0f} rpm | Phantom 3 실측 C_T 로 푼 값 |",
       f"| 로터별 산포 | ±{HOV['static_spread']:.2%} | ⭐PX4 텔레메트리 실측(0.07~0.29 %) |",
       f"| 제어루프 흔들림 | ±{HOV['wobble_amp']:.2%} @ {HOV['wobble_hz']:.1f} Hz | ⚠선언된 가정 — 비행 로그가 채운다 |",
       f"| PRF | {HOV['prf_hz']:.0f} Hz = 4×f_tip | 나이퀴스트(2×f_tip)의 두 배 여유 |",
       f"| 관측 창 | {HOV['seconds']:.0f} s = {HOV['n']:,} 표본 | 능선의 시간 변화를 보려고 |",
