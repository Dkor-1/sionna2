...
_RBT = f"{_ROOT}/outputs/report07_ray_budget_test.json"
RBT = json.load(open(_RBT)) if os.path.exists(_RBT) else None   # 광선예산 결판 시험
...
cells = [
    md("# 리포트 07 — 마이크로도플러 한눈에 보기", "",
       ...
       "| ⭐9 | Sionna 경로해는 거리에 무너지지만 **광선을 규칙대로 쏘면 되살아난다** | 아래 |",
    ...
    md("## 그림 2 — 두 엔진이 날개끝 아래에서 겹치고 위에서 갈린다", "",
       ...
       f"⛔그 초과분이 블레이드에서 오지 않는다는 판정은 철회한다(2026-09-06) — 그것이 "
       f"무엇에서 오는지는 아직 안 쟀다. 가장자리를 자동으로 찾는 검출기는 그 초과분까지 "
       f"가장자리로 읽을 수 있다.", "",
    ...
    md("## 그림 3 — ⭐세 엔진을 같은 격자에, 거리 3 / 8 / 15 m", "",
       ...
       f"절반이다. 8 m 는 원거리장 경계(2D²/λ ≈ {FARFIELD_M:.1f} m) 바로 곁이다.", "",
       "| 엔진 | 변조 p-p | 무늬 |",
       "|---|---|---|",
       ...
       "",
       "⚠ **깊이는 p-p 로 읽지 마라.** 그림 제목에 p5~p95 를 앞세운 이유가 있다 — 고해상도"
       "(4,096 자세)로 가니 Sionna 팔의 p-p 가 거리마다 32.6 / 2.5 / 50.1 dB 로 요동쳤는데, "
       "그 요동은 물리가 아니라 **광선 표본화**다. 경로를 거의 못 찾은 자세 하나가 −186 dB 까지 "
       "떨어져 전폭을 혼자 정한다. 강건한 p5~p95 로 보면 7.5 / 1.1 / 20.2 dB 이고, 우리 팔은 "
       "p-p 7.9 ↔ p5~p95 3.6 dB 로 둘의 간격이 좁다 — **그 간격 자체가 안정성의 척도**다.", "",
       "⚠ 두 팔의 차이는 **가림만이 아니다.** 기본 팔은 Mitsuba 광선이 처음 맞은 지점만 "
       "적분하고(가림 공짜), 대조 팔은 점구름의 모든 면을 더한다(가림 없음) — 이산화 방식도 "
       "다르므로 이 비교는 **단일축이 아니다**. 가림만 떼어낸 깨끗한 축은 `report15b` 의 "
       "F(동체 Γ=0 — 막되 산란 안 함) ↔ G(동체 면만 제거, 정점은 남겨 광선 격자 동일) 이고, "
       "그 실측이 배 쪽에서 최대 −15.96 dB 다.", "",
       ...
       "광선 예산이 규칙 (R/3)² 를 **다 채우지 못했기** 때문이다(8 m 에서 0.72배) — 예산과 기하가 섞인 관측이라고 정직하게 적는다.", "",
       "**이 그림의 설정** — 재계산·재현:", "",
       "```bash",
    ...
    md("## 그림 9 — 거리를 늘리면 Sionna 경로해가 어떻게 무너지나", "",
       ...
       "거리마다 늘렸다 — 1M · 8M · 16M · 32M. ⚠**정직 정정(2026-08-10 적대적 검증)**: 본문이 "
       "한때 «(R/3)² 로 보상했다» 고 적었는데 실제 예산은 규칙값의 1.00 / 0.72 / 0.36 / "
       "**0.18배**였다. 그래서 40 m 결과가 «표적 입체각 축소» 인지 «광선 부족» 인지 "
       "분리되지 않았고, **별도 시험으로 결판냈다 — 광선 부족이었다**(아래 표).", "",
       embed("report07_f9"), "",
       ...
       "경로 0** 이었다. ⚠**그런데 그것은 구조가 아니라 광선 부족이었다** — 같은 40 m 에서 "
       "광선만 사다리로 올리니 빈 자세가 **77.3 % → 0.0 %** 로 사라졌다(32M → 64M 에서 이미). "
       "PathSolver 는 TX 에서 광선을 뿌리고 그 광선이 표적을 맞혀야 경로가 생기며, "
       "표적이 차지하는 입체각은 (D/R)² 로 줄어든다 — 그 축소는 실재하지만, **광선을 그만큼 "
       "부으면 보상된다.** 40 m 규칙값은 178M 이고 그 절반(64M)에서 이미 빈 자세가 사라졌다.", "",
       "| 40 m 광선 | 경로 중앙 | 경로 평균 | 빈 자세 |",
       "|---|---|---|---|",
       "\n".join(f"| {k} | {v['paths_median']:.0f} | {v['paths_mean']:.2f} | {v['zero_frac']:.1%} |"
                 for k, v in RBT["ladder"].items()) if RBT else "| — | — | — | — |",
       "",
       "⭐ **그러면 무엇이 남는가 — 비용이다.** 거리가 두 배면 광선을 네 배 부어야 같은 경로 수를 "
       "얻는다. 40 m 한 자세에 178M 광선이면 100 m 에서는 그 여섯 배가 필요하고, 자세를 수천 개 "
       "쌓는 마이크로도플러에서는 그 곱이 감당 밖으로 간다. "
       "반면 우리 SBR+PO 는 광선 격자를 **표적에 앵커**(λ/12 간격)한 평면파 조명이라, "
       f"원거리장 경계({FARFIELD_M:.1f} m) 밖에서는 무늬가 거리에 **불변**이고 수신 레벨만 1/R² 로 "
       "내려간다 — 즉 **한 번 계산하면 거리에 무관**하다. 마이크로도플러를 원거리에서 물을 때 "
       "이 축을 SBR+PO 가 맡는 이유는 «PathSolver 가 못 해서» 가 아니라 **«비용이 거리와 함께 "
       "터지기 때문»** 이다.", "",
       "⚠ 정직 표시 셋. ⓪ 위 4거리 맵(그림 9)은 **예산이 부족한 판**이다 — 결판 시험이 그것을 "
       "밝혔으므로, 이 맵의 40 m 열은 «PathSolver 의 한계» 가 아니라 «우리가 광선을 덜 쏜 "
       "결과» 로 읽어야 한다. ① 40 m 맵은 자기 최대 정규화라 몇 안 되는 «경로 있는 자세» 만 "
       ...
       "| 광선 수 | 3 m 1M · 10 m 8M · 20 m 16M · 40 m 32M (규칙 (R/3)² 대비 1.00/0.72/0.36/0.18배) |",
    ...
    md("## 그림 10 — 같은 채널에 5G 파형을 실제로 태우면", "",
       ...
       "⚠ **이 수치가 무엇을 재는지 정확히 적는다**(2026-08-10 적대적 검증의 지적). "
       f"{WM['full_capture_max_rel_err']:.1e} 는 **OFDM 수신 사슬이 채널을 정확히 되돌려주는가**"
       "(기지 심볼 나눗셈 Y/X → 부반송파 평균)를 잰 것이다. 평탄-채널 근사 아래에서 그 사슬은 "
       "항등식이므로 이 값이 작은 것은 **구현이 맞다는 확인**이지, 5G 와 CW 가 같은 물리를 "
       "본다는 **독립 검증이 아니다** — 같은 물리라는 것은 평탄 채널 모형이 이미 가정한다. "
       "참고로 28 kHz 로 올렸다 5 kHz 로 되돌리면 보간 왕복 오차가 3.2e-2 남는다. "
       ...
       "⭐ **네 패널은 축이 완전히 같다**(시간 0~0.3 s · 도플러 ±1300 Hz — 2026-08-10 "
    ...
    md("## 부록 — 그림별 데이터 이력", "",
       ...
       + ("" if TRR is None else
          ...
          f"2026-08-10 | ✅ 후 | ±{TM['rpm_spread_frac']*100:.2f} %(그림 3 격자 상속) |")),
    ...
    md("## 부록 — 그림을 만든 설정 전부", "",
       ...
       "| 광선 간격 | λ/12 | SBR 격자 (`rcs_sbr.DEFAULT_DIV = 12`, 스크립트가 spacing 미지정) |",
