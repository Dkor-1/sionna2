...
_easy = md(
    ...
    f"**좁고**(그래서 {NR['dR_m']:.0f} m 안쪽 물체는 한 덩어리로 뭉쳐 보임), **20 ms 에 한 번만** 깜빡입니다"
...
cells.append(md(
    ...
    "",
    "두 표적을 거리로 가르는 최소 간격을 **거리분해능 ΔR** 이라 합니다(우리는 **바이스태틱**이라 "
    "$\\Delta R_b$=c/$B_{ref}$; 모노스태틱 등가는 절반). 여기서 결정적인 것은 채널이 "
...
cells.append(md(
    ...
    "<sub>LTE 스펙트럼 — CRS 가 전대역(18.0 MHz)을 켜 거리분해능이 촘촘하다"
    "(ΔR_b 16.7 m, 5G SSB 의 약 2.5배 촘촘).</sub>"
))
