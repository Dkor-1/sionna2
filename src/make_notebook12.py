...
cells.append(md(
    ...
    "시뮬레이터는 존재하지 않는다 — 상용 도구(Ansys HFSS SBR+·Altair Feko)도 환경 전파·표적 RCS·"
    "신호처리를 별도 층으로 다루고, 레이더 논문의 시뮬레이션도 전부 '채널 모델 + 자체 신호처리' 구조다. "
    "검출기(ECA·CFAR)는 우리가 **만들어서 검증하는 대상**이므로 애초에 남의 것일 수 없다.",
    "",
    "**그럼 Sionna 가 실제로 사 준 것은?** ① 환경이 진짜처럼 행동한다는 **담보** — 챔버 바닥 반사 "
    "손계산 대조 0.0005 dB(report01), 링크버짓 RT 교차검증(report11). ② RT 가 아니었으면 몰랐을 위협의 "
    "**발견** — 표적경유 바닥 유령(report09, 전대역 5G 100% 오검출). ③ ITU-R P.2040 재질·장면·렌더 기반. "
    "④ 파형·지연커널의 **독립 교차검증 상대**. 즉 Sionna 의 역할은 '검출을 대신 해 주는 것'이 아니라 "
    "'**채널이 물리적으로 맞는가를 담보하는 것**'이다.",
    "",
