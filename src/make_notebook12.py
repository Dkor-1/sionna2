...
GLOSS = [
    ...
    dict(term="거리분해능 ΔR",
         desc="두 물체를 거리로 가르는 능력. 신호가 쓰는 **대역폭**이 넓을수록 작아진다(좋다). ΔR=c/(2B)."),
...
cells.append(md(
    ...
    "",
    "- **WiFi 는 상시라도 광대역**(프리앰블 76.6 MHz) → 셋 다 거리분해능 2 m 로 좋다. 데이터 점유(W1→W3)는 "
    "분해능을 거의 안 바꾼다(프리앰블이 고정 기준이라).",
    "- **5G 는 상시(G1=SSB)면 7.2 MHz 로 병적으로 좁다**(ΔR 20.8 m) — 진짜 '5G 이중고'. 하지만 "
    "**G2·G3(NR-PRS)로 가면 98 MHz** 전대역 → ΔR 1.5 m 로 급반전.",
...
cells.append(md(
    ...
    "10⁻⁴ 와 정확히 같지 않다**(대략 0.3~0.6배 수준). 따라서 **§3 의 모드 간 SNR50 차이 중 ~1 dB 안쪽의 "
