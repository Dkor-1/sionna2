...
cells += provenance_cells(
    ...
    spine=dict(
        ...
        prior=("패시브 바이스태틱 드론탐지 선행은 표적 산란을 **외부에서 구해 채널에 주입**"
               ...
               "(Wypich Sensors 2026 · Demissie IET RSN 2025). 단 조사한 선행은 대개 단일 조명원·최대 "
               "2채널이라 **표준 간·다중 Rx 공정 비교가 비어 있다**(§6)."),
        ...
        verify=(f"**GPU 배치 몬테카를로 K={K:,}회**(각 SNR·N·모드)로 Pd. pyAPRiL 로 NR/WiFi/LTE 3모드 모두 "
                f"**CAF 봉우리 거리빈이 정답과 일치**함을 확인(`verify_pyapril.py`; ⚠CFAR 표적셀 발화는 아님); 지연 연산자는 해석식과 상관 {CORR:.3f}"
...
cells.append(md(
    ...
    f"(이 **특정 자세**의 값 — 방위평균은 −18.4 dBsm(3.5 GHz)·밴드평균 −17.1 dBsm, report08; RCS 는 자세에 ~14 dB 출렁인다), "
...
cells.append(md(
    ...
    "dB)**. N=4 면 +6.0 dB. (직관: 여러 사람이 같은 소리를 방향 맞춰 함께 들으면 소리는 커지고 잡음은 "
    "그대로여서 더 또렷해진다.) 이는 **교과서적 배열이득이며, 여기서는 이상적 상한**이다: **완벽 조향**"
    "(표적 방위를 정확히 앎)·**소자 간 등분산 독립잡음**·보정오차 0 을 가정한다. 몬테카를로는 이 정합 "
    "빔포머와 동치인 형태로 √N 을 신호에 주입하고, 잡음쪽 전력보존(σ²)만 독립 실측한다. 실측 X410 은 조향 "
    "불일치·상호결합·동기오차로 **이 상한 이하**다.",
))
...
cells.append(md(
    ...
    f"(방위평균 **−18.4 dBsm**@3.5 GHz — 위 §2 의 이 자세값 {M['sigma_dbsm']:.1f} dBsm 은 **널이라 포락선 하단**이니 구분)도 소형드론 실측 포락선(−28~−16 dBsm)과 **정합하되 밝은 상단**"
    "(few-λ 공진영역에서 PO 가 절대레벨을 낙관적으로 잡을 소지, → report08)인 것이지 그 자체가 새 측정은 "
    "아니다. **다행히 헤드라인(5G 이중고·모드 비교)은 상대 결론이라 σ 가 밴드 내에서 움직여도 불변**이다. "
...
cells.append(md(
    ...
    f"**NR/WiFi/LTE 3모드 모두 표적을 정답 거리빈에 검출**함을 확인했다(`benchmark/verify_pyapril.py`). "
    ...
    "GPU 로 대량 반복한 결과다.",
    "> - **RCS → 자작 SBR+PO**(선행 BVH SBR+PO arXiv:2604.09243 과 같은 방법; 상용 CADFEKO·비공개 "
