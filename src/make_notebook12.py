...
cells += provenance_cells(
    ...
    spine=dict(
        ...
        verify=(f"**GPU 배치 몬테카를로 K={K:,}회**(각 SNR·N·모드)로 Pd. pyAPRiL 로 NR/WiFi/LTE 3모드가 "
                f"정답 거리빈에 검출됨을 확인(`verify_pyapril.py`); 지연 연산자는 해석식과 상관 {CORR:.3f}"
...
cells.append(md(
    ...
    f"(이 자세 σ={M['sigma_dbsm']:.1f} dBsm)도 소형드론 실측 포락선(−28~−16 dBsm)과 **정합하되 밝은 상단**"
