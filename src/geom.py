...
#: ⭐⭐**정본 수리** (2026-08-17 확정) — 환경변수를 안 주면 **이 둘이 켜진다.**
#  마감 규칙(docs/RESUME.md «메쉬 마감»)의 A 통 — «σ 영향이 판정 밴드 밖 **이고** 근거가 있다»:
#    · `battery` — 배터리 팩과 구조판이 서로 파고들어 PO 가 면적을 이중계상한다(4기체 47.9~50.0 %).
#      σ 방위평균 **+1.02~+1.88 dB**(최악 방위 9.6~18.9 dB) — 2층 최대 단일 항목이고 전부 밴드 밖.
#      근거는 이 저장소 자신의 선언이다(drone_cad INTERNALS 주석: «battery·pcb 는 union 목록에
#      없으니 겹치지 않게 놓는다»). 실측표를 받은 matrice4e·phantom3 은 이미 0 % 라 안 바뀐다.
#      ⚠파급: battery 부피 −12.27 % → 질량 −21~31 g · 관성 대각 +1.9~2.8 % · CoM 0.18~0.45 mm.
#        로터 요동 모델(자세 응답)로 내려가는 사슬이니 그쪽을 다시 잴 때 이 값을 쓸 것.
#    · `i5` — mini2 body 구멍(경계 모서리 3). σ 로는 **없다**(1e-10 dB). 켜는 이유는 산란이 아니라
#      **검사 정확도**다 — 껍질이 안 닫혀 `contains()` 가 성립 안 해 검사가 body 를 컨테이너에서
#      빼고, 그 결과 매몰면을 29.71 % 로 읽었다(참값 44.45 %, **14.74 pp 를 숨김**).
#
#  ⛔**B 통(영구 한계 — 인증서 `not_guaranteed` 에 등재, 다시 안 본다)**:
#    · `i3` 매몰면 — 2층 최대 dB(+1~5)이지만 **프로펠러 면까지 지우고 마스크가 날과 함께 돈다**
#      (mini2 878→88장) → 마이크로도플러가 망가진다. 게다가 «어느 그룹이 설계 의도인가» 라는
#      규칙 선택에 1~2 dB 가 매달려 있다. ⇒ 고칠 결함이 아니라 **우리 커널 PO 의 구조적 성질**이다
#      («PO 는 가림을 안 본다» — rcs_po.py 자기선언).
#    · `i4` 캐노피 · `m4` x500v2 · `m6` 극점 — σ 가 밴드 안이거나 정확히 0(극점은 드론에 안 쓰임).
#    · mini5pro·mavic4pro **세로 배분** — 두 기체의 공식 CAD 가 없고, 셸 높이는 이미 사진 실측이라
...
def mesh_fix_set() -> set[str]:
    ...
    환경변수 `MESH_FIX` 를 **호출할 때마다** 읽는다.
      · 안 주면 → **정본** `MESH_FIX_CANON`(위 표 참조).
      · `MESH_FIX=none`(또는 off·legacy) → **전부 끔** = 2026-08-16 이전과 비트동일.
      · 그 밖 → 준 대로.
    """
    raw = os.environ.get("MESH_FIX")
    if raw is None:
        return set(MESH_FIX_CANON)
    if raw.strip().lower() in ("none", "off", "legacy", "-"):
        return set()
