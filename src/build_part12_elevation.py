# -*- coding: utf-8 -*-
"""
build_part12_elevation.py — 권 4 「앙각 커버리지」의 조각들 → reports/_parts/8N_*.ipynb
==========================================================================================
권 4 는 관측 앙각을 0° 에서 −90° 까지 내리며 같은 표적을 재고, 커버리지를 정하는 것이
...
⭐ 조각(`reports/_parts/`)은 **사람이 직접 읽는 문서가 아니다.** 숫자를 원장에서 주입하는
   층이고, `src/build_volumes.py` 가 이 조각들을 이어 권 `reports/04_elevation-coverage.ipynb`
...
#  ⛔2026-09-01 — 우리 팔의 «0.22~17.18 %» 는 **λ/12 격자 한 판**의 값이다. 같은 팔·같은
...
#  이 권의 세 절(79 빗살 · 80 상한 위 누설 · 82 나딧 잔여)이 함께 읽는다 — 셋 다
#  «광선을 더 부으면 이 자리가 내려가나» 를 묻기 때문이다. 답은 상한 위에서 «그대로다» 이고,
#  ⚠그 사다리는 **10 m·물리 끔** 자리라 이 권의 15 m 판과 거리부터 다르다. 그래서 인용할
#  때마다 조건을 함께 적는다.
AC17 = from_json("outputs/raybudget_ac_ladder.json")
...
#  하나다. 우리 커널은 광선 예산 축 밖의 별개 엔진이라(원장의 `spp` 칸이 비어 있다) «엔진» 축의
#  기준 팔로 함께 둔다.
...
#  ⚠**다른 추정기**의 판이다. 방향만 읽고 크기는 인용하지 않는다.
WG = from_json("outputs/sbr_grid_convergence.json")

#: ⭐격자 축을 **이 잣대·이 스윕 위에서 직접** 흔든 판. 같은 커널·같은 자세 8192 개·같은
#  15 m·같은 표적에서 격자만 λ/12 → λ/24 로 조인 팔(`ours_r15_n8192_div24`)이다.
#  ⚠이 원장이 표로 싣는 앙각은 el 0·−15·−30·−45 **네 자리**다. `ours_r15_n8192_div24`
#  자체는 `outputs/elevation_sweep_md.json` 에 일곱 앙각(0~−90) 전부 `n_missing = 0` 으로
#  살아 있고, el −60·−75 의 상한 위 몫은 **이 절이 읽는 두 원장에 아직 실리지 않은 것**이다
#  (돌리지 않은 것과 다르다 — 다음 단계 표 셋째 줄이 싣는다).
GC = from_json("outputs/grid_convergence_check.json")

#: λ/24 칸이 원장에 실려 있는 앙각. 나머지는 «원장 미적재» 로 적는다 — 빈칸을 0 으로
#  읽히지 않게, 그리고 «안 돌렸다» 로 읽히지 않게.
GRID_ELS = (0.0, -15.0, -30.0, -45.0)


def leak24(el: float) -> str:
    """같은 팔·같은 자세 8192 개를 λ/24 격자로 다시 깐 판의 상한 위 몫 한 칸 [%]."""
    if el not in GRID_ELS:
        return "원장 미적재"
    return GC.num(f"per_elevation.{el:+.0f}.layer2_statistics.div24.above_f_tip_pct",
                  fmt="{:.2f}", unit="%")


def gap12(el: float) -> str:
    """λ/12 판에서 우리 팔과 바로 옆 팔 사이의 간격 — 단위 %p 는 태그 밖에 적는다."""
    return GC.num(f"arm_order_tables.prereg_3arms.per_elevation.{el:+.0f}."
                  "above_f_tip_pct.min_gap_at_ours_div12", fmt="{:.2f}")


def order24(el: float) -> str:
    """같은 칸을 λ/24 로 다시 깔았을 때 원장이 찍은 사전등록 판정."""
    return GC.num(f"arm_order_tables.prereg_3arms.per_elevation.{el:+.0f}."
                  "above_f_tip_pct.verdict")


def grid_band() -> str:
    """이 잣대의 격자 산포 밴드 — 네 앙각 |λ/24 − λ/12| 의 최댓값 [%p]."""
    return GC.num("arm_order_tables.prereg_3arms.per_elevation.-15."
                  "above_f_tip_pct.grid_band", fmt="{:.2f}")


def n_poses24() -> str:
    """격자 대조 두 판이 쓴 자세 수 — 손으로 치지 않는다."""
    return GC.num("per_elevation.-15.n_poses", fmt="{:.0f}", unit="개")
...
def blocks_80() -> list:
    return [
        header(
            num=80,
            title=TITLE_80,
            did="세 팔 여섯 앙각에서(PathSolver 두 팔은 같은 광선 예산) 날개끝 속도가 정하는 "
                "상한 위 대역의 에너지 몫을 재고, 엔진과 물리 스위치를 가르는 잣대로 삼았다.",
            results=[
                ...
                f"{leak(K_OURS, -15.0)} 사이다 — f_tip 이 0 보다 큰 여섯 앙각의 "
                f"**λ/12 격자 한 판** 값이고, 같은 팔을 λ/24 로 다시 깐 네 앙각은 "
                f"{leak24(0.0)}·{leak24(-15.0)}·{leak24(-30.0)}·{leak24(-45.0)} 다.",
            ...
            method=[
                ...
                ("팔 사이 공정성",
                 "PathSolver 두 팔이 **광선 40 억 발**로 같고, 그 둘이 갈리는 축은 물리 "
                 "스위치뿐이다. 우리 커널은 광선 예산 축 밖의 별개 엔진이라(원장의 `spp` 칸이 "
                 "비어 있다) 엔진 축의 기준 팔로 함께 둔다"),
                ("누설 몫",
        ...
        md("## 상한은 날개끝 속도가 긋는다", "",
           ...
           f"직하방에서는 {sat(-90.0)} 다. 원장은 그 자리를 «블레이드 회전 도플러로는 설명되지 "
           f"않는 자리» 로 적고, 무엇이 채웠는지(격자 표본화 잡음 · 근접장 곡률 · 가림 · 창 "
           f"누설)는 따로 잰다고 못 박는다"
           f"⟨{WJ} : _meta.physical_limit_ko⟩ — 귀속 원장은 "
           f"⟨outputs/refute_nadir_mechanism_final.json : R5_detection.nadir_ac_split⟩ 다.", "",
           "⛔그래서 이 절은 그 에너지를 «인공물» 이라고 부르지 않는다 — 상한 위라는 관찰과 "
           "무엇이 그것을 만들었는가라는 귀속은 다른 물음이다."),

        md("그 «설명되지 않는 에너지» 를 0 이 참값인 자리로 읽는 것은 이 잣대가 서는 범위 **밖**이다. "
        ...
        md("## 세 팔이 상한 위에 남기는 몫", "",
           table(["앙각", "f_tip", "우리 커널 λ/12", "우리 커널 λ/24",
                  "PathSolver 물리 끔", "PathSolver 물리 켬"],
                 [[f"{el:+.0f}°",
                   W.num(f"cells.{K_OURS}/el{el:+.0f}.f_tip_hz", fmt="{:.1f}", unit="Hz"),
                   leak(K_OURS, el), leak24(el), leak(K_OFF, el), leak(K_ON, el)]
                  for el in ELS]), "",
           "⚠격자 축은 우리 커널에서, 그것도 el 0°·−15°·−30°·−45° 네 자리에서 흔들었다. "
           "el −60°·−75° 의 λ/24 판은 자세 시계열까지 이미 돌아 있고, 그 값을 이 절의 "
           "원장에 싣는 일이 다음 단계 표 셋째 줄이다. PathSolver 두 팔의 격자 축은 "
           "이 원장 밖이다."),
        ...
        md("### 이 잣대가 서는 범위", "",
           table(["조건", "이 절이 재는 자리"], [
               ...
               ["비교 축",
                ...
                "밖이다"],
               ["격자",
                "⛔우리 팔의 값은 **λ/12 한 판**이고, 원장이 이 인용에 "
                + GC.num("prereg_verdict.enforced_tag_ko")
                + " 꼬리표를 강제해 뒀다. 같은 팔·같은 자세 " + n_poses24()
                + " 로 격자만 λ/24 로 조이면 el −15° 의 " + leak(K_OURS, -15.0) + " 가 "
                + leak24(-15.0) + " 로 내려간다. 이 잣대의 격자 산포 밴드는 " + grid_band()
                + " %p 이고, 두 팔의 차이가 그 밴드 안이면 판정 불가다"],
               ["광선 예산",
                "⭐물리를 끈 10 m 사다리에서 광선을 360 배 부어도 상한 위 바닥은 el −15 에서 "
                + AC17.num("headline.ladder_a_360x_change_db.el-15.floor_above_db",
                           fmt="{:+.2f}")
                + " · el −45 에서 "
                + AC17.num("headline.ladder_a_360x_change_db.el-45.floor_above_db",
                           fmt="{:+.2f}", unit="dB")
                + " 로 제자리다 — 광선을 더 부어도 이 몫은 그 자리에 남는다. ⚠물리를 켠 두 "
                "계단(22.5 배·깊이 3)은 같은 자리가 el −15 에서 "
                + AC17.num("ladder_a_physics_on.clean_two_rungs_delta[1].d_floor_db",
                           fmt="{:+.2f}", unit="dB") + " 올라간다"],
               ["합격선", "미정 — 임계는 다음 단계 표가 정한다"],
           ])),

        md("## λ/12 판에서 우리 팔이 물리 끔 PathSolver 보다 높은 자리는 세 곳이다", "",
           f"el −15° 는 {leak(K_OURS, -15.0)} 대 {leak(K_OFF, -15.0)}, el −30° 는 "
           f"{leak(K_OURS, -30.0)} 대 {leak(K_OFF, -30.0)}, el −45° 는 "
           f"{leak(K_OURS, -45.0)} 대 {leak(K_OFF, -45.0)} 다. 나머지 세 자리"
           f"(0°·−60°·−75°)에서는 우리 팔이 아래에 있다.", "",
           f"⛔같은 커널·같은 자세 {n_poses24()} 로 격자만 λ/24 로 조이면 el −15° 는 "
           f"{leak24(-15.0)}, el −45° 는 {leak24(-45.0)} 로 내려가 두 자리 다 물리 끔 "
           f"**아래로** 간다 — 원장의 사전등록 판정이 그 두 칸을 {order24(-15.0)} · "
           f"{order24(-45.0)} 로 찍는다. el −30° 는 {leak24(-30.0)} 로 순서가 남고, "
           f"판정은 {order24(-30.0)} 다.", "",
           f"λ/12 판의 여유는 el −15° {gap12(-15.0)} · el −30° {gap12(-30.0)} · el −45° "
           f"{gap12(-45.0)} %p 이고, 이 잣대의 격자 산포 밴드는 {grid_band()} %p 다 — "
           f"셋 다 밴드 안이다.", "",
           f"그 밴드를 어떻게 읽는지는 원장이 적어 뒀다 — "
           f"{GC.num('prereg_verdict.band_self_reference_ko')}", "",
           "⛔세 자리가 높은 기전은 이 절의 범위 밖이다. 후보는 λ/12 격자의 표본화와 자세 "
           "시계열 창의 누설 둘이고, 둘을 가르는 실험은 다음 단계 표 넷째 줄이다. 거리 "
           "곡률은 후보에서 빠진다: 이 스윕은 원거리장 경계 밖에서 쟀다"
           + f"(" + ref("el-sweep-design", "스윕 규약") + ").", "",
           f"⚠λ/8 → λ/32 사다리는 **다른 추정기**의 판이라 방향만 읽는다 — 같은 "
           f"종류의 대역밖 몫이 "
           f"{WG.num('rows[0].froz_frac_power_beyond_ftip', fmt='{:.2%}')} 에서 "
           f"{WG.num('rows[4].froz_frac_power_beyond_ftip', fmt='{:.3%}')} 로 내려간다"
           f"(얼린 격자 팔, 블레이드 대역을 분모로 쓴다).", "",
           "⇒ 이 절이 확정한 것은 **λ/12 판 여섯 칸의 값**까지다. 우리 팔과 물리 끔 "
           "PathSolver 사이의 순서는 격자를 조이면 el −15°·−45° 에서 뒤집히므로 "
           "**판정 불가**로 남는다."),
        ...
        md("### 검증된 것과 열어 둔 것", "",
           table(["무엇", "상태"], [
               ...
               ["세 팔 18 칸의 누설 몫",
                "계산값(우리 팔은 λ/12 격자 한 판) — 합격선은 다음 단계가 정한다"],
               ["우리 팔과 물리 끔 PathSolver 의 순서 (el −15·−30·−45)",
                "⛔판정 불가 — λ/24 로 조이면 el −15·−45 에서 순서가 뒤집힌다"],
               ["우리 팔 세 자리가 높은 기전",
                "⛔미상 — 격자 표본화와 창 누설을 가르는 실험이 다음 단계 넷째 줄이다"],
           ])),
        ...
        next_steps([
            ...
            ("이미 돌아 있는 격자 팔들(`ours_r15_n8192_div24` 일곱 앙각 · `_div48` el −15 "
             "· `_shift0.5` 네 앙각)을 이 절의 규약으로 상한 위 원장에 싣는다",
             "격자 흔들림 폭을 이 스윕 위에서 직접 인용하게 되고, el −60°·−75° 의 λ/24 "
             "칸이 채워진다",
             "`WB_ARMS` 에 세 팔을 더해 `benchmark/build_wideband_energy_fig.py` 재실행 · "
             "⭐시계열은 `outputs/elevation_sweep_md.npz` 에 **이미 있다**"),

            ("우리 팔 −15°·−30°·−45° 세 자리의 남은 후보(격자 표본화 대 창 누설)를 가른다",
             "격자 산포 밴드 안에 있는 세 자리의 기전이 특정된다",
...
# =========================================================================== #
#  조각 78 — 권 4 절 1 「무엇을 어떻게 쟀나 · 설정과 그 대가」
...
#: 주력 세 팔 **밖의** 대조 팔 — 자유공간·평면파·단일축 스위치 넉 장·깊이 2. 아래 표의 정본이다.
CTRL78 = ("ours_free_r15_n8192", "ours_r15_n8192_pw",
          ...
          A_ON2)
#: ⭐이 권이 서술하는 팔의 **전수**다 — 주력 셋 + 대조 일곱.
#  ⚠원장 `elevation_sweep_md.json` 은 이 권 말고도 여러 권·여러 축의 실험이 함께 쓴다:
#    다른 기체(mini 5 Pro · S1000+), 방위 축(`…_az45…`), 별편 5-2 의 7 조합 스위치 격자
#    (`…_swR#D#E#F#…`), 스톡 기본 설정 팔, 부품 전파 판 등. 그 팔들은 원장에 그대로 살지만
#    **이 권이 인용하지 않으므로** 이 권의 표와 세는 수 밖이다(행 색인은 전체 배열 기준 유지).
VOL_ARMS78 = ARMS78 + CTRL78
_R15 = [r for r in _ROWS78
        if r.get("range_m") == R78_PRIMARY and r.get("engine") in VOL_ARMS78]
N78_ALL = len(_R15)                                      # 이 권이 서술하는 행
N78_OK = sum(1 for r in _R15 if not r.get("n_missing"))  # 인용 가능한 행
N78_COMPLETE = (N78_ALL == N78_OK)
...
def _idx78_rest() -> list[int]:
    ...
    ⚠이 권 밖의 팔(10 m 옛 판 · 다른 기체 · 방위 축 · 별편 5-2 의 스위치 격자 · 스톡 기본
      설정 등)은 뺀다 — `VOL_ARMS78` 이 그 경계다. 원장에는 그대로 남는다.
    """
    seen, out = set(), []
    for i, r in enumerate(_ROWS78):
        e = r.get("engine")
        if (e not in CTRL78 or e in seen
...
def blocks_78() -> list:
    ...
    return [
        header(
            num=78,
            title=TITLE_78,
            did=f"관측 앙각 {N78_EL} 점을 15 m 구면 한 자리에 고정해 세 팔로 같은 표적을 "
                f"재고(PathSolver 두 팔은 같은 광선 예산), 규약과 완결성을 이 조각에 못 박았다.",
            results=[
                ...
                f"광선 예산은 PathSolver 두 팔이 같다 — 둘 다 "
                f"{S.num('_meta.sionna_spp_primary', fmt='{:,.0f}', unit='발')} 이고, 우리 "
                f"커널은 이 축 밖이라 원장의 `spp` 칸이 비어 있다. 이 판의 "
                + (f"{N78_ALL} 행은 빠진 자세 없이 전부 완결이다." if N78_COMPLETE else
                   f"{N78_ALL} 행 중 {N78_OK} 행이 완결이고, 나머지는 인용하지 않는다."),
        ...
        md("## 세 팔이 같은 자리에서 낸 것", "",
           ...
           "PathSolver 다. 아래 두 판의 광선 예산은 같고, 위 줄은 광선 예산 축 밖의 별개 "
           "엔진이다. 판마다 자기 최댓값으로 정규화했으므로 "
        ...
        md(f"## 어느 행을 인용해도 되나 — {N78_ALL} 행이 모두 완결이다"
           if N78_COMPLETE else
           f"## 어느 행을 인용해도 되나 — {N78_ALL} 행 중 {N78_OK} 행이 완결이다", "",
           f"이 권이 쓰는 팔은 열이고(주력 셋 + 대조 일곱), 그 {N78_ALL} 행은 전부 "
           "`n_missing = 0` 이다. `n_missing` 은 시계열에 0 이 박힌 자세의 수이고, 0 이 "
           "박히면 스펙트럼과 레벨이 그만큼 눌린다. 이 권은 그런 행을 인용하지 않는데, 이 "
           "판에는 그런 행이 없다."
           if N78_COMPLETE else
           f"이 권이 쓰는 팔은 열이고(주력 셋 + 대조 일곱), 그 {N78_ALL} 행 중 {N78_OK} "
           "행이 `n_missing = 0` 이다. 나머지는 인용하지 않는다.", "",
           "⚠같은 원장을 이 권 밖의 실험도 함께 쓴다 — 다른 기체, 방위 축, 별편 5-2 의 "
           "스위치 격자, 스톡 기본 설정 팔이 그 안에 산다. 이 권이 인용하지 않으므로 위 "
           "숫자와 아래 표 밖이다.", "",
...
# <<<PART82_BEGIN>>>
# =========================================================================== #
#  조각 82 — 권 4 「직하방 — 도플러가 원리적으로 0 인 자리에서 무엇이 남나」
...
#: ⛔전 제목은 «나딧 잔여의 64 % 는 …» 로 64 % 를 확정값처럼 실었다(2026-09-06 정정).
...
def blocks_82() -> list:
    ...
    return [
        ...
        md("⭐그 «표본화 축» 은 **위상 흔들림**에서 잰 것이다. 같은 예산 축을 **세기**로 "
           "읽으면 광선을 부어도 내려가지 않는 자리가 따로 있다 — 물리를 끈 10 m 사다리에서 "
           "광선을 360 배 늘려도 날개끝 상한 위 바닥이 el −15 에서 "
           + AC17.num("headline.ladder_a_360x_change_db.el-15.floor_above_db", fmt="{:+.2f}")
           + " · el −45 에서 "
           + AC17.num("headline.ladder_a_360x_change_db.el-45.floor_above_db",
                      fmt="{:+.2f}", unit="dB")
           + " 로 제자리다.", "",
...
# =========================================================================== #
#  조각 79 — 권 4 절 2 「커버리지를 정하는 것은 표적이 아니라 분석 대역이다」
...
def blocks_79() -> list:
    ...
    return [
        ...
        md(f"⭐그 세 숫자가 재는 범위는 여기까지다. 기준띠에도 빗살 21~26 차가 들어 있어서, "
           ...
           + " 이 예산 축에서 가른다.", "",
           "⭐그 축의 한쪽은 값이 이미 나와 있다 — 물리를 끈 10 m 사다리에서 광선을 360 배 "
           "부어도 상한 위 빗살 선은 el −15 에서 "
           + AC17.num("headline.ladder_a_360x_change_db.el-15.comb_above_db", fmt="{:+.2f}")
           + " · el −60 에서 "
           + AC17.num("headline.ladder_a_360x_change_db.el-60.comb_above_db",
                      fmt="{:+.2f}", unit="dB")
           + " 로 제자리다. 광선을 더 부어도 상한 위 빗살은 그 자리에 남으므로, 이 절이 "
           "기다리는 판정은 «광선을 늘리면 사라지나» 밖의 축에서 온다 — 그 사다리는 "
           "10 m·물리 끔·앙각 0~−60 자리이고 여기 −75° 는 그 밖이다."),
