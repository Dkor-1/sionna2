...
cells.append(mdl([
    ...
    "",
    "같은 CFAR 에 **실제 파형의 기준신호로 정합필터링한 잡음** + DPI + ECA 를 통과시킨 맵을 먹였다 "
...
cells.append(md(
    ...
    "",
    "> **여기서 하는 일:** ECA 가 직접파를 얼마나 지우는지, 그리고 **표적까지 얼마나 지우는지** 잰다.",
    "",
...
cells.append(md(
    ...
    "",
    "> **여기서 하는 일:** 점표적 하나를 넣었을 때 RD 맵에 그려지는 응답 |χ(τ, f_d)| 을 잰다. "
...
cells.append(md(
    ...
    "그걸로 드론의 **3D 위치**를 결정할 수 있는가? — SNR 이 아니라 **원리**의 문제다.",
    "",
...
cells.append(mdl([
    ...
    "",
    f"입사각 {POL['V']['theta_i_deg']:.1f}° 는 콘크리트의 **브루스터각 바로 아래**다 → V(TM) 편파가 "
    "**이례적으로 약하게** 반사된다.",
    "",
