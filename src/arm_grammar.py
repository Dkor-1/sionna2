...
_COMMON = [
    ...
    #: ⭐⭐**안테나 무늬** (2026-09-15 신설) — `_anttr38901` = 3GPP 소자를 드론에 고정 조준.
    #  `_anttr38901c50` = 같은 공식에서 감쇠 상한만 50 dB. ⛔**등방(기본)에는 안 붙는다** —
    #  창고 전부가 등방이라 이름이 그대로여야 이어진다. 본문을 좁게 묶어 다른 값을 막는다.
    #  ⚠씨앗(`s<N>`)은 `_rot` 뒤에 붙으므로 여기까지 오지 않는다(_SEED_HOSTS 에 안 넣는다).
