# -*- coding: utf-8 -*-
"""
chamber.py — 대형 차폐시설(반무반사실 / semi-anechoic chamber) 3D 모델
=====================================================================
...
차폐시설이 무엇인가요? (아주 쉽게)
  ...
  * "무반사(anechoic)" : 안쪽 면을 **뾰족한 전파흡수체(피라미드 폼)** 로 덮어,
                         쏜 전파가 벽에 부딪혀 되돌아오지 못하게(메아리 없게) 합니다.

**이 방은 anechoic 이 아니라 semi-anechoic 입니다** — 용어를 정확히 씁시다.
  흡수체는 **벽 4면 + 천장**에만 있고, **바닥은 타일(콘크리트)로 반사성**입니다
  (chamber_group_style: floor_* → concrete_light/dark, materials.make_material → ITU 'concrete').
  6면 전부 흡수체여야 anechoic(자유공간 모사)이고, 바닥만 반사면 정의상 **semi-anechoic
  (ground-plane) 챔버**입니다. EMC 시험장의 표준 형태이기도 합니다.

  → 결과: 방 안에서 **유일하게 강한 반사면이 바닥**입니다. 이건 문서상의 말장난이 아니라
    시뮬레이션 결과에 그대로 나타납니다. 바이스태틱 기하에서 바닥 반사는 **스침각(grazing)**
    으로 들어와 콘크리트가 거울처럼 반사하고, **표적을 경유한 바닥 경로는 표적과 같이
    도플러가 실려** ECA(정지 클러터 제거)로 지워지지 않습니다.
    "무반사라서 클러터가 약하다"는 **바닥에는 적용되지 않습니다.**
