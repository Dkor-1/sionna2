...
cells.append(md(
    ...
    "",
    "> 📚 **선행 대비 우리 방식.** ISAC 문헌에서 표적 밝기를 다루는 표준은 세 갈래다 — **(b) 확산계수 S "
    "가정**(Great-X·EuCAP 2026 Deterministic-Modeling), **(c) RCS 상수 주입**(NIST 5GNRad·3GPP·MATLAB), "
    "**(d) 커스텀 산란 add-on**. 우리는 (d)로 **부위별 재질·형상에서 σ 를 계산**한다 — 소형 드론은 확산 S 를 "
    "실측 보정할 데이터가 없고, 금속 모터 vs 플라스틱 셸의 재질 차이가 RCS 를 지배하기 때문이다. "
    "근거·출처는 `prior_work/pw01`(선행 논문)·`OPENSOURCE.md`(RadarSimPy 교차검증 계획).",
    "",
    "---",
