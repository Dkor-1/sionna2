...
cells.append(md(
    ...
    "",
    "> 📚 **선행 대비 우리 방식.** ISAC 문헌에서 표적 밝기는 세 갈래로 처리된다 — **(b) 확산계수 S "
    "가정**(Great-X·EuCAP 2026), **(c) RCS 상수 주입**(NIST 5GNRad·3GPP·MATLAB), **(d) 자작 SBR+PO / 산란 "
    "add-on**(BVH SBR+PO arXiv:2604.09243 — 우리와 동일 방법; Ziganshin UTD). 우리는 **(d) 자작 SBR+PO** — "
    "소형 드론은 확산 S 실측 보정 데이터가 없고 부위별 재질 차이가 RCS 를 지배하기 때문이다. 상용 CADFEKO "
    "(LAMBDA)나 비공개 RadarSimPy 대신 **선행이 실제로 쓰는 자작 SBR+PO 방식을 따랐다**(근거: `prior_work/pw01`).",
    "",
    "### 📏 절대값 앵커 — **실측 문헌 RCS 와 교차검증**",
    "",
    "해석해(평판·구, report07)는 방법이 옳음을 보이지만 드론의 **절대 σ** 는 보장하지 못한다. 그래서 우리 "
    "SBR+PO σ 를 **공개 실측 드론 RCS** 와 대조해 스케일을 앵커한다(값 출처: `prior_work` 파일럿 조사, "
    "각 값은 1차 출처 확인):",
    "",
    ...
    "(크기·재질분포·지배산란체)이 옳다는 증거이기도 하다(mesh 완성도의 한 기둥, →mesh08).</sub>",
    "",
    "---",
