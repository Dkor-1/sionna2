...
cells += provenance_cells(
    ...
    spine=dict(
        ...
        verify=("우리 mavic4pro **방위평균 −19.6·봉우리 −12.6 dBsm** 가 위 실측 범위 안에 든다(§6 대조표). "
...
cells.append(md(
    ...
    "|---|---|---|---|",
    "| **Li & Ling 2017** (IEEE AWPL, ~99인용) | **3–6 GHz** ★밴드일치 | Phantom 2 **−27.5**, 3DR Solo −24.2, Inspire 1 −13.7 dBsm (모두 **peak/특정자세**, 자세 스프레드 ~14 dB) | 우리 mavic4pro **방위평균 −19.6 dBsm → 이 범위 안** (봉우리 자세 −12.6 dBsm 는 상한 −13.7 보다 ~1.3 dB 밝으나 개별각 수렴 불확실도 ~2 dB 이내 — 유의미한 초과 아님) |",
    ...
    "−19.6 dBsm 를 감싼다. 봉우리 자세(−12.6 dBsm @3.5 GHz)는 문헌 상한(−13.7)보다 ~1.3 dB 밝지만, "
    ...
    "이 되어야 한다(RCS 는 주파수↑에 단조↑ 이므로 3.5 GHz ≤ 15 GHz 값). 우리 mavic4pro 는 **방위평균 −19.6·"
    "봉우리 −12.6 dBsm** — 봉우리가 이 범위에 들고 방위평균은 그 아래(더 어둡게=보수적). **−17~−3 dBsm 을 크게 "
    "벗어나면 적신호**인데 우리는 그 안이다. ⚠ 정확 모델·측정조건·자세규약이 달라 **±몇 dB 앵커**이지 점일치가 "
    "아니다 — 그래도 시뮬을 다른 시뮬과 대조하는 것보다 **실측과 대조**하는 편이 강한 근거다. 이 일치는 메쉬의 **레이더 유효 형상**"
...
cells.append(md(
    ...
    "**−15~−5 dBsm 범위 안**임을 확인했다(우리 봉우리 −12.6 dBsm, 방위평균은 더 보수적)(§6).",
    "",
