...
cells += provenance_cells(
    ...
    spine=dict(
        core=("탐지는 표적이 레이더 눈에 **얼마나 밝은가(RCS, σ)**에서 출발한다. 이 리포트는 드론 5종의 "
              ...
              "**실측 문헌의 소형 멀티로터 RCS 범위 안**에 듦을 보인다(우리 mavic4pro 봉우리 −12.6 dBsm)."),
        ...
        prior=("소형 멀티로터의 RCS 는 **실측 문헌**이 기준이다. S밴드(3–6 GHz) 실측 aspect-peak 은 "
               "Li & Ling 2017(IEEE AWPL): Phantom 2 **−27.5** ~ Inspire 1 **−13.7 dBsm**. 방위평균 포락선은 "
               "대략 **−28~−16 dBsm**(중앙 ~−22). ⚠ 소형드론 RCS 는 **저주파로 갈수록 떨어진다**(공진/레일리) — "
        ...
        verify=("우리 mavic4pro σ 는 위 실측 포락선(−28~−16 dBsm) **안에 들고 크기·자세·주파수 트렌드를 "
                "재현**한다(§6 대조표). 단 위치는 **밝은 상단** — S밴드 주파수 하강분을 감안하면 few-λ(공진영역)"
                "에서 PO 가 절대레벨을 **최대 ~7~10 dB** 낙관적으로 잡을 소지가 있다(§6 외삽 −25~−28 대비). 그래서 검출은 σ 밴드로 제시해 "
    ...
    caveats=[
        "**절대 RCS 를 보장하지 않는다.** SBR 은 해석해(구·평판)로 검증되고, 드론 절대값은 §6 에서 **실측 문헌에 "
        "대조**했다 — 포락선 안이나 **밝은 상단**(few-λ PO 낙관 소지). 즉 절대 dBsm 은 유도가 아니라 교정·대조된 값이라 "
...
cells.append(md(
    ...
    *_rows,
    ...
    f"**{_span:.1f} dB** — 퍼센트 수준이 아니라 **약 5 배** 차이입니다. 오른쪽 산점도가 대각↔밝기 "
    "추세를 그대로 보여줍니다.",
    "",
...
cells.append(md(
    ...
    "arXiv:2606.07328), **(d) 자작 SBR+PO / 산란 add-on**(Sionna-RT 확장 계열 — 우리와 **가장 가까운 "
    "선행**은 Ziganshin arXiv:2604.05991: Sionna-RT 에 UTD 로 큰 표적; 우리는 같은 확장에 PO 로 작은 "
    "드론. BVH SBR+PO arXiv:2604.09243 은 독립엔진). 우리는 **(d) 자작 SBR+PO** 를 택했다 — 소형 드론은 확산 S 실측 보정 데이터가 없고 부위별 "
    ...
    "|---|---|---|---|",
    "| **Li & Ling 2017** (IEEE AWPL, ~99인용) | **3–6 GHz** ★밴드일치 | Phantom 2 **−27.5**, 3DR Solo −24.2, Inspire 1 −13.7 dBsm (모두 **peak/특정자세**, 자세 스프레드 ~14 dB) | 우리 mavic4pro 값이 이 **aspect-peak** 범위 안 — ⚠ 단 이는 우리 값 vs 그들의 peak 비교라 비대칭이다. **방위평균끼리** 보면 우리가 **밝은 상단**(S밴드 방위평균 포락선 −28~−16, §6 정리) |",
    ...
    "",
    "<sub>정리: (1) **같은 밴드(3–6 GHz)** 실측 aspect-peak(Phantom 2 −27.5 ~ Inspire 1 −13.7 dBsm)과 "
    "방위평균 포락선(대략 **−28~−16 dBsm**)이 우리 값을 **감싼다** — 자릿수·부호가 맞고 이상치가 아니다. "
    "(2) ⚠ **단, 우리 값은 밝은 상단이다.** 소형드론 RCS 는 저주파로 갈수록 떨어지는데(공진/레일리 천이), "
    "Li & Ling 실측에서 3–6 GHz 는 12–15 GHz 보다 평균 ~12 dB 낮다. 15 GHz Mavic Pro 방위평균 −17 에 이 하강을 "
    "적용하면 3.5 GHz 진값은 **−25~−28 dBsm** 쪽이어야 하므로, few-λ(공진영역)에서 PO 가 σ 를 밝게 잡아 우리 "
    "값이 **수 dB 낙관적일 소지**가 있다(모노스태틱 PO 는 few-λ 에서 marginal). "
    "(3) 그래서 절대 dBsm 은 신뢰의 중심에 두지 않고, 검출은 **σ 밴드**로 제시해 상대 결론(모드·파형 비교)이 "
    "밴드 전체에서 흔들리지 않음을 보인다. 서지: `/data/public/jeong_drone_refs/`.</sub>",
    "",
    ...
    "",
    "<sub>**교차검증 판정.** ⚠ 위 표의 밝은 값(−15~−5 dBsm)은 **대형·고정익·고주파** 실측이라 소형 쿼드 "
    "S밴드의 기준이 **아니다**(크기·주파수 mismatch). **소형 쿼드(0.3~0.5 m)의 3.5 GHz 방위평균 기준은 대략 "
    "−28~−16 dBsm**(§6)이고, 우리 값은 그 **안·밝은 상단**이다 — 크기 mismatch 된 밝은 참조와 비교해 "
    "'보수적'이라 읽으면 과장이다. few-λ(공진영역)에서 PO 는 σ 를 밝게 잡는 경향이 있어 우리 값이 수 dB "
    "낙관적일 소지를 연다. ⚠ 정확 모델·측정조건·자세규약이 달라 **±몇 dB 앵커**이지 점일치가 아니다 — "
    "그래도 실측과 대조하는 편이 시뮬끼리 대조보다 강한 근거다. 이 일치는 메쉬의 **레이더 유효 형상**"
...
cells.append(md(
    ...
    "드러난다(§3·§5).",
    "3. **절대값은 실측 포락선 안이나 밝은 상단이다.** 스톡 Sionna 는 표적 σ 를 못 주므로(→report06) 자작 "
    "SBR+PO 로 계산했고(→report07), 그 절대값이 소형드론 실측 포락선(3–6 GHz 방위평균 −28~−16 dBsm)에 들고 "
    "크기·자세 트렌드를 재현함을 확인했다. 단 few-λ 에서 PO 특성상 **밝은 상단**이라, 검출은 σ 밴드로 제시해 "
    "상대 결론의 robust 함을 보인다(§6).",
    "",
