...
EXTERNAL = [
    ...
    dict(no="12", title="실외 장면 — 지면과 벽이 서면 무엇이 달라지나",
         #: ⛔2026-09-04 — 전 서사는 «실외 기록의 날개 박자는 솔버 인공물에 가려 있었고,
         #  그 인공물의 정체는 지면 거울 반사였다» 였다. 빗살 SNR 이 자유공간으로 **돌아오는
         #  것은 el −30° 한 칸뿐**이고(59.66 ↔ 59.62), el −60° 는 16.53 으로 51.82 에 한참
         #  못 미치며 el 0° 는 바닥(p99 ≈ 8.4) 아래라 이 원장이 말하지 않는다. 특정 자체는
         #  근거가 있다(진폭 7 자리 일치 · |Γ| 0.3929) — 문제는 **범위와 표본**이었다.
         thesis="자유공간에서 세운 판정이 **지면과 건물이 있는 자리**에서도 서는지 묻는다. "
                "빗살 하모닉 SNR 로 다시 재면 **el −30° 한 칸에서** 솔버 낙차 자세를 메운 "
                "뒤 값이 자유공간과 같아진다. el −60° 는 못 미치고, el 0° 는 바닥 아래라 "
                "이 원장이 말하지 않는다. 그 자세에서 빠지는 경로는 지면 정반사로 "
                "특정했다 — ⚠실외 한 칸의 자세 하나를 덤프한 결과다.",
