...
VOLUMES = [
    ...
    # ------------------------------------------------------------------ #
    #  2026-08-12 추가 — 앙각 축과 엔진 물리 스위치. 위 15 권은 손대지 않는다.
    #  ⚠ 계획(`outputs/volumes_16_17_plan.json`)의 조각 81 `el-beat-vs-tip` 은 이
    #    라운드에 지어지지 않았고, 조각 87 은 예산 축이라 권 16 의 마지막 절로 세운다.
    #    조각 88 은 계획 밖에서 지은 권 17 의 범위 절이다.
    # ------------------------------------------------------------------ #
    ("16", "elevation-coverage", "앙각 커버리지 — 어느 각도까지 유효한가",
     ...
     "두 번째 결론이다.",
     ["78", "79", "80", "82", "87"], "79"),
    ("17", "engine-physics", "엔진의 물리 스위치 — 켜면 무엇이 달라지나",
