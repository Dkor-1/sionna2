...
cells.append(md(
    ...
    "",
    "**🎬 애니메이션** — 프로펠러가 회전하며 스펙트로그램의 시간커서가 **블레이드 플래시와 동기**되는 모습:",
    "![microdoppler anim](outputs/figures/report3_anim_microdoppler.gif)",
    "",
    "- **블레이드 플래시 주파수** = (날 수)×(회전/초). 회전수는 **프로펠러 크기에 맞춘 드론별 호버 rpm**(작은 프롭 빠르게·큰 프롭 느리게):",
    ...
    "",
    "> **report2 의 파일럿 반복률과 연결**: f_tip 이 1~2 kHz라, 모호 없이 보려면 **PRF ≳ 2·f_tip(수 kHz↑)** 이 필요합니다.",
