...
■ 왜 PO 를 없앨 수 없나 — 그리고 "광선을 더 쏘면 되지 않나"에 대한 답
  ...
  표면적분 단계가 없으므로 **표적의 σ 가 창발하지 않는다**. 광선을 늘려도 안 된다 — 실측:
    ...
    · 드론 확산 에코(mavic4pro @ 3.5 GHz · `bistatic_scene` 기하 β 47.6° · 시드 5 평균):
      광선 100M→400M 로 늘리면 **코히어런트** 합이 **+7.0 dB 계속 커진다**(−61.08 → −54.05 dB ·
      25M→400M 이면 +16.1 dB — 수렴 안 함).
      그리고 **산란계수 S 에 비례**한다(S 2배 → +15 dB). S 는 드론과 무관한 재질 노브다.
      ⚠ **비코히어런트** 합은 갈래가 다르다 — 같은 100M→400M 에서 −73.63 → −72.97 dB 로
        좁혀진다(25M 부터 보면 −75.93 → −72.97). 두 합산 규약을 갈라 적는다.
      원장 `outputs/rt_ray_budget.json` : `A_ray_sweep[].spp` · `.coh_db` · `.incoh_db`
        (`seeds` = 5 · `geometry.beta`) · 재현 `benchmark/verify_rt_rays.py`.
      ⛔ 정정(2026-09-06): 이 자리에 오래 **"+8~12 dB"** 라 적어왔는데 원장에 없는 어림수였고
        수렴하는 비코히어런트 갈래와 구분도 안 했다. 같은 수가 `docs/ARCHIVE.md:18` 에도 있다.
    · 게다가 ITU metal 은 **S=0** → 모터·배터리·PCB(지배적 산란체)가 확산에 **기여 0**.
  → σ 는 **표면적분에서 나온다** — 산란적분 단계 없이는 표적 σ 가 창발하지 않는다.
     PO 는 꼼수가 아니라 **그 적분의 고주파 근사**다: 균일전류 J⁰ 만 센다. 빠진 프린지 항
     J¹ = J − J⁰ 는 `src/ptd_edges.py` 가 ptd=True 일 때 따로 코히어런트 가산한다
     (아래 「PTD (모서리 프린지) 배선」 절 — 기본은 꺼져 있다).
...
def sbr_field_ground(mesh: Mesh, group_mat: dict, fc: float, u, *,
    ...
    ■ 무엇을 하나 — **거울상(image) 법**이다. 평평하고 균질한 지면에서 지면 반사 경로는
      **지면 아래로 거울상을 뜬 레이다**에서 곧장 오는 경로와 정확히 같다. 그러면 왕복
      경로가 넷이고, 지면 한 번 튀는 둘을 **한 항으로 묶어** 셋으로 센다:

          E = E(û,û)  +  2Γ·E(û′,û)  +  Γ²·E(û′,û′)
              직접-직접      지면 한 번          지면 두 번

      ⛔⛔**여기서 «2» 는 상호성이 아니라 근사다 — 옛 판은 이것을 「상호성 E(a,b)=E(b,a)」
        라 적었는데 그 말은 이 커널에 성립하지 않는다.** 광선 격자가 **입사 방향 û_i 로**
        깔리고(:1364 `_ray_grid(ctr, Rout, n, d, u_i)`) 조명 패스가 û_i 에만 의존하므로,
        인자 순서를 바꾸면 **어느 삼각형이 조명되는지**가 달라진다. 곧 이 커널에서
        E(û′,û) ≠ E(û,û′) 이고, 위 식은 둘 중 **한쪽만 재서 2 를 곱한** 것이다.
        ⭐실측(matrice4e · 3.5 GHz · 15 m · **생산 간격 λ/12** · 앙각 넷 ×
          방위 여덟 = 32 줄. benchmark/ground_reciprocity_0907.py
          → outputs/ground_reciprocity_0907.json):
            · 두 순서의 상대차 — 중앙값 **0.195** · 90 분위 0.3891
              · 최대 1.6062
            · 그 차가 «합친 장» 에 걸리는 몫 — 중앙값 **0.1799 dB**,
              0.5 dB 넘는 줄 7/32 · 1 dB 넘는 줄
              2/32 · 최대 1.3659 dB
        ⛔⛔**「이것은 이산화 오차다 — 격자를 촘촘히 하면 준다」는 철회한다**
          (2026-09-15, 외부 점검 메모 7 번 · 우리가 직접 재서 확인).
          그 문장은 **드론 메쉬 사다리 하나**(상대차 중앙값 lam/8 0.3549 · lam/12 0.1442 ·
          lam/16 0.0865 · lam/24 0.0576)에서 **일반화한 것**인데, 한 각도 쌍만 떼어
          재면 격자를 아무리 촘촘히 해도 안 준다.
          ⭐반례 실측 — 정사각 평판 0.25 m · 3.5 GHz · |Γ|=1 · 관통/PTD 끔 ·
            입사 0° / 출사 60°. 순서를 바꾼 크기 비:
              lam/12  1.962970 · lam/24  2.078568 · lam/48  1.996787 · lam/96  1.999196
            곧 1 이 아니라 **cos(0°)/cos(60°) = 2 로 수렴한다.**
          ⭐왜 그런가 — 광선 격자가 û_i 에 수직으로 깔리고 히트마다 d² 를 더하므로,
            넓이 S·법선 n̂ 인 면이 받는 광선 수는 S·(n̂·û_i)/d² 다. 곧 이 합은
            **입사 코사인만** 무게로 진다(Σd² = S·cos θ_i). 순서를 바꾸면 그 무게가
            cos θ_s 로 바뀐다. 각 쌍 여덟 개로 재 보면 여섯 쌍이 cos θ_i/cos θ_s 를
            0.06 안쪽으로 따라간다(널 근처 두 쌍은 벗어난다 — 위상 적분도 함께 바뀌므로
            cos 비가 전부는 아니다).
          ⇒ **격자 세분으로 없어지는 것이 아니다.** 드론 메쉬에서 사다리가 준 것은
            면 방향이 여럿이라 cos 비가 서로 상쇄된 몫까지 함께 본 것이다.
          ⛔**어느 쪽이 물리적으로 옳은지는 여기서 판정하지 않는다** — PO 자체가
            상호적이지 않고, 상호적인 식이 무엇을 줄지는 별도 기준해와 맞대야 안다.
          ⚠그래도 **지면 항의 절대 크기는 인용하지 않는다** — 생산 간격에서 합친 장이
            최대 1.3659 dB 움직인다.
          ⚠간격을 바꾸면 격자 원점도 바뀌므로, 사다리의 「촘촘해서 준 것」과
            「원점이 달라서 준 것」은 이 원장이 안 가른다.
        ⇒ **지면 항의 절대 크기를 인용하지 않는다.** 이 근사를 없애려면 E(û,û′) 도 재서
          평균하면 된다(계산이 넷으로 는다).
      ⚠`benchmark/verify_ground_kernel.py` 의 게이트들은 이 근사를 **안 덮는다** —
        그 파일은 드론 메쉬를 만들지 않고(try/except 두 갈래가 다 None) 3 cm 껍질에서만
        잰다. 껍질은 λ(≈8.6 cm)보다 작아 방향 쏠림이 거의 없으니 상호성이 잘 맞는 것이
        당연하다.
