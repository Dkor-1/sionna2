...
def fig_geometry(outdir=FIG, target="mavic4pro", fc=3.5e9):
    ...
    ax.text(*(0.5 * (tx + rx) + [0.5, 0, 0.6]), "Direct path L (DPI)", color="#c62828", fontsize=8)
    # 드론 메쉬(확대) + 속도벡터
    ...
    ax.text(tg[0] + 2.5, tg[1], tg[2] + 2.0, f"Target {_NAME[target]}\nv={np.linalg.norm(VEL):.1f} m/s",
            color="k", fontsize=9)
    ...
    axp.plot(tg[0], tg[1], "o", ms=9, color="k"); axp.text(tg[0]+0.6, tg[1], "Target", color="k")
    ...
    axp.plot(ell[:, 0], ell[:, 1], color="#7b1fa2", lw=1.3, label="Iso-Rb ellipse (single-Rx position ambiguity)")
    ...
    axp.set_title(f"Plan view · L={p['L']:.1f}m  Rb={p['Rb']:.1f}m  delay={p['tau']*1e9:.0f}ns  "
                  f"f_d={p['fd']:+.0f}Hz  bistatic angle={p['beta']:.0f}°", fontsize=10)
...
def fig_rangedoppler(outdir=FIG, fc=3.5e9, M=48):
        ...
        if "After" in tag:
            ...
            if pk:
                ax.plot(pk["Rb"], pk["fd"], "x", color="r", ms=12, mew=2.5,
                        label=f"Detection ({pk['Rb']:.0f}m, {pk['fd']:+.0f}Hz)")
    ...
    axes[0].text(0.02, 0.96, f"True target ○ (Rb={p['Rb']:.0f}m, f_d={fd:+.0f}Hz)", transform=axes[0].transAxes,
...
def gif_bistatic_tracking(outdir=FIG, target="mavic4pro", occupancy="G3",
    ...
    def update(kf):
        ...
        axg.set_title(f"Flight {kf+1}/{n_frames} · Rb={p['Rb']:.0f}m f_d={p['fd']:+.0f}Hz", fontsize=10)
