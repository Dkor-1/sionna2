...
cells.append(md(
    ...
    "3. **거리분해능은 명목 대역폭이 아니라 '기준신호가 실제 점유한 대역'이 좌우** — PRS 가 켜지면 5G(98MHz)·WiFi(80MHz)가 ≈1.5~1.9m로 또렷하지만, **한가한 5G 는 SSB(7.2MHz)뿐이라 ≈21m로 오히려 가장 거칠다**. LTE 는 CRS 가 상시 전대역(18MHz)이라 한가해도 ≈8.3m로 일정 — 이 점유 의존성(5G 의 거리·속도 이중고)이 본 보고서의 핵심.",
    "",
...
cells.append(md(
    ...
    "",
    "$$\\sigma(\\hat u)=\\frac{4\\pi}{\\lambda^2}\\Big|\\sum_{\\text{조명면}} |\\Gamma_g|\\,(\\hat n\\cdot\\hat u)\\,\\Delta A\\,e^{\\,j2k(\\mathbf r\\cdot\\hat u)}\\Big|^2$$",
    "",
    ...
    "",
    "→ 볼록 표적(평판·구, |Γ|=1 금속)에서 이론과 일치. 단 이 PO 는 **자기차폐·다중반사를 무시**하므로",
    "  오목한 드론(암·로터)의 *절대* RCS 는 다소 과대계상될 수 있습니다(상대·각도 경향은 신뢰).",
    "",
    "### 재질 가중 — 플라스틱 셸은 RF 에 반투명하다",
    "드론을 전부 금속(PEC)으로 두면 셸 기여가 과대계상됩니다. 실물에서 후방산란을 지배하는 건",
    "**배터리팩·모터·ESC/PCB·카메라 금속**입니다. 그래서 부위별 진폭 반사계수 $|\\Gamma_g|$ 를 적분에 곱하고",
    "(플라스틱 셸 0.28 · 프로펠러 0.25 · 카본 암 0.90 · 금속/배터리 1.0 · PCB 0.80 — `drones.GROUP_GAMMA`),",
    "메쉬에 **내부 금속 산란체**(배터리·PCB, 파라메트릭 박스)를 추가했습니다. 셸이 반투명하니 내부가 '보이는'",
    "효과를 '셸 |Γ| 축소 + 내부 |Γ|=1 합산'으로 1차 근사한 것입니다(왕복 투과손실 ~0.5 dB 는 무시):",
    "",
    "![rcs materials](outputs/figures/report2_rcs_materials.png)",
    "",
    "방위평균 RCS 가 기종별 **2~6 dB 낮아져** 실측 문헌값 대에 들어옵니다(아래 앵커링 표).",
    "",
    "### 실측 앵커링 — 공개 측정 문헌과 대조",
    "",
    "| 급 (우리 모델, PO 재질가중 @3.5 GHz) | 실측 문헌값 | 판정 |",
    "|---|---|---|",
    "| **Phantom 4 ≈ −19.8 dBsm** | 비행 실측 평균 **−17**(문헌 합의 −20) @8.75 GHz [De Quevedo 2018] · 방위평균 **−15.0** @15 GHz [Ezuma 2021] · 챔버 **−20~−30** @10 GHz(Phantom 2) [Schröder 2015] | **±2~3 dB 정합** ✓ |",
    "| **Mavic 4 Pro ≈ −20.8 dBsm** | Mavic Pro(구세대·더 소형) 방위평균 **−17.1** @15 GHz [Ezuma 2021], **−16.8** @26 GHz [Semkin 2020] | 대역·기종차 감안 합리(다소 보수적) |",
    "| **S1000+ ≈ −6.0 dBsm** | S1000+ 챔버 **−8**(VV) @5.8–8.2 GHz [Herschfelt 2017] · S900 비행 modal **−8** @24 GHz [Rahman 2019] · M600 방위평균 **−11.7** @15 GHz [Ezuma 2021] | **동일 등급**(~2 dB 상향) ✓ |",
    "| **Mini 5 Pro ≈ −25.8 dBsm** | 250 g급 직접 실측 **공백** — 간접값 ~−30 dBsm(Mini 2, 챔버) [Vovchuk 2025] | 데이터 갭(그럴듯한 대역) |",
    "",
    "주의: 실측은 대역(X·Ku·K)·측정종류(방위평균/modal/특정각)·편파가 제각각이라 **±수 dB 산포는 본질적**입니다.",
    "순수 S-band 쿼드 방위평균 공개 실측은 사실상 공백이며, 문헌 경향(주파수↑→RCS 완만↑, Ezuma 15→25 GHz +3~5 dB)을",
    "감안하면 Phantom급의 3.5 GHz 기대값은 **−22~−16 dBsm** — 우리 재질가중 PO 가 그 안에 듭니다.",
    ...
    "  구는 πr²로 주파수 무관 → 평판·구가 섞인 드론은 **완만(≈+0.5~3.6 dB/oct)하고 일부 구간은 비단조**. 전체 크기순은 S1000+>…>Mini.",
    "",
    ...
    "0.4 mm 해상도, Thingiverse 1456295, CC-BY)과 같은 조건(둘 다 PEC·프로펠러/카메라 제외)으로 대조했습니다:",
    "",
    "![shape ab](outputs/figures/report2_rcs_shape_ab.png)",
    "",
    "- 방위평균 차이는 **대역별 +0.7~+2.8 dB** — 이 대역(λ=6~16 cm)에서 λ/8 이하 형상 디테일은 파장이",
    "  '보지 못하므로', 실루엣·부위 배치를 맞춘 파라메트릭 메쉬로 충분하다는 뜻입니다.",
    "- 즉 **정확도 개선의 지렛대는 형상(≈1~3 dB)이 아니라 재질·내부 구조(≈2~6 dB)** — 위 재질 가중이 그 답입니다.",
))
...
cells.append(md(
    ...
    "",
    "| 표준 | 변형 | 반송파 | 채널대역(점유) | SCS | FFT | 기준신호 | 분해능(기준대역) |",
    "|---|---|---|---|---|---|---|---|",
    "| **WiFi** | 802.11ac VHT | 5.2 GHz | 80 MHz (점유 76) | 312.5 kHz | 256 | **VHT-LTF**\\* | 1.9 m |",
    "| **LTE** | Rel-9 FDD | 1.8 GHz | 20 MHz (점유 18) | 15 kHz | 2048 | **PRS**/CRS | 8.3 m |",
    "| **5G NR** | Rel-16 n78 | 3.5 GHz | 100 MHz (점유 98) | 30 kHz | 4096 | **NR-PRS**/SSB | 1.5 m (SSB만 → ≈21 m) |",
    "",
    ...
    "> 그래서 LTE 는 20 MHz 채널이지만 CRS/PRS 점유 18 MHz 로 **7.5 m 가 아닌 8.3 m** 입니다(아래 셀 출력과 일치).",
    "",
...
cells.append(md(
    ...
    "LTE CRS(1.8 GHz·1 kHz)는 ≈41 m/s로 충분하지만, 같은 1 kHz라도 WiFi(5.2 GHz)는 λ가 3배 짧아 ≈14 m/s에 그칩니다(위 표). 5G 는 한가하면",
