...
cells.append(md(
    ...
    "  로브 사이사이의 **깊은 널(null)** 은 오류가 아니라 산란체들의 위상이 상쇄되는 코히런트 PO 의 정상 거동입니다",
    "  (예: Mavic 4 Pro @3.5 GHz, 328.3° 에서 −72 dBsm 까지 파임 — 폭은 2~3° 에 불과). 다만 이렇게 깊은 널은 현실에선",
    "  **유한 대역폭·표면거칠기·다중경로가 메우므로** 관측되지 않습니다. 그래서 그림의 반경축은 **−45 dBsm 에서 잘라(clip)** 두었습니다",
    "  — 자르지 않으면 널 한 점이 선을 원 중심까지 끌고 들어가 표적이 사라진 것처럼 보입니다.",
...
cells.append(md(
    ...
    "|---|---|---|---|---|",
    "| **G1** 유휴 셀 | **상시 신호만** — 패시브 레이더의 기본선 | 프리앰블만(L-LTF) | 동기 + **CRS**(상시·전대역) | **SSB 만**(협대역 비콘) |",
    ...
    "**LTE G1 은 전대역에 촘촘히 박힌 CRS 격자(3.2%)**, **WiFi G1 은 광대역 프리앰블(15%)** 입니다 —",
    "같은 '유휴'인데 5G 만 기준신호가 좁습니다. G3 로 갈수록 데이터가 채워집니다(WiFi 90% · 5G 80% · LTE 59%).",
    "",
...
cells.append(md(
    ...
    "",
    "→ **추정 RCS 가 참값(PO)과 일치**합니다(LTE·5G 오차 ≲0.02 dB, WiFi +0.3 dB — WiFi 는 프리앰블의 L-STF 가",
    "기준신호 상관에 조금 섞여 피크를 약간 들어 올립니다) → 구 보정 정합필터가 정상 동작합니다.",
