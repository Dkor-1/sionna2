...
cells.append(md(
    ...
    "",
    "**3줄 결론**",
    "1. 드론 RCS 는 **물리광학(PO)** 으로 메쉬에서 직접 계산 — 평판·구 이론과 일치하게 검증됨(@3.5 GHz).",
...
cells.append(md(
    ...
    "",
    "**왜 광선추적(Sionna RT)이 아니라 PO?** — 검증해 봤더니 **Sionna RT 의 기본 path solver 에는 산란적분 단계가 없어 표적 σ 가 창발하지 않습니다.**",
    "",
    "> ⚠️ **범위를 정확히**: *\"레이트레이싱은 RCS 를 못 낸다\"* 는 **거짓**입니다 — SBR(GO+PO)은 기하에서 RCS 를 계산해냅니다. 참인 명제는 훨씬 좁습니다: **\"산란적분 단계가 없는 전파용 레이트레이서(Sionna RT 포함)에서는 창발하지 않는다.\"**",
    "",
    "그 기본 solver 가 표적에서 만드는 경로는 두 종류뿐인데 둘 다 σ 의 함수가 아닙니다:",
    "",
    "| RT 경로 | 진폭 법칙 | 실측 |",
    "|---|---|---|",
    "| **정반사** | image-source(무한거울) | 금속 평판의 변을 0.2 → 4 m 로 키워 **σ 를 52 dB** 바꿔도 진폭비는 **−7.91 dB 로 불변**(산포 0.00 dB) |",
    "| **확산** | 산란계수 **S** 의 함수 | 같은 금속구에서 S 만 0.2 → 1.0 으로 올리면 **+15.8 dB** 이동(S² 법칙) |",
    "",
    "게다가 물리적으로 옳은 **PEC 금속구(S=0)에서는 RT 가 경로를 하나도 만들지 못합니다**(반지름 0.3~3 m, 표본 100만~1600만, 전부 0개).",
    "즉 RT 로 PO 를 맞추는 건 **자유 파라미터 S 를 피팅하는 순환논법**입니다. 그래서 **표적 σ 는 PO, 환경(벽·다중경로)은 RT** 로 나눕니다 —",
    "**기본 solver 를 쓰는 한** 편의가 아니라 필연입니다(정공법이 없는 건 아닙니다 — 메인테이너가 지목한 custom PathSolver 로 in-Sionna 산란적분을 넣는 길이 존재하며, 우리가 안 하는 이유는 **부재가 아니라 비용/충실도**입니다).",
    "(근거·재현: `benchmark/verify_rt_no_rcs.py` · `docs/VERIFY_RT_VS_PO.md`)",
    "",
    "PO 는 CAD 표적 RCS 의 표준기법이고, report1에서 만든 메쉬에서 바로 계산합니다:",
    "",
    "$$\\sigma(\\hat u)=\\frac{4\\pi}{\\lambda^2}\\Big|\\sum_{\\text{조명면}} |\\Gamma_g|\\,(\\hat n\\cdot\\hat u)\\,\\Delta A\\,e^{\\,j2k(\\mathbf r\\cdot\\hat u)}\\Big|^2$$",
    "",
    "**검증(이 방법이 맞다는 근거)** — 표준 표적과 이론값 비교 (**@3.5 GHz, λ=8.57 cm**):",
    "",
    "| 표적 | PO 결과 | 이론 | 오차 |",
    "|---|---|---|---|",
    "| 평판 0.3 m | 11.42 dBsm | 4πA²/λ² = 11.42 | 0.00 dB |",
    "| 구 r=0.3 m | −5.49 dBsm | πr² = −5.49 (주파수 무관) | 0.01 dB |",
    "",
    "표의 두 값은 **3.5 GHz 기준**입니다. 평판은 σ ∝ 1/λ² 이라 대역이 바뀌면 이론과 PO 가 **함께** 이동하고",
    "(1.84/5.21 GHz 에서 5.84/14.88 dBsm), 구는 이론이 πr² 로 대역 무관입니다.",
    "",
    "> ⚠️ **평판 검증은 근거로 쓰면 안 됩니다.** 수직입사에서는 위상항이 사라져(P·û ≡ 0), 커널에 버그를 주입해도",
    "> 여전히 0.00 dB 가 나오는 **진단력 0의 항등식**입니다. 커널을 실제로 시험하는 건 **구**뿐이며,",
    "> 우리의 이산 PO 는 구의 **해석적 PO** 를 **±0.015 dB** 로 재현합니다(그 해석적 PO 자신이 GO(πr²) 대비 ±0.35 dB — PO 의 O(1/ka) 항).",
    "",
    "→ 볼록 표적(평판·구, |Γ|=1 금속)에서 이론과 일치. 단 이 PO 는 **자기차폐·다중반사를 무시**합니다 —",
    "  자기차폐 무시는 앞 구조에 가려질 뒷면까지 더하므로 **과대** 방향, 다중반사 무시는 암-동체 이면반사·로터-모터",
    "  코너반사 기여를 아예 빠뜨리므로 **과소** 방향입니다. **두 오차의 부호가 반대**라 오목한 드론(암·로터)의 *절대* RCS",
    "  순 편향은 형상에 따라 달라 한 방향으로 단정할 수 없습니다(상대·각도 경향은 신뢰). 실제로 아래 앵커링 표에서",
    "  우리 값은 실측 문헌 대비 오히려 **보수적**(작은 쪽)입니다.",
    "",
    ...
    "",
    "> 🔍 **깊은 널(null)은 왜 안 그리나 — 검증 기록**",
    "> 단일 주파수 코히런트 PO 로 그리면 로브 사이에 **−60 dBsm 이하로 꽂히는 바늘**이 생깁니다. 셋을 확인했습니다:",
    "> 1. **통계적으로는 정상**입니다 — 코히런트 산란체의 σ 는 지수분포(레일리 페이딩)를 따르고, 측정 CDF 가 이론과 일치합니다",
    ">    (P(σ<0.1·σ̄): 측정 13% vs 이론 10%). 즉 '깊은 널이 드물게 존재'하는 것 자체는 물리입니다.",
    "> 2. **그러나 개별 널은 수치 아티팩트입니다** — 메쉬 이산화를 λ/7 → λ/12 로 바꾸면 방위평균은 0.12 dB 밖에 안 변하고",
    ">    패턴 상관은 0.986 인데, **최저점 위치가 330° → 127° 로 완전히 이동**합니다. 어느 각도가 얼마나 파이는지는 신뢰할 수 없습니다.",
    "> 3. **실제 레이더는 그 널을 보지 못합니다** — 5G 100 MHz 로 대역평균하면 최저값이 **−59 → −45 dBsm(+14 dB)**,",
    ">    거기에 안테나 빔·표적 요(yaw) 지터에 해당하는 **3° 각도 창**까지 평균하면 −41 dBsm 이 됩니다.",
    ">    이때 **로브 피크(−11.5 → −11.7 dBsm)와 방위평균(−20.8 dBsm)은 사실상 불변**입니다.",
    ">",
    "> → 그래서 (a) 는 **레이더가 실제로 보는 값**(100 MHz 대역평균 + 3° 창)으로 그립니다. 앵커링 표·막대그래프의 방위평균 값은",
    "> 대역평균 여부와 무관하므로 그대로입니다. (`rcs_po.drone_rcs_pattern_bw` · `angular_smooth`)",
    ...
    "  반면 **중간 세 쿼드는 크기순이 아니고 주파수마다 순서가 뒤바뀝니다** — @3.5 GHz 는 Phantom(−19.8) > Mavic(−20.8) > Matrice(−21.7) dBsm 인데",
    "  대각거리는 정반대(Matrice 438.8 > Mavic 400 > Phantom 350 mm). 방위평균을 지배하는 건 대각거리가 아니라 **동체·배터리 같은 금속 산란체의 크기·배치**와 글린트 간섭 위상입니다.",
    "",
...
cells.append(md(
    ...
    "",
    "→ **추정 RCS 가 참값(PO)과 일치**합니다(오차 ≲0.12 dB) → 구 보정 정합필터가 정상 동작합니다.",
    "(WiFi 만 +0.12 dB 로 조금 큰데, 프리앰블 기준신호가 프레임에서 차지하는 에너지 비중이 낮아 잡음에 더 민감하기 때문입니다.)",
    "다만 이건 **처리체인의 자기일관성 점검**이지 절대 RCS 물리의 검증은 아닙니다(에코와 보정구를 같은 레이더방정식으로",
    "만들었으니 λ·R² 항이 상쇄됩니다). 절대값의 타당성은 §2 의 **평판·구 이론 검증**과 **실측 문헌 앵커링**이 담당합니다.",
))
...
cells.append(md(
    ...
    "- 드론 RCS 를 **물리광학(PO)** 으로 특성화 — 평판·구 이론 검증 + 재질 가중 + 실측 문헌 앵커링.",
    "- **실제 WiFi/LTE/5G OFDM 파형**을 표준 구조대로 만들고, **각 표준의 상시 기준신호**로 표적을 재서 비교.",
