...
q4 = dict(
    question_ko="격자 λ/24 전앙각·λ/48 신규분이 산포 밴드(리듬 21.8%p·AC 3.86dB)를 좁히나 "
                "넓히나. " + "⛔R29 — 리듬 몫의 **크기는 인용하지 않는다**(창 반폭 hw 2/8/32 Hz 로 같은 데이터가 9.9/63.4/90.0 %). «λ/12 한정» 꼬리표로는 부족하다 — 격자보다 분석 손잡이가 더 크게 흔든다. 순서만 읽는다.",
...
doc = {
    ...
    "verdicts_ko": {
        ...
        "q4_grid_band": "⛔R29 — 리듬 몫의 **크기는 인용하지 않는다**(창 반폭 hw 2/8/32 Hz 로 같은 데이터가 9.9/63.4/90.0 %). «λ/12 한정» 꼬리표로는 부족하다 — 격자보다 분석 손잡이가 더 크게 흔든다. 순서만 읽는다. 빗각 AC 밴드는 좁아지고(≤1.31 dB) 전체 폭은 −90° 가 5.62 dB 로 넓힘·리듬 21.8 %p 유지·빗살 대비에 새 격자 단서(≈5 dB)",
    },
