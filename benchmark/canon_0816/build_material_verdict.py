...
scoring = {
    ...
    "H3_배선대조군_el0": {
        ...
        "verdict_ko": ("셸 0.75+프롭 0.9 결합판인데도 el 0 은 레벨·DC·AC 전부 0.000 dB — "
                       "⛔«손잡이가 의도 밖 재질(금속)을 안 잡았다» 는 J6 이 내렸다(원인 진술이었다): "
                       "손잡이는 정면에서도 프롭을 잡았고, 그 몫이 이 칸 자신의 요동보다 52~55 dB "
                       "아래라 «묻혔다» 가 옳다(outputs/material_canon_0816.json : "
                       "adjudications[5].ruling_ko · do_not_write_ko[1]). 배선 대조군으로서의 "
                       "판정(0.000 dB, 통과)은 그대로다. 덤으로 §2-4 의 "
...
headlines = {
    #: ⛔`build_material_canon.py` 의 J4 가 「H5 방향 진술 **철회**. 깊이 1 실측은 반대다 —
#  굴절 팔이 절대로도(−19.4 dB) 다 끔 대비로도(−2.4 dB) **더 어둡다**」라 적고, affects_ko
#  가 이 키를 정확히 지목한다. 그런데 이 문장이 그대로 원장으로 나가고 있었다(2026-09-04).
"굴절만_거리_생존": ("유지(**기울기만**) — 거리 기울기는 재질 곱셈 이득과 무관해 안 "
                     "흔들린다. ⛔**레벨 «방향» 진술은 철회한다**(정본 J4) — 깊이 1 실측은 "
                     "굴절 팔이 **더 어둡다**(절대 −19.4 · 다 끔 대비 −2.4 dB). 깊이 ≥2 는 "
                     "미측정. 옛 문장: 굴절만 팔은 전부 100 mm 에서 돌았고, 정정(셸 0.75 mm)은 "
