...
이 시험
    같은 씬·같은 씨앗·같은 광선 수에서 **스레드 수만** 바꾼다.
      스레드 1 에서 흔들림이 사라지면 ⇒ **병렬 실행에 기대는 처리**가 후보로 남는다
      스레드 1 에서도 흔들리면    ⇒ 병렬 실행 밖의 자리를 봐야 한다

⛔⛔**정정(2026-09-14) — 「원인은 병렬 축약 순서다」로 못 박지 않는다.**
  옛 머리말과 `reads_ko` 는 「스레드 1 에서 발생률이 0 이면 원인은 병렬 축약 순서다」라고
  적었다. 그 결론은 이 시험이 못 짚는다 — **스레드 수는 축약 순서만 바꾸지 않는다.**
  실행 순서·작업 분할·커널 실행 방식·해시 통에 먼저 닿는 스레드가 함께 바뀐다.
  어느 것인지 가르려면 **그 연산을 바꾼 대조**가 필요하다(예: 축약을 결정적 구현으로
  갈아 끼우고 같은 사다리를 다시 재기 · 후보 집합을 그대로 두고 합만 순서 고정하기).
  ⚠그리고 적은 판 수에서 같았다는 것이 **다른 입력 전체의 보장도 아니다** — 이 시험은
    합성 평판 한 장면·씨앗 하나·판 40 개다.
...
def main() -> None:
    ...
    json.dump({"_meta": {
        ...
        #: ⛔⛔2026-09-14 정정 — 옛 문장은 「원인은 **병렬 축약 순서**다」였다. 이 시험은
        #  스레드 수만 바꾸므로 **어느 병렬 처리인지 못 짚는다**(실행 순서·작업 분할·
        #  커널 실행 방식·해시 통 선점이 함께 바뀐다). 관측만 적고 원인은 후보로 둔다.
        "reads_ko": ("시험한 입력·판 수에서 스레드 1 의 결과가 서로 같았다면, **병렬 실행에 "
                     "기대는 처리**가 원인 후보로 남는다 — ⛔어느 연산인지는 이 시험이 못 "
                     "짚는다. 1 에서도 흔들리면 병렬 실행 밖을 봐야 한다. "
                     "⛔판정은 여기 적지 않는다."),
        "scope_ko": ("합성 평판 한 장면 · 씨앗 하나 · 각 단 40 판이다. 다른 입력·다른 "
                     "기계에서도 같다는 보장이 아니다."),
