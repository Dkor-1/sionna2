...
# ═══════════════════════════════════════════════════════════════════════════ #
#  ⭐재현기 — 한 자세를 여러 판 풀어 «경로 하나 손실» 의 귀속을 가른다 (2026-09-02)
#    ⛔이 파일은 `elevation_sweep_md.py` 의 **사본**이다. 원본은 큐가 돌리는 중이라
#      건드리지 않는다. 장면 구성이 원본과 한 줄도 다르지 않아야 뜻이 있다.
# ═══════════════════════════════════════════════════════════════════════════ #
_PROBE_POSE = (int(os.environ["PROBE_POSE"]) if os.environ.get("PROBE_POSE") else None)
_PROBE_REPS = int(os.environ.get("PROBE_REPS", "8"))
_PROBE_FRESH = os.environ.get("PROBE_FRESH", "") not in ("", "0")
_PROBE_OUT = os.environ.get("PROBE_OUT", "/tmp/probe_drop.json")
#: ⭐**달굼(warm-up) 자세 수** — 목표 자세 앞의 몇 자세를 «정상 경로로» 먼저 돌릴지.
...
def _probe_one(RP, sc, diffuse, mdep, sw, spp, fc, pose, el, solver=None):
    ...
    ⭐가르려는 것 — 경로가
      (ⓐ) **안 돌아온 것**            → `n_ret` 이 줄어든다  (솔버 쪽)
      (ⓑ) **돌아왔는데 NO_OBJ 라 버린 것** → `n_ret` 은 그대로인데 `n_hit` 이 준다 (우리 쪽)
    """
    import hashlib
    rows = []
    # ⭐달굼을 거친 **그 솔버 객체**를 그대로 쓴다 — 상태가 쌓였다면 여기 있다.
    solver = None if _PROBE_FRESH else (solver or RP.rt.PathSolver())
    for k in range(_PROBE_REPS):
        ...
        E = complex(np.sum(t))
        dtau = np.sort(tau[~hit]) if aa.size else np.zeros(0)
        rows.append(dict(
            rep=k, n_ret=int(aa.size), n_hit=int(hit.sum()), n_noobj=int((~hit).sum()),
            E_re=float(E.real), E_im=float(E.imag), E_abs=float(abs(E)),
            tau_hash=hashlib.sha1(np.sort(tau).astype("<f8").tobytes()).hexdigest()[:16],
            hit_tau_hash=hashlib.sha1(np.sort(tau[hit]).astype("<f8").tobytes()).hexdigest()[:16],
            drop_tau=[float(x) for x in dtau[:8]],
            incoh=float(np.sum(np.abs(aa) ** 2)),
        ))
        print(f"  판 {k}: n_ret={aa.size} n_hit={int(hit.sum())} "
              f"버림={int((~hit).sum())} |E|={abs(E):.6e}", flush=True)
    with open(_PROBE_OUT, "w") as fh:
        json.dump(dict(pose=int(pose), el=float(el), reps=_PROBE_REPS,
                       fresh_solver=_PROBE_FRESH, spp=int(spp), max_depth=int(mdep),
                       sw={k: bool(v) for k, v in sw.items()}, rows=rows), fh, indent=1)
    print(f"\n✅ {_PROBE_OUT}", flush=True)
    nr = sorted({r["n_ret"] for r in rows}); nh = sorted({r["n_hit"] for r in rows})
    print(f"  n_ret 종류 {nr} · n_hit 종류 {nh} · |E| 종류 {len({round(r['E_abs'],18) for r in rows})}")
    if len(nr) == 1 and len(nh) > 1:
        print("  ⇒ ⭐돌려받은 수는 같은데 남긴 수가 다르다 — **우리 NO_OBJ 마스크가 버렸다**")
    elif len(nr) > 1:
