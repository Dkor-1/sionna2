    ...
    for key in keys:
        ...
        for asp in aspects:
            ...
            cell["findings"] = {
                ...
                "occlusion_plate_caveat_ko": (
                    "⚠ 단일축은 **부호와 존재**까지다. 두 팔이 같은 판을 써도 기하가 달라서"
                    "(막는 팔은 동체가 있고 안 막는 팔은 동체 면이 없다) 판 오프셋 편향이 "
                    "공통모드로 빠지지 않는다 — 판을 반 칸 옮기면 위 두 dB 가 인용값보다 크게 "
                    "움직인다. 실측은 outputs/freeze_plate_sensitivity.json 의 verdict 다. "
                    "⭐크기를 쓰려면 오프셋 여러 판의 앙상블 평균이 먼저다."),
