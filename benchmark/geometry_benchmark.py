...
def readjudicate(mp, win, bf):
    ...
    rows = [
        ...
        #: ⛔2026-09-04 — 전 판은 `slope_mean` 을 4 자리 단일값으로 싣고 할 일 칸이 «없음»
        #  이라 1/4 이 물리로 확인된 것처럼 읽혔다. RETRACTION_LOG R11 이 넷으로 쪼개
        #  거둬들였다 — 15 전역 적합의 **평균**이고, 셀별 0.223~0.256, **국소는 0.049~0.357
        #  로 7.2 배**이며 국소 최대가 이론 상한 0.25 를 넘는다(σ 격자 고도 조회의 계단
        #  아티팩트). 그리고 순위불변성은 발견이 아니라 **항등식**이다.
        C("C5", "공통모드 σ 불변성 — dR_dB/dσ_dB = 1/4",
          "both", "σ 사슬이 두 팔에서 동일하다(모노는 같은 격자의 β=0 조회)",
          {"slope_mean": SIG["common_mode"]["slope_mean"],
           "slope_note_ko": "⚠15 전역 적합의 평균이다 — 셀별 0.223~0.256, 국소 "
                            "0.049~0.357(7.2배). 4 자리 단일값은 거짓 정밀이다(R11)",
           "monostatic_dsigma_db": st["monostatic_arm"]["dsigma_db"]},
          "⚠**항등식이지 발견이 아니다** — 세 모드가 같은 σ/R⁴ 사슬을 쓰므로 균일 dB "
          "오프셋은 모든 곡선을 같은 양만큼 평행이동시킨다. 국소 최대 0.357 은 이론 상한 "
          "0.25 를 넘으므로 σ 격자 고도 조회의 계단 아티팩트다(RETRACTION_LOG R11)",
          "defensible"),
...
def gaps():
    items = [
        ...
        dict(rank=6, item="φ 스윕을 σ·커버리지 산출물로 확장",
             ...
             decides="'기하는 링크버짓에 영향 없다' 가 φ=90° 의 산물인지 아닌지 "
                     "(⛔함정 T1b 의 «최대 23.2 dB» 는 R14 가 무효화했다 — φ 스윕 72 점에서 "
                     "R90 span 0.48 %)",
    ...
    return dict(
        ...
        order_to_cut=[
            ...
            dict(rank=2, cut="φ 13점 → 3점 {0°, 90°, 180°}",
                 saves="φ 의존 산출물 ×13 → ×3",
                 costs="φ 중앙값 통계가 3점 평균으로 거칠어진다. ⛔«absmax 23.2 dB» 를 "
                       "근거로 들던 문장은 R14 가 무효화했다 — φ 스윕 72 점에서 R90 span "
                       "0.48 % 이고 φ=90° 가 세 팔 모두 최소다",
...
def contribution():
    ...
    return dict(
        ...
        paragraph=[
            ...
            "⭐ **링크버짓은 다르다.** 기하의 링크버짓 기여는 20log10(R_조명원/R_센서) 한 "
            "줄이고 장면 방위에 따라 부호가 뒤집힌다. ⛔여기 적혀 있던 «최대 23.2 dB» 는 "
            "RETRACTION_LOG R14 가 무효화했다(2026-08-03 φ 실측 스윕 72 점) — 그 수는 φ 의 "
            "성질이 아니라 **스윕하지 않은 고도차 Δz = 35 m** 의 성질이고, 단일 d 칸 값이며 "
            "R90 동작점에서는 **≤1.20 dB**(d 중앙값 ≤3.10 dB)다. φ 의존은 사실상 없다 — "
            "R90 span 0.48 %(≤0.083 dB)이고 **φ=90° 가 세 팔 모두 최소**다. 같은 항이 "
...
def write_md(out):
    ...
    A("")
    #: ⛔이 문서는 2026-07-31 스냅숏이다. 그 뒤 철회 기록이 여러 줄을 무효화했는데,
    #  정정이 아래쪽에만 붙어 위에서부터 읽는 사람이 죽은 문장을 산 문장으로 읽었다.
    A("> ⚠⚠ **2026-07-31 스냅숏이다.** 그 뒤 `RETRACTION_LOG.md` 가 아래 여러 줄을")
    A("> 무효화했다 — **R14**(φ 스윕: «최대 23.17/23.2 dB» 는 φ 가 아니라 스윕하지 않은")
    A("> 고도차 Δz = 35 m 의 성질이다. φ 의존은 사실상 없고 φ=90° 가 세 팔 모두 최소) ·")
    A("> **R11**(공통모드 기울기 0.2462 는 15 전역 적합의 평균이고 국소는 0.049~0.357 로")
    A("> 7.2 배. 순위불변성은 발견이 아니라 항등식) · **R3**. ⛔이 문서의 절대 수치를")
    A("> 인용하기 전에 그 세 항목을 먼저 읽는다.")
    A("")
    ...
    A(f"| 기하 의존 | 없음 — 모노 = 바이 β=0, 차이 "
      ...
      f"있음 — 20log10(R_조명원/R_센서). ⛔전 판의 «장면 최대 "
      f"{_f(ax['geometry_absmax_db'],4)} dB» 는 R14 가 무효화했다 — R90 동작점 ≤1.20 dB · "
      f"d 중앙값 ≤3.10 dB, φ=90° 가 세 팔 모두 최소 |")
