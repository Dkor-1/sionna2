...
⭐손잡이는 **병렬 스레드 수**다 — 스레드 1 이면 40 판 전부 같은 값(발생률 0.0 · 경로 수도
   8059 하나)이고, 2 에서 0.025, 192 에서 0.35 로 오른다⟨outputs/thread_ladder_0903.json⟩.
    ⚠그 «발생률» 은 각 줄이 **자기 최빈값**을 기준으로 센 값이다 — 스레드 2 의 최빈
    경로 수는 8058 로 옮겨가 있어, 1-스레드 값 8059 를 공통 기준으로 다시 세면 다른 판이
    1 개가 아니라 **39 개**다(2026-09-10 에 잡음). 줄끼리 나란히 읽으려면
    thread_ladder_0903.py 가 새로 굽는 rate_off_vs_base 를 본다.
