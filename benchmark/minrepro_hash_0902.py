...
⚠경로가 적으면(454) 안 흔들린다. **경로가 많아 해시 통이 겹칠 때** 흔들린다.
⛔순정 씬(`simple_street_canyon`)으로는 재현이 안 된다 — 경로가 6 개뿐이고 재질
   산란계수가 0 이라 확산을 켜도 안 는다. 그때 |h| 폭은 **0.0008 dB**(부동소수점 수준)다.
   이 재현기의 4.7 dB 는 그보다 **2,700 배** 크다.

어디를 보나
-----------
`sionna/rt/path_solvers/sb_candidate_generator.py:484-498` — 정반사 사슬을 해시 통으로
중복제거하는데, `dr.scatter_inc` 로 통을 **먼저 올린 스레드만** 그 경로를 남긴다.
그 자리에 race condition 이 **주석으로 적혀 있다.**
