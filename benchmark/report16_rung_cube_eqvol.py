...
**우리 데이터가 그 지적을 상당 부분 뒷받침한다**:
  · 절대 세기에서 **부피를 맞춘 구**가 우리 정밀 메쉬보다 레벨이 가깝다
...
def _findings(J):
    ...
    F["q2_is_it_a_different_kind_of_modulation"] = dict(
        ...
        how_to_read_ko=("주기(period_deg)와 으뜸 차수(dominant_order)가 다르면 «다른 종류» 다. "
                        ...
                        "(차수 2의 배수)다. 파형 상관이 낮으면 두 파형이 서로 다르다는 뜻이다 — "
                        "실제로 구별되는지는 잡음·적분시간·문턱이 있어야 답한다. "
                        "이 단에서는 정합필터를 돌린 적이 없다(같은 파일 "
                        "`limits.not_a_detector_claim`)."))
