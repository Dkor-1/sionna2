...
#
# ⚠ 2026-07-14 이후 이 값은 **폴백**이다 — run_matrix.py 는 반송파별 RT 실측 잔향
#   (channel.rt_chamber_clutter, outputs/rt_env_clutter.json)을 주입한다. 캐시가 없을 때만 이 값이 쓰인다.
# ⚠ 그리고 이 값은 **의도적으로 그대로 둔다** — 위 docstring 참조. ECA 가 정적 클러터를 진폭과
