...
def summarize(led):
    ...
    led["open_problems"] = [
        ...
        "기준채널 잡음은 프레임마다 독립인데 이 모델은 한 실현을 M 번 재사용한다. "
        "⭐**그런데 E1c 가 이 걱정을 지웠다**: 정합필터만 오염시키면 손실이 −0.4~+0.7 dB "
        "(사실상 0)이고, ECA 만 오염시키면 전체 손실이 거의 그대로 나온다. 즉 헤드라인 손실은 "
        "**전부 ECA 단**에서 나오므로 이 모델링 한계는 결론에 영향을 주지 않는다. "
        "(정합필터 쪽 대가를 정밀하게 재려면 여전히 range_doppler 를 손봐야 한다.)",
        "⚠ **5G G1 의 운용점이 «천장» 이다.** σ² 를 0 으로 보내도 팔 A 의 출력 SINR 이 "
        "12.0 dB 를 못 넘는다(직접파 잔류가 바닥). 그래서 G1 의 팔들은 열잡음 한계가 아니라 "
        "잔류 한계에서 비교된 것이고, «자기일관 ρ_ref» 구성은 G1 에는 적용되지 않는다. "
        "1차 실행에서는 교정기가 천장 근처에서 못 움직여 σ²≈0 으로 발산했고(경험 Pfa 가 "
        "2.75e-3 로 깨졌다), 괄호치기+이분으로 고쳐 다시 돌렸다.",
