...
def range_normalisation():
    ...
    n2 = dict(
        ...
        meaning="φ=90°(수직이등분선)에서는 R1≈R2 라 두 기하가 거의 겹친다. φ 가 베이스라인 축에 "
                ...
                f"⛔이 절의 absmax_over_phi_db("
                f"{max(v['delta_db_absmax'] for v in n2_by_phi.values()):.3g} dB)와 바로 위 ⭐ 문장은 "
                "RETRACTION_LOG R14 가 무효화했다(2026-08-03, φ 0~355° 5° 간격 72 점 실측) — "
                "그 수는 φ 가 아니라 스윕하지 않은 고도차 Δz = 35 m 의 성질이고 단일 d 칸 값이다. "
                "φ 스윕에서 R90 span 은 0.48 % 이고 φ=90° 가 세 팔 모두 최소였다. 남는 것은 "
                "«지금까지의 결과가 φ=90° 단일 방위였다» 는 보고 범위뿐이지 위험의 크기가 아니다.")
    n3 = dict(
    ...
    traps = [
        ...
        dict(id="T1b", name="⭐ 헤드라인 장면방위 φ=90° 하나로만 재기",
             bite=f"φ=90° 는 베이스라인의 수직이등분선이라 R₁≈R₂ 가 구조적으로 성립한다. "
                  ...
                  f"뿐이다. ⛔**전 판이 여기 적은 «φ 를 쓸면 최대 "
                  f"{norm['N3_equal_scene_range']['absmax_over_phi_db']:.3g} dB 로 벌어진다» 는 "
                  f"RETRACTION_LOG R14 가 무효화했다**(2026-08-03, φ 0~355° 72 점 실측) — "
                  f"그 수는 φ 가 아니라 **스윕하지 않은 고도차 Δz = 35 m** 의 성질이고 "
                  f"단일 d 칸 값이다. φ 의존은 사실상 없다(R90 span 0.48 % · φ=90° 가 세 팔 "
                  f"모두 **최소**). 남는 것은 «한 컷만 보고했다» 는 보고 범위다",
             fix="기하 비교의 φ 의존은 사실상 없다(R14) — 남는 규율은 «어느 φ 에서 잰 "
                 "값인지 적는다» 뿐이다. 한 φ 의 값을 격자 결론으로 쓰지 않는다",
             bias_direction="⛔«φ=90° 만 쓰면 200배 넘게 과소평가한다» 는 R14 가 뒤집었다 — "
                            "φ=90° 는 과소평가가 아니라 세 팔 모두의 **최소**이고, R90 은 "
                            "φ 전체에서 0.48 % 밖에 안 움직인다"),
