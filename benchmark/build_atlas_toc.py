...
⭐**이름은 `A_` 로 시작한다** — `src/build_volumes.py` 는 `reports/_parts/` 를 `^\\d{2}_.*\\.ipynb$`
  로만 훑고 권 편성은 `VOLUMES` 상수가 정본이라, 권 번호 체계 밖의 부록인 이 파일은 어느 색인에도
  안 걸린다(`src/make_legacy_stubs.py` 의 `existing()` 도 두 자리 숫자 앞머리로만 찾는다).
...
def build() -> NB:
    ...
    nb.md(
        ...
        "",
        "이 부록은 **권 번호 체계 밖**이다 — 결론을 주장하는 문서가 아니라 원장을 "
        "찾아보는 문서라, 파일 이름이 `A_` 로 시작하고 리포트 색인에 안 걸린다. "
