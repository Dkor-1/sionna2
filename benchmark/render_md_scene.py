...
무엇을 그리나 — 세 엔진 비교(그림 3)의 배치 그대로
    ...
    TX = RX   거리 3 m · az 0° · el −15° · **기선 0(BASE=0.0, 48 행) = 진짜 모노스태틱**
              송신점과 수신점이 한 점이라 렌더에도 공이 하나만 뜬다

세 컷
    f0a  옆에서 본 전체 — TX/RX 공과 드론이 한 화면에 (기하가 한눈에)
    f0b  TX/RX 쪽에서 드론을 본 근접 — 레이더가 보는 배 쪽 모습
    f0c  같은 씬에 PathSolver 경로를 겹쳐 그린 컷 (자세 1 · 시드 1 · 광선 100 만 발)

⚠ 경로 수는 **원장 없음** — 101 행이 stdout 으로만 적고 `json.dump` 가 없다. 리포트
  08_1 은 그 수를 «원장 없음» 으로 표시하고 비율에서 뺀다(비율의 분모는 원장
  `report07_three_engine_ranges.json:ranges.R3.paths_median`).
