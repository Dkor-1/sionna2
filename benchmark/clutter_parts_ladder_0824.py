...
8/18 덱 23 번 «The drowned blades» 가 낸 결론은 이랬다:
  ...
  · 프로펠러 **단독**은 통짜 정면보다 박자 구조가 뚜렷하다.
    ⛔**리듬 몫의 크기는 인용하지 않는다**(RETRACTION_LOG R29) — 창 반폭 hw 를 2/8/32 Hz 로만 옮겨도 같은 데이터가 9.9/63.4/90.0 % 로 움직이고, `f_above` 를 함수 기본값(1.5·f_flash)으로 두면 63.4 % 가 98.9 % 가 된다. **순서만** 읽는다. 그리고 «백색잡음 13 %» 도 측정이 아니라 빗살폭÷빗살간격
    (2×8/126.67 = 12.6 %)이라는 **기하 바닥**이다.
...
def cs_eca(x, fcut=FCUT):
    ...
    실측(el 0, matrice4e, 8,192 자세, PRF 19.7 kHz):
      ...
      · 날개 박자 126.7 Hz 는 노치 가장자리보다 26.7 Hz 위 — **박자 빗살 칸은 하나도 안 지워진다**
        ⛔«신호 전체가 보존된다» 는 뜻이 **아니다**(2026-09-10 재측정). 노치가 지우는 83 칸
        안에 들어 있어 함께 사라지는 몫이 통짜 팔 AC 에너지의 el 0 에서 0.04~0.94 %,
        el −30·−60 까지 넓혀도 최대 1.16 % 다(AC 절대전력은 0.05 dB 안에서 움직인다).
        정확히 0.0000 % 인 것은 **빗살 칸 손실**이다.
        ⭐그래도 이 사다리가 내는 rhythm_pct 는 **안 바뀐다** — el +0 의 raw→eca 가
          63.36→63.36 · 13.08→13.08 · 14.27→14.27 · 14.02→14.02 · 89.72→89.72 ·
          12.69→12.69 이고 움직이는 것은 body only 13.42→12.86 하나뿐이다.
        ⚠el −90 은 f_tip 이 무너져 **빗살 잣대 자체가 성립하지 않는다** — 손실 %를 여기
          적지 않는다. 이 파일이 그 칸을 metric_valid=false 로 비우는 것과 같은 이유다(:201).
...
def main() -> int:
    ...
    doc = {"_meta": {
        ...
        #: ⛔리듬 몫 크기는 R29 가 내렸다 — 손잡이 없는 양(요동 진폭비)으로 바꾼다.
    "why_ko": "8/18 덱 23 번: 프로펠러 단독은 박자를 보이고 통짜 정면은 안 보인다 — "
              "el 0 에서 프로펠러 반향이 동체보다 한참 아래다. ⛔리듬 몫 크기(90 %)는 "
              "RETRACTION_LOG R29 로 철회 — 창 반폭 hw·f_above 를 탄다. 크기가 필요하면 "
              "손잡이 없는 요동 진폭비를 쓴다",
        "gpu_used": False,
