...
NEW_ORDER = [
    dict(no="01", title="선행연구 — 무엇이 가능하고 무엇이 아닌가", change="변동 없음",
         why="보유 아카이브(고유 218편 — docs/REFERENCE_LIBRARY.md 의 전수 집계) 안에서는 "
             "드론 메쉬를 PO 표면전류 적분에 통과시켜 산란을 계산한 저작을 찾지 못했다 — "
             "⛔«게재본 중 전례가 0편» 이라고는 쓰지 않는다. 그 범위를 먼저 확정해야, "
             "뒤 편들이 무엇을 새로 하는지가 정의된다."),
...
A("")
#: ⛔`rate_k` 는 sweep 로그가 없으면 None 이다(위 86~93 행). 그때 이 문장을 그대로
#  찍으면 TypeError 로 죽는다 — 원장은 이미 저장된 뒤라 **JSON 은 새 것인데 md 는 옛
#  것**으로 갈리는 자리였다. 없으면 없다고 적는다.
A("카드 0·1 은 타 사용자가 점유 중이라 2·3 만 썼다. 광선율 모델은 sweep 로그의 "
  "(rays/az, 초) 쌍에 원점 통과 최소제곱을 세운 것이다 — "
  + (f"k = {rate_k:.3e} s/(ray·az)." if rate_k is not None
     else "⛔이번 판에서는 sweep 로그가 없어 k 를 세우지 못했다."))
A("")
A("### §5.1 두 번째 기체는 얼마인가")
A("")
A("| 항목 | 값 |")
