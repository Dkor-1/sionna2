...
상당 부분 뒷받침한다**. 절대 세기(RCS)만 놓고 보면 부피를 맞춘 구가
우리 정밀 메쉬보다 레벨이 가깝다 — ⚠**«모수 0개» 가 아니다**(2026-09-04) — **구의 부피를 무엇으로 잡는가가 자유 매개변수**다. +0.96 dB 는 «논문표기 상자와 같은 부피» 판이고, **메쉬 부피와 같은 구는 −4.47 dB 로 우리(−3.30)보다 나쁘다**(편 27). 부피 규약을 바꾸면 결론이 뒤집히므로 «구가 이긴다» 를 그 자체로 인용하지 않는다.
그 숫자는 손으로 적지 않고 outputs/p3_validation_v2.json 에서
...
def rcs_ladder_context():
    ...
    return dict(
        ...
        rows=rows,
        reading_ko=("세기(RCS)만 놓고 보면 부피를 맞춘 구가 우리 메쉬보다 레벨이 가깝다"
