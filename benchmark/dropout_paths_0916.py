...
ANCHORS = [
    ...
    (792, "spp = int(a.spp) if a.spp else rule_spp(rng_m)"),
    (802, 'm = _re.fullmatch(r"R([01])D([01])E([01])F([01])", swbits)'),
    (822, 'mdep = int(a.max_depth) if getattr(a, "max_depth", 0) else 1'),
    (834, "cols = drone_colors(spec)"),
    (953, "_solver = RP.rt.PathSolver()"),
    (961, "_lay = InMemGroups(_mv0.f, _mv0.g)"),
    (975, "_lay.update_vertices(RP.mi, _par_cache[g], mv.v, g)"),
    (1016, "sc, _ctr, _scene_obj_names = build_scene_builtin("),
    (1021, "sc = RP.build_scene(parts, fc=fc)"),
    (1050, "RP.place(sc, center=_ctr, az=az, el=el, rng=rng_m, baseline=0.0,"),
    (1057, "samples_per_src=spp, max_num_paths_per_src=RP.MAX_PATHS, seed=1)"),
    (1059, "aa, tau, _, O = RP.unpack(p, want_doppler=False)"),
    (1087, "_id2nm = {int(_o.object_id): str(_nm)"),
    (1124, "hit = (O != RP.NO_OBJ).any(axis=0) if O.size else np.zeros(aa.size, bool)"),
    (1125, "_t = aa[hit] * np.exp(-1j * 2 * np.pi * fc * tau[hit])"),
    (1145, "E[j] = complex(np.sum(_t_sum))"),
]
