...
FTIP0 = float(ROW[("ours", 0.0)]["f_tip_hz"])            # el 0 의 f_tip = 1272.9 Hz
    ...
    for k, el in enumerate(ELS):
        ...
        for eng in show:
            ...
            a.plot(FR, 10 * np.log10(P / dc), lw=0.7, color=ARM_COL[eng],
                   alpha=0.9 if eng == "ours" else 0.75)
        ft = float(ROW[("ours", el)]["f_tip_hz"])
        for s in (+1, -1):
...
def fig3():
    ...
    _elmap(ax[0, 1], "sionna_p250000000")
    ...
    for i, el in enumerate(ELS[:-1]):                 # −90 은 f_tip = 0 이라 못 나눈다
        ft = float(ROW[("ours", el)]["f_tip_hz"])
        c = cmap(i / 5.0)
        for eng, ls, lw, al in (("ours", "-", 2.0, 0.95),
                                ("sionna_p250000000", ":", 1.4, 0.75)):
    ...
    style = {"ours": dict(marker="o", ms=11, mfc="#c62828", lw=2.4, zorder=5),
             "sionna": dict(marker="^", ms=11, mfc="none", mew=1.8, lw=1.4, zorder=3),
             "sionna_p250000000": dict(marker="s", ms=8, mfc="none", mew=1.8,
                                       lw=1.4, zorder=4)}
...
def fig4():
    fig, ax = plt.subplots(1, 2, figsize=(14.6, 6.2), sharey=True)
    for a, band, ttl in (
        ...
        if band == "fixed":
            ...
            base = DER["cells"]["ours/el-15"]["share_fixed_db"]
            xs, ys = [], []
            for el in ELS:
...
def fig5():
    ...
    a = ax[1]
    for eng in ("sionna", "sionna_p250000000"):
    ...
    fig.suptitle("CH1-F5   Ray budget versus flash-rate accuracy", fontsize=14, y=0.99)
...
def judge():
    ...
    for el in ELS:
        ...
        if fr_ <= 0:
            continue
        err[f"{el:+.0f}"] = round(DER["cells"][f"ours/el{el:+.0f}"]["share_fixed_db"]
                                  - (base + 10 * np.log10(fr_)), 1)
    g["G3_ours_fixed_minus_prediction_db"] = err
    g["G3_worst_db"] = max(abs(v) for v in err.values())
    # G4 — −63.8° 아래에서 박자가 무너지나
    g["G4_ours_beat_track_at_-75"] = DER["cells"]["ours/el-75"]["beat_track_hz"]
    g["G4_ours_beat_fixed_at_-90"] = DER["cells"]["ours/el-90"]["beat_fixed_hz"]
    g["G4_leak_ratio_at_-90_db"] = V["B_decomposition"]["ours/el-90"][
        "fixed_band_true_over_leakage_db"]
    ...
    g["G6_ours_track_share_span_db"] = round(
        max(DER["cells"][f"ours/el{el:+.0f}"]["share_track_db"] for el in ELS[:-1])
        - min(DER["cells"][f"ours/el{el:+.0f}"]["share_track_db"] for el in ELS[:-1]), 1)
    DER["gates"] = g
    return g
...
if __name__ == "__main__":
    ...
    p = f"{ROOT}/outputs/ch1_elevation_figdata.json"
