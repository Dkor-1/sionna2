# -*- coding: utf-8 -*-
"""
render_mini2_mesh.py — mini2 메쉬 온전성 렌더 (2026-08-03)
============================================================

무엇을 하나
  새로 등록한 `mini2`(DJI Mini 2) 메쉬를 네 방향 직교투영으로 그려
  `outputs/figs/mini2_mesh.png` 에 저장한다. 색은 저장소 규약대로 **순수 재질 5색**이고,
  그림 안 글자는 전부 영어다(주석·print 는 한국어).

왜 별도 스크립트인가
  `src/viz_mesh_gallery.py` 는 전 기종 원장(`outputs/mesh_gallery.json`)과 그림을 통째로
  다시 쓴다. 지금 다른 워크플로가 같은 원장을 만지고 있어서 **이 기체 한 장만** 새 경로에 낸다.
  렌더러 자체는 그 모듈 것을 그대로 재사용한다(음영·직교카메라 규약 동일).
...
#  DJI 공식 CAD(GLB, WM161) 실측 — 프롭 제외 바깥치수와 모터 대각. 그림에 비교로 적는다.
CAD_BBOX_MM = (159.1, 203.4, 56.0)
CAD_DIAG_MM = 213.05
...
def main():
    ...
    fig.suptitle("DJI Mini 2  —  mesh (colour = pure material)", fontsize=14, y=0.988)
    sub = (f"triangles {len(F):,}  (frame {len(frame.f):,} + 4 propellers)   "
           f"frame bbox {ext[0]:.1f} x {ext[1]:.1f} x {ext[2]:.1f} mm  "
           f"vs published 159 x 203 x 56 (props excl.)\n"
           f"same box measured on DJI's own CAD "
           f"{CAD_BBOX_MM[0]:.1f} x {CAD_BBOX_MM[1]:.1f} x {CAD_BBOX_MM[2]:.1f} mm   "
           f"with propellers {extd[0]:.0f} x {extd[1]:.0f} x {extd[2]:.0f} mm\n"
           f"wheelbase {env['wheelbase_opposite_mm']:.2f} mm vs published 213.0 "
           f"(CAD {CAD_DIAG_MM:.2f})   envelope fit scale "
           f"{tuple(round(s, 4) for s in env['fit_scale'])}\n"
           f"frame z-range {Vf[:, 2].min() * 1000:+.1f} .. {Vf[:, 2].max() * 1000:+.1f} mm "
           f"(z = 0 is the arm axis at the motor station)")
    fig.text(0.5, 0.963, sub, ha="center", va="top", fontsize=8.6, color="#333")
    fig.subplots_adjust(top=0.872, bottom=0.062, left=0.02, right=0.98, hspace=0.02, wspace=0.02)
    fig.savefig(OUTPNG, dpi=170)
    plt.close(fig)
    print(f"[mini2] 렌더 저장: {OUTPNG}")
