...
■ ⛔**2026-09-08 설계 검토 ⑯ — 손잡이와 원인은 일대일이 아니다.**
  기체 교체는 메쉬뿐 아니라 치수·재질·회전수를 함께 바꾸고, 크기 변경은 가림뿐 아니라
  경로 후보와 위상을 바꾼다. 광선 예산도 후보 탐색과 수치 처리에 함께 작용한다.
  ⇒ 아래 물음은 «이 손잡이를 바꾸면 사건과 경로 목록이 어떻게 변하나» 를 **기록하는**
    것이지, 「기체를 바꿔도 나면 메쉬 탓이 아니다」처럼 원인을 확정하지 않는다.
  ⚠더 작은 대조가 필요하다 — 회전 고정 · 드론 제거 · 드론과 안 만나는 지면 경로 ·
    같은 메쉬의 위치/재질만 교체.

■ 이 발주가 살피려는 것 (묶음마다 물음 하나)
  A **기체를 바꾸면 사건이 어떻게 변하나** — phantom4 는 «되풀이된 도달» 이 0 % 인 기체다.
    ⚠**읽기 전에 대역을 확인한다** — `comb_snr.f_tip` 이 프롭 지름을 안 써서 기체별 대역이
      틀렸던 것을 2026-09-08 에 고쳤다(mini5pro 920.57 → 512.37 Hz). 옛 값과 섞지 마라.
  ...
  D **드론 크기를 바꾸면 어떻게 변하나** — 동체를 반으로/두 배로.
    ⚠크기는 가림뿐 아니라 경로 후보와 위상도 바꾼다 — 따라간다고 «가림» 으로 읽지 않는다.
