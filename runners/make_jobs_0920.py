...
⛔발주서를 손으로 쓰지 않는다. 이 생성기가 원장(work/wf/queue_design_0920_result.json)을 읽어
...
쓰는 법:
    /workspace/.venvs/py312/bin/python runners/make_jobs_0920.py --summary
    /workspace/.venvs/py312/bin/python runners/make_jobs_0920.py > runners/jobs_0920.txt
"""
import json
import os
import sys
...
SRC = f"{ROOT}/work/wf/queue_design_0920_result.json"
NSH = 2

with open(SRC, encoding="utf-8") as f:
    R = json.load(f)

#: 두 합침 안 가운데 **줄이 많은 쪽**을 고른다 — 이번 판은 둘 다 100 일꾼시간 언저리라
#:   덮는 물음이 더 많은 쪽이 낫다(널의 폭 · 손잡이가 앙각을 타나 · 남의 씬에서도 갈리나).
...
OUT = []
OUT.append("# ══ 0920 큐 — 2026-09-09 설계판 (0918 을 읽고 그 위에서) ══")
