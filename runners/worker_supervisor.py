...
#   그 줄서기가 자세당 시간을 **약 23 배** 부풀렸다(한산한 상자 267 ms ↔ 프로필 6,095 ms
#   = 22.8 배).
#   ⛔2026-09-06 정정 — 여기 적혀 있던 «50~70 배» 를 내린다. 같은 문장이 근거로 든 두 수
#     (267 ms ↔ 6,095 ms)가 22.8 배라 배수와 어긋났고, 50~70 이 나온 측정의 원장을 레포
#     안에서 찾지 못했다. 다른 측정에서 나온 값이면 그 측정의 두 수를 함께 적고 되살린다.
...
#   inactive 1.6), 진짜 anon 은 5.9 GiB 뿐이었는데 감독자는 여유를 5.9 로 읽어
#   문턱 6 에 딱 걸렸다. 그 사이 **GPU 1 은 0 % 로 완전히 놀았다.**
#   ⛔캐시까지 전부 여유로 치는 쪽(active_file 도 빼기)은 **안 한다** — OOM 으로
#     큐가 통째로 죽은 적이 있다. 문턱만 낮춰 여유분 2 GiB 를 회수한다.
...
class Sup:
    ...
    def launch(self, gpu, line) -> bool:
        ...
        #     12 코어를 두고 싸워 자세당 시간이 약 23 배로 부풀었다(267 ms ↔ 6,095 ms;
        #     ⛔«50~70 배» 는 2026-09-06 에 내렸다 — 55 행 참조). 랩 서버 과부하의 원인.
