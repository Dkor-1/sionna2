...
sec("C 남의 씬 × 기본 아닌 기체 — 합성 배율 축과 실제 기체 축이 같은 답을 주나",
    ...
    #: ⛔⛔2026-09-10 정정 — 옛 죽는조건은 «널 띠 안이면 기체 교체가 목록을 안 옮긴다» 였다.
    #  그 띠(0.60~0.77)는 **같은 물체를 두고 광선 격자를 다시 썬 수치 민감도** 이고,
    #  판독기 자신이 「두 점은 신뢰구간이 아니다」(read_canyonnull_0910.py:186)·「자세는 시각이고
    #  로터 위상 구조가 있다 — 유의성 검정이 아니다」(:20-21)로 못 박아 두었다.
    #  게다가 두 앙각의 관측 구간(el−30 0.6031~0.6250 · el−60 0.7315~0.7740)은 서로 안 겹쳐
    #  «띠» 로 합치면 어디서도 관측된 적 없는 구간이 생긴다. ⇒ 물리적 변경(다른 기체)의
    #  동등성 문턱으로 쓰지 않는다. 아래 «안답함» 과도 충돌하던 것을 함께 푼다.
    "죽는조건  이 칸은 **레벨과 빈도만** 읽는다 — mini5pro 의 사건 수·레벨이 matrice4e 와",
    "        같은 자리에 서는지 본다. 자카드는 적되 «크다/작다» 로 판정하지 않는다.",
    "⛔안답함  ⚠**격자 민감도 띠를 기체 교체의 문턱으로 쓰지 않는다.** 0922 A 가 산 값은",
    "        «격자를 갈았을 때의 민감도» 이지 널이 아니다(read_canyonnull_0910.py:186).",
    "        앙각마다 따로 읽고, 두 앙각을 한 띠로 합치지 않는다.",
