...
처음이면 [리포트 1 «이 연구가 묻는 것과 답한 방식»](01_map.ipynb) 부터다 — 열일곱 권의 지도와 읽는 경로 셋이 거기 있다.
...
| [5 «우리 커널 — 무엇이고, 무엇이 아닌가»](05_kernel.ipynb) | SBR + 물리광학이 무엇을 계산하고 무엇을 계산하지 않는지를 정의하고, 해석해가 있는 과녁(구·평판·이면각)… | 6 | [절 4 «해석 PO 구 대비 구현오차는 kr 전 구간에서 λ/16 격자 0.201 dB · 생산 λ/12 격자 0.254 dB 안이다»](05_kernel.ipynb) |
| [6 «σ 를 무엇에 붙들어 매나»](06_anchor.ipynb) | 우리 σ 의 절대 레벨을 붙드는 것은 공개 문헌 한 기체·한 실험실뿐이다. 그 끈의 장력을 재고, 끊어질 자리… | 6 | [절 1 «σ = A(f)·B₁·B₂ 에서 A(f) 의 기울기만 측정에서 받고, 레벨과 각패턴은 우리 SBR+PO 커널(B) 출력이다»](06_anchor.ipynb) |
...
| [11 «처리 사슬 — 직접파를 죽이고 표적을 세운다»](11_detector.ipynb) | ECA 로 직접파를 지우고 CFAR 로 문턱을 세운다. 문턱을 어디에 두느냐가 결과를 정하므로 그 교정을 먼저… | 4 | [절 3 «운용 형상에서 경험 Pfa 를 재니 명목값의 1.52~2.66 배였다»](11_detector.ipynb) |
| [12 «관측가능성과 기하 — 어디에 서야 보이나»](12_observability.ipynb) | 송신기·수신기·표적의 배치가 검출을 정한다. 볼 수 없는 자리를 먼저 지도로 그리고, 그 다음에 거리를 말한다. | 4 | [절 1 «한 순간의 (R_b, f_d) 는 랭크 2 이고, 수신기를 하나 더하면 위치가 풀린다»](12_observability.ipynb) |
| [13 «결과 — 얼마나 멀리서 보이나»](13_results.ipynb) | R90 과 순위가 이 연구의 정량 결론이다. 적분시간·잔류·σ 가정을 흔들어 순위가 견디는지까지 함께 적는다. | 5 | [절 2 «앵커 σ 위의 R90 은 비교가능 12칸에서 3.69~7.44 km 이고, 밴드 순서는 기체마다 바뀐다»](13_results.ipynb) |
...
| [16 «앙각 커버리지 — 어느 각도까지 유효한가»](16_elevation-coverage.ipynb) | 관측 앙각을 0° 에서 −90° 까지 내리며 같은 표적을 재면, 커버리지를 정하는 것은 표적이 아니라 우리가… | 5 | [절 2 «−75° 에서 추적 대역 몫은 고정 대역보다 38.55 dB 크고, 그 차이를 만든 것은 대역을 어디에 놓았는가 하나다»](16_elevation-coverage.ipynb) |
| [17 «엔진의 물리 스위치 — 켜면 무엇이 달라지나»](17_engine-physics.ipynb) | 스톡 PathSolver 의 굴절·회절·모서리회절·다중반사를 하나씩 켜서 무엇이 결과를 만들었는지 귀속한다… | 5 | [절 1 «나딧에서 레벨을 −130.78 dB 에서 −64.23 dB 로 올리는 스위치는 회절 하나다»](17_engine-physics.ipynb) |

셀 977 개 · 각주 2154 개 · 그림 82 개.
...
### [1 «이 연구가 묻는 것과 답한 방식»](01_map.ipynb)

| 절 | 제목 | 조각 |
|---|---|---|
| 1 | 열일곱 권의 지도 — 무엇이 어디에 있나 | (이 스크립트가 생성) |
...
### [2 «스톡 Sionna 로는 왜 부족한가»](02_stock-engine.ipynb)
...
### [3 «선행연구는 어디까지 왔고 우리는 어디 서는가»](03_prior-work.ipynb)
...
### [4 «표적을 짓는다 — 메쉬와 재질»](04_target-mesh.ipynb)
...
### [5 «우리 커널 — 무엇이고, 무엇이 아닌가»](05_kernel.ipynb)
...
| 편 | 무엇에 답하나 |
|---|---|
| [08_1_scene.ipynb](08_1_scene.ipynb) | 무엇을 보고 있나 — 시나리오와 신호의 정체 |
...
### [9 «마이크로도플러 — 무엇이 그 무늬를 흐리나»](09_microdoppler-limits.ipynb)
...
### [12 «관측가능성과 기하 — 어디에 서야 보이나»](12_observability.ipynb)
...
### [13 «결과 — 얼마나 멀리서 보이나»](13_results.ipynb)
...
### [14 «결론이 무엇에 기대고 있나 — 강건성과 하드웨어»](14_robustness.ipynb)
...
### [15 «실측 계획 — 무엇을 재야 이 문서가 닫히나»](15_measurement.ipynb)
...
### [16 «앙각 커버리지 — 어느 각도까지 유효한가»](16_elevation-coverage.ipynb)

| 절 | 제목 | 조각 |
|---|---|---|
| 1 | 앙각 7 점을 15 m 한 자리에서 광선 40 억 발로 재고, 52 행이 모두 완결이다 | `_parts/78_el-sweep-design.ipynb` |
| 2 | −75° 에서 추적 대역 몫은 고정 대역보다 38.55 dB 크고, 그 차이를 만든 것은 대역을 어디에 놓았는가 하나다 | `_parts/79_el-band-tracking.ipynb` |
| 3 | 물리 상한 위 누설은 우리 팔 0.22~17.18 %, 스톡 PathSolver 0.81~96.17 % 이고, 물리를 켜면 여섯 앙각이 전부 78 % 위다 | `_parts/80_el-above-tip-limit.ipynb` |
...
### [17 «엔진의 물리 스위치 — 켜면 무엇이 달라지나»](17_engine-physics.ipynb)
