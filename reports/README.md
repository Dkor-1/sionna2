...
처음이면 [리포트 1 «이 연구가 묻는 것과 답한 방식»](01_map.ipynb) 부터다 — 열한 권의 지도와 읽는 경로 셋이 거기 있다.
...
별편은 **부모 권 물음의 심화·지원·변주**다 — 번호가 «부모번호-K» 이고, 부모를 읽은 뒤에 여는 글이다. 6 권의 다섯 편은 이것과 달리 그림 무게로 나눈 **분권**(한 권의 장)이다. 6-6 만 파일 이름이 분권 꼬리를 잇고 지위는 별편이다.
...
| 절 | 제목 | 조각 |
|---|---|---|
| 1 | 열한 권의 지도 — 무엇이 어디에 있나 | (이 스크립트가 생성) |
...
| 절 | 제목 | 조각 |
|---|---|---|
| 1 | 앙각 7 점을 15 m 한 자리에서 광선 40 억 발로 재고, 58 행이 모두 완결이다 | `_parts/78_el-sweep-design.ipynb` |
...
- [8-2 «기준채널이 현실이면 얼마를 잃는가»](08_2_two_channel.ipynb) (빌더 `src/make_report11_2_two_channel.py`) — 8 권의 사슬을 한 줄도 안 고치고, 기준신호를 «송신 파형 그대로» 에서 «잡음·다중경로가 섞인 측정 신호» 로 바꿔 손실을 잰다.
