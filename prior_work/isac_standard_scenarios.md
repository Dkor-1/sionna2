...
> · **TR 38.765 §6.3 에서 목표를 못 맞춘 항목은 대부분 P_md·오경보 Type 2 였다** — 속도 정확도만으로
>   떨어진 사례가 Case 1-1 에 1 건 있고, 클러터를 켠 §6.3.1 사례는 수평 **11.1 m** 로 정확도도 목표를
>   넘겼다. 통과 사례의 수평/수직 정확도는 1~4 m(목표 10 m)였고 **실패 사례의 정확도는 §6.3 에
>   보고되지 않았다**. 그리고 130 건 평가에 **바이스태틱은 0 건**이다.
>   (⛔ 2026-09-06 정정 — 옛 판 «정확도는 어디서나 1~4 m … 떨어진 건 **전부** P_md·P_fa Type 2» 는
>   근거가 통과 사례 표뿐이라 §⑫-3 에서 내렸다.)
...
⭐ **읽어야 할 것**: 읽어 낸 두 대안은 **둘 다 강한 LOS** 를 준다. **B.1.1** 은 LOS 에서
K 20 dB · 각도확산 0.1~1° · 지연확산 10 ns(NLOS 는 K 10 dB · 30 ns)이고, Rel-20 채택본 **B.1.3** 은
**스톡 UMa 에 K = 15 dB 만 얹은 것**이라 각도·지연 확산은 스톡 UMa 그대로다. **B.1.2 는 못 읽었다.**
따라서 «세 대안 공통의 확산값» 은 이 표에서 나오지 않는다 — 위 표의 칸별로만 읽는다.

⛔ **정정(2026-09-06).** 옛 판은 이 자리에 «**어느 대안을 골라도** 공중링크는 거의 단일 광선이다.
K 15~20 dB, 각도확산 0.1~1°, 지연확산 10~30 ns … 다리는 **사실상 자유공간에 가깝다**» 고 적었다.
(ⅰ) 값을 못 읽은 B.1.2 까지 포함한 전칭 판정이고, (ⅱ) 채택되지 않은 B.1.1 의 확산값을 세 대안
공통값처럼 붙였으며, (ⅲ) 채택본 B.1.3 의 확산은 스톡 UMa 라 0.1~1° 가 아니다 — 그래서 내렸다.
바로 아래 K = 15 dB 계산은 **채택본 B.1.3 의 K** 만 쓰므로 그대로 유효하다.
...
| el < −57° — 행 평균이 클램프행보다 밝은 구간(⛔[정정 2026-09-05] «전 기체·전 밴드» 는 과했다 — 147칸 중 139칸이고, −90° 행에 −11.29 dB 칸이 있다) | 15.5 % |
...
⛔ **철회(2026-09-06)** — 이 자리의 옛 한 줄은 «**정확도는 어디서도 문제가 아니다** — 통과·실패를
가리지 않고 1~4 m 로 목표 10 m 를 크게 밑돈다» 였다. 위 표가 적은 1~4 m 는 **통과 사례뿐**이고
실패 17 건의 정확도는 §6.3 에 보고돼 있지 않으며, 바로 아래 §6.3.1(클러터 켬) 사례는 수평
11.1 m 로 목표를 넘긴다. 전칭(«어디서도»·«통과·실패를 가리지 않고»)이 근거보다 세서 내렸다.

⭐ 통과 사례의 수평/수직 정확도는 **1~4 m** 로 목표 10 m 에 여유가 있었다(목표값 원장:
`outputs/isac_standard_gaps.json` → `tr38765_clause9.performance_objectives`
= `horizontal_accuracy_m_at_90pct` 10.0 · `vertical_accuracy_m_at_90pct` 10.0).
**실패 17 건의 정확도는 §6.3 에 보고돼 있지 않다.** 다만 클러터를 켠 §6.3.1 사례는 수평
**11.1 m** 로 목표를 넘겼다. Case 1-4 에서 목표를 못 맞춘 항목으로 보고된 것은 **P_md 와
오경보 Type 2** 다.
