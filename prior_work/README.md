...
- **Crossref**(ISSN·DOI 앞자리 · 제목) + **OpenAlex**(초록까지) 두 채널 — 학술지 25 종 +
  **학회 11 종**, 2025-01 이후 — ⚠**275 편은 그때(2026-09-11)의 수다.** 지금 원장은
  아래 「지금 원장의 수」를 본다.
- 탑티어 기준은 `dl_toptier_anchors.md` 의 것을 따르고 주제 학술지·학회를 얹었다(까닭은 원장에)
- ⭐**네 판을 거쳤다**(⚠**그 판들의 자기감사 수치**다 — 지금 수는 아래 표를 본다):
...
- ⛔**아직 안 된 것**(⚠그때 기준) — 재현율은 **셀 수 있는 닻 4 개 중 4 개**이고
  «4/6» 은 셀 수 없는 닻 둘을 분모에 넣은 옛 표기다(아래 표 참조) ·
...
### 지금 원장의 수 (⛔손으로 쓰지 않는다 — `toptier_0911.json` 의 키에서 읽었다)

| 항목 | 지금 | 어디서 |
|---|---:|---|
| 편 수 | **970** | `_meta.n_rows` · `rows` 길이 |
| 정본 / 일반 | 620 / 350 | `rows[].tier` |
| 초록이 있는 편 | **802** | `rows[].abstract` 가 빈 문자열이 아닌 것 |
| 구간 | 2025-01-01 이후 | `_meta.since` |
| 재현율 | **4/4** | `_meta.recall` |

⛔**재현율의 분모를 섞지 않는다.** 닻은 모두 **6** 개인데, 그중
DOI 를 알고 조사 구간 안에 있어 **셀 수 있는 것이 4** 개다. 4/4 은
그 셀 수 있는 분모에서 나온 수이고, 4/6 로 적으면 셀 수 없는 닻
둘(구간 밖 1 · DOI 미상 1)을 **못 찾은 것처럼** 읽힌다.
`_meta.recall.how_ko`: 「ANCHORS 의 **DOI 로만** 원장을 찾는다. 구간 밖이거나 DOI 를 모르는 닻은 분모에서 뺀다.」

⚠**읽은 범위**: 초록까지다. 논문 본문을 다 읽은 조사가 아니다.
