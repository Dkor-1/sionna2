...
CAUTION_SCALAR = ("**스칼라 RCS 만으론 위상이 없다.** 일반 RCS $\\sigma_b$ 는 전력 크기라, coherent 센싱(Range-Doppler·"
                  ...
                  "보강/상쇄 간섭을 보존한다. **⭐ 우리 SBR+PO 는 스칼라 RCS 가 아니라 복소장 $E$ 를 직접 낸다.** "
                  "LAMBDA 논문은 RCS **크기** 결합만 서술하고 위상·편파 처리는 명시하지 않는다(이 문서 A1 항목의 "
                  "«복소 위상·편파행렬·rotor micro-Doppler 까지인지는 불명확»). 논문 서술이 크기 중심인 것과 "
                  "CADFEKO 라는 도구의 능력은 다른 문제라 **우열 판정은 보류한다.** "
                  "⛔ 옛 판은 여기서 «LAMBDA(CADFEKO 크기 중심 설명)보다 앞선다» 고 단정했다 — "
                  "상대 도구의 능력을 확인하지 않은 우열 판정이라 2026-09-06 에 내렸다.")
...
OUR_POSITION = ("우리(sionna2)는 **Sionna 기반 site-specific passive 채널** + **자작 SBR+PO 로 계산한 UAV bistatic "
                ...
                "위 §9 공백을 정면으로 겨냥한다. 특히 §6 의 복소 산란행렬 요구를 우리 SBR+PO 복소장이 충족한다. "
                "LAMBDA 논문은 RCS 크기 결합만 서술하고 위상·편파 처리는 명시하지 않는다 — 다만 CADFEKO 라는 "
                "도구가 무엇까지 낼 수 있는지는 확인한 바 없어 우열 판정은 보류한다(§6 과 같은 이유). "
                "⛔ 옛 판은 이 자리에 «LAMBDA(CADFEKO 크기 중심) 대비 차별점이다» 라고 적었다 — "
                "상대 도구의 능력을 확인하지 않은 단정이라 2026-09-06 에 내렸다.")
