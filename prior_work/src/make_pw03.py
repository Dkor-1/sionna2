...
cells = [
    ...
    md(
        ...
        "⭐ **관찰(성능 판정 아님):** $s_q$ 를 스칼라 $\\sigma_b$ 로 주면 위상을 잃지만(RD/CAF 에서 여러 반사가 "
        "복소 위상으로 합쳐지므로 손해), **복소 바이스태틱 산란행렬** "
        "$\\mathbf{S}_b=\\begin{bmatrix}S_{\\theta\\theta}&S_{\\theta\\phi}\\\\ S_{\\phi\\theta}&S_{\\phi\\phi}\\end{bmatrix}$ "
        "가 이상적이다 — **우리 SBR+PO 는 복소장 $E$ 를 내므로** 스칼라-RCS 주입(대다수 선행)이 잃는 "
        "산란 위상을 보존한다. ⛔«보다 앞선다» 는 성능 판정이라 내렸다 — 두 방식을 같은 검출 체인에 "
        "넣고 나란히 돌린 대조가 0 건이고 실측 대조도 0 건이라, 지금 말할 수 있는 것은 «위상을 "
        "보존한다» 는 구현 사실까지다.",
    ),
    md(
        "---",
    ...
    md(
        ...
        "## §4. 정직한 한 문단 (외부에 말할 때)",
        "",
        "> *\"우리는 실외 장면의 전파를 NVIDIA Sionna RT 로 담보하고(선행 Deterministic-Modeling·SimART 와 "
        ...
        "검출체인(ECA·CAF·CFAR)은 pyAPRiL(패시브 전용, GPLv3)·NIST 5GNRad·NIST ISAC-PLM 처럼 공개 "
        "구현이 있으나, 우리 조건(패시브 바이스태틱 + GPU 대량 몬테카를로)에 그대로 맞는 것이 없어 "
        "직접 구현했고 pyAPRiL 로 정합성을 검증할 계획이다.\"*",
        "",
        "⛔ **정정(2026-09-06).** 옛 판은 이 자리에 «검출체인(ECA·CAF·CFAR)은 **어느 시뮬레이터도 "
        "제공하지 않는** 연구 대상이라 직접 구현·검증했다» 고 적었다. 범위를 안 붙인 전면 부재 주장인데다, "
        "앞 편 [pw02](pw02_opensource_tools.ipynb) 가 **pyAPRiL·NIST 5GNRad·NIST ISAC-PLM** 을 "
        "검출체인 보유 도구로 ✅ 표시해 둔 것과 어긋나 내렸다. 정합성 검증도 아직 **계획** 단계다.",
        "",
        "이 문단의 각 주장은 pw01·pw02 의 해당 절에 근거를 단다 — 다만 마지막 문장의 "
        "«pyAPRiL 로 정합성 검증» 만은 계획이라 아직 근거가 붙지 않는다.",
        "",
        "⛔ **정정(2026-09-06).** 옛 판은 이 자리에 «이 문단은 **모든 주장에 선행 근거**가 붙어 "
        "있어(pw01·pw02), 과장 없이 방어된다» 고 적었다. 검증한 적 없는 자기보증이라 내렸다 — "
        "읽는 쪽이 이 한 줄 때문에 근거 확인을 건너뛴다.",
    ),
    code(
