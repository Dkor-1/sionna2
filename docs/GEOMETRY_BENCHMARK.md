...
> ⚠⚠ **2026-07-31 스냅숏이다.** 그 뒤 `RETRACTION_LOG.md` 가 아래 여러 줄을
> 무효화했다 — **R14**(φ 스윕: «최대 23.17/23.2 dB» 는 φ 가 아니라 스윕하지 않은
> 고도차 Δz = 35 m 의 성질이다. φ 의존은 사실상 없고 φ=90° 가 세 팔 모두 최소) ·
> **R11**(공통모드 기울기 0.2462 는 15 전역 적합의 평균이고 국소는 0.049~0.357 로
> 7.2 배. 순위불변성은 발견이 아니라 항등식) · **R3**. ⛔이 문서의 절대 수치를
> 인용하기 전에 그 세 항목을 먼저 읽는다.
...
├─ 재현 ─────────────────────────────────────────────────────────────────┤
│ PYTHONPATH=src:benchmark /workspace/.venvs/py312/bin/python benchmark/geometry_grid.py   (0.8 s)
│ PYTHONPATH=src:benchmark /workspace/.venvs/py312/bin/python benchmark/mono_link.py       (16.8 s)
│ PYTHONPATH=src:benchmark /workspace/.venvs/py312/bin/python benchmark/geometry_benchmark.py (1.6 s)
...
| C5 | 공통모드 σ 불변성 — dR_dB/dσ_dB = 1/4 | **both** | slope_mean 0.2536 · slope_note_ko ⚠15 전역 적합의 평균이다 — 셀별 0.223~0.256, 국소 0.049~0.357(7.2배). 4 자리 단일값은 거짓 정밀이다(R11) | ⚠**항등식이지 발견이 아니다** — 세 모드가 같은 σ/R⁴ 사슬을 쓰므로 균일 dB 오프셋은 모든 곡선을 같은 양만큼 평행이동시킨다. 국소 최대 0.357 은 이론 상한 0.25 를 넘으므로 σ 격자 고도 조회의 계단 아티팩트다(RETRACTION_LOG R11) |
| C6 | 조명원 대가 원장 — 점유 · λ² · ΔR_b · 듀티 | **both** | duty_db  · duty_pair_gaps_db -12.8/3.18/16 | ⚠ 듀티는 여전히 R90 경로에서 호출되지 않는다 — §5 결함 D2 |
| C7 | 승자 주장 — LTE CRS 가 최선의 상시 조명원 | **both (강화됨)** | ranking_same_across_placements σ 격자를 가진 7 기체 전부에서 조명원 순위가 네 배치에 걸쳐 동일하다(True). 기하를 바꿔도 조명원 축의 답이 안 바뀐다 — 두 축이 독립이라는 직접 증거다. · winner_margin_db_aspect_avg_anchored 8.311 | σ 축 문장에 '네 배치에서 순위 동일' 을 근거로 추가한다 |
...
⭐ **링크버짓은 다르다.** 기하의 링크버짓 기여는 20log10(R_조명원/R_센서) 한 줄이고 장면 방위에 따라 부호가 뒤집힌다. ⛔여기 적혀 있던 «최대 23.2 dB» 는 RETRACTION_LOG R14 가 무효화했다(2026-08-03 φ 실측 스윕 72 점) — 그 수는 φ 의 성질이 아니라 **스윕하지 않은 고도차 Δz = 35 m** 의 성질이고, 단일 d 칸 값이며 R90 동작점에서는 **≤1.20 dB**(d 중앙값 ≤3.10 dB)다. φ 의존은 사실상 없다 — R90 span 0.48 %(≤0.083 dB)이고 **φ=90° 가 세 팔 모두 최소**다. 같은 항이 검지 한계(d ≈ 14L)에서는 0.44 dB 로 죽는다 — 기하 축은 근거리에 산다.
...
**신규성 경계** — Rzewuski(NATO STO 2021)가 드론 바이스태틱 RCS → 커버리지 → 50 m 실외 검출을 이미 닫았다 — '최초' 류를 쓰지 않는다 · v_max = λ·PRF/(4cos(β/2)cosδ) 의 우선권은 **Abratkiewicz 외, IEEE JSTARS 16:3469-3484 (2023), 식 (16) p.3476** 이다 — 반구간 규약까지 같다. Chen 2024(Appl. Sci. 14:4282)는 **닫힌 식이 없다**('PRF' 0회) — 우리 이전 기록의 Chen 귀속은 철회됐다 · v·R = c·λ/8 은 Skolnik 의 고전 결과다
...
두 결함은 **둘 다 조명원 축**이다. 기하 축은 그대로 서고, 조명원 축은 WiFi ×0.487 · LTE ×0.976 · 5G ×0.396 로 움직인다 — LTE 의 우위가 커진다.

| 결함 | 무엇인가 | 영향 칸 | 기하 축 영향 | R90 배율 | 그러면 결정되는 것 |
|---|---|---|---|---|---|
| **D1** | 앵커가 배선은 됐으나 발행 σ 격자에 실체화되지 않았다 | 6칸 전부 — σ 사슬은 두 기하가 공유한다 | 0 | WiFi ×1.025 · LTE ×0.976 · 5G ×0.9997 | 자세평균 승자 여유가 2.666 dB → 8.311 dB 로 오르고, 그때 비로소 현실 오차범위 5.013 dB 밖으로 나간다 |
| **D2** | 듀티 축이 정의만 되고 R90 생산경로 밖에 있다 | 6칸 전부 — 자원격자가 정하는 값이라 기하와 무관하다 | 0 | WiFi ×0.4754 · LTE ×1 · 5G ×0.396 | 조명원 축의 LTE−5G 간격이 16.02 dB 늘고, 승자 여유가 21.15 dB 가 된다 |

**정정 지도** — 두 결함을 다 넣으면 검지거리가 이렇게 바뀐다.

| 칸 | 현재 R90 | 앵커 적용 | 앵커+듀티 | 총 배율 |
|---|---:|---:|---:|---:|
| A-wifi | 357.7 m | 366.6 m | 174.3 m | ×0.487 |
| A-lte | 383.8 m | 374.6 m | 374.6 m | ×0.976 |
| A-nr | 421.6 m | 421.5 m | 166.9 m | ×0.396 |
| B-wifi | 3512 m | 3600 m | 1711 m | ×0.487 |
| B-lte | 2261 m | 2207 m | 2207 m | ×0.976 |
| B-nr | 3263 m | 3262 m | 1292 m | ×0.396 |
| C-wifi | 5304 m | 5436 m | 2584 m | ×0.487 |
| C-lte | 6900 m | 6734 m | 6734 m | ×0.976 |
| C-nr | 5317 m | 5315 m | 2105 m | ×0.396 |
...
⭐ 기하 축을 두 배로 만든 검증은 8개 중 4개이고, 그 4개의 실제 비용은 CPU 17.47 s · GPU 0 이다. 가장 비싼 검증(σ 격자 570 s GPU · Pfa 2717 s MC)은 정확히 두 배가 되지 않는다 — σ 는 원래 모노고 Pfa 는 기하 무관이기 때문이다.
