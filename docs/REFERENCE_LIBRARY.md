...
- **우리와의 관계** — ⭐⭐ E. 우리 재질 가중 PO(GROUP_GAMMA)와 내부 배터리/PCB 모델의 가장 강한 외부 근거. 재질이 7 dB 를 가른다는 수치가 여기서 온다.
...
- **우리와의 관계** — E. Das 와 같은 기체(Phantom 3)를 같은 연구실에서 잰 것으로 보이며, 두 논문의 mu 가 우리 세 대역에서 3.23-3.59 dB 어긋난다 - 그 격차가 우리 앵커 수준의 불확실성이다.
...
| 2 | `chen2024appliedsci` | Applied Sciences 2024 - 우리 식의 성분(반복률 x cos(beta/2) x cos delta)을 기호까지 같게 쓴다. LaSen 이 우선권을 명시적으로 이 논문에 돌린다. | 식 자체의 우선권은 우리 것이 아니다. novelty 는 교차표·설계역산·검출률 귀결로 재정의해야 한다. |
...
- ⚠⚠ 식 자체의 우선권은 우리 것이 아니다. Chen 외(Appl. Sci. 14(10):4282, 2024)가 성분(반복률 x cos(beta/2) x cos delta)을 기호까지 같게 쓰고 LaSen 이 우선권을 그 논문에 명시적으로 돌린다. ⚠ 그 논문은 MDPI 403 으로 원문 미확보 상태이므로 인용 시 반드시 'LaSen 이 그렇게 요약한다' 로 표기(E4/E6).
...
- 아카이브 미독본 12편 잔존(2601.10846, 2606.07328, 2502.11540, 2507.12235, 2512.03506, 2601.03302, 2601.08042, 2407.19084, bilkent_thesis, diva_1595877, gbppr, 1910.13706).
