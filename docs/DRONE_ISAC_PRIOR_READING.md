...
> ⚠ **2026-08-10 정정 — 위 −20 dBsm 행을 «규격 값» 으로 읽으면 안 된다.**
> 원문(3gpp.org 1차자료)을 확인하니 −20 dBsm 은 **기고 단계 제안치**(R1-2403921)이고,
> **TR 38.901 V19.4.0 §7.9.2.1 이 실제로 채택한 값은 다르다**:
>
> | 규격 표적 | 외형 [m] | σ_M [dBsm] | 자세 의존 |
> |---|---|---|---|
> | UAV **small** | 0.3 × 0.4 × 0.2 | **−12.81** (σ_S 3.74 dB, 주파수 무관) | 평평(섹터 1) |
> | UAV **large** | 1.6 × 1.5 × 0.7 | **−5.85** | **6섹터 각도의존** |
>
> ⭐두 갈래가 있다는 것이 이 정정의 핵심이다 — 우리 5기체는 소형(Mini 5 Pro)부터 대형
> (S1000+)까지 걸쳐 있으므로 **한 상수로 전부 대조하면 안 된다**. 위 표의 «거친 정합» 판정
> (소·중형 3기 ±1.6 dB)은 −20 기준이라 규격 대조로는 다시 계산해야 한다.
> 근거·인용은 `prior_work/isac_standard_scenarios.md`, 기계형은 `outputs/isac_standard_scenarios.json`.
