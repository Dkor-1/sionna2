...
⭐**원장 대조를 직접 돌린 각주 107 개에서는 불일치가 0 건이었다.** 한 검증 요원의 말 그대로
「원장 대조 107 개 각주 전부 일치」다. ⚠전권 각주가 모두 몇 개인지는 이 감사가 세지 않았다 —
대조한 것은 그 107 개뿐이다.
⛔«숫자 위조나 원장 불일치는 사실상 없다» 는 내린다(2026-09-06): 아래 **D-2** 가 04·05 편에서
각주 인용값이 원장 행과 갈린 자리를 따로 잡고 있다(`rows[28].f_tip_hz` 인용 **1273** ↔ 실제
**1135.1** · 인용 **0** ↔ 실제 **304.1**).
이번에 확인된 59 건은 **대부분** **「맞게 계산한 숫자를, 그 숫자가 답할 수 없는 질문에
...
오염은 **넓지만 얕다**. ⛔**「리포트 13편 중 12편」 은 분모를 잘못 적어 내린다**(2026-09-06 재집계) — 같은 문서가 13·24·25 세 분모를 섞어 썼다. 다시 세면 이 스윕이 **§2 에 절을 낸 편이 22 편**이고, 여기에 §4 E 가 「findings 0 건」으로 적은 `01_2_prior-work` · `05_engine-physics` · `A_atlas` 셋을 더해 **연 것은 25 편**이다(§4 E 의 「24 편을 덮었다」 와도 어긋나므로 그 줄도 함께 고쳐야 한다). 편성 단위로는 **본편 13 권 + 별편 7 편**이고 `reports/` 의 `.ipynb` 는 2026-09-06 현재 **33 개**다(`docs/REPORTS_VOLUMES.md` 첫 줄 · `outputs/volumes_index.json : _meta.n_notebooks`). ⚠§2 가 절을 낸 `08_2_two_channel.ipynb` 는 2026-09-03 에 `archive/` 로 내려가 지금 `reports/` 에 없다 — 그 절의 지적은 되살릴 때만 유효하다. 흔적의 성격은 대부분 「결론의 방향은 맞고 **인쇄한 크기와 순위가 못 쓴다**」다. 재실험이 필요한 건 거의 없고 문장·표 수정이다. 결론 자체가 뒤집히는 것은 10건 남짓이며 **그중 절반이 어제·오늘 만든 `12_outdoor-scene` 한 편에 몰려 있다**.
...
> ⛔⛔ **2026-09-04 소급 — 아래 ① 의 원인 귀속과 처방은 이틀 뒤 내려졌다.**
> ⓐ 「메우면 ρ +0.974 / +0.976」 — `docs/RHO_IS_SMOOTHNESS_0902.md` 가 ⛔로 내렸다. 메우는
> 것은 **지면 반송파를 되돌리는** 일이고 ρ 는 그 «매끄러움» 을 읽을 뿐이라, ρ 는 리듬
> 검출기가 아니다. **이 두 수는 인용하지 않는다.**
> ⓑ 「기하가 아니라 **자세 인덱스에 걸린 버그**다」 — `docs/DEEP_DROP_0902.md` 가 뒤집었다.
> 낙차 45 칸 중 **42 칸이 자유공간**이고 블록 간격이 날개 박자 주기의 1.00 배라, 빗각의
> 낙차는 결함이 아니라 **날개가 지나가며 가리는 것**(재려던 신호)이다. 실외 3 칸만 원인
> 미상이다.
> ⓒ 「~780배」 — 지금 원장은 **1,388배(el −30°) · 1,279배(el −60°)** 다(리포트 12 절 3).
> ⇒ 살아남는 관측은 자세 수(el −30 : 8,192 중 74 · el −60 : 92)와 레벨 상승뿐이다.
...
`outputs/depth_axis_verdict_0816.json : bounce_ladder` → `third_over_second = 4.867`, 튄 자세 1·8 개를 빼도 4.671·4.847, `decaying_series = false`. 0825 덱 13 장이 이걸 발표했다. 리포트에서 `튕김`·`4.9 배`·`third_over_second` 전부 **0 회**. 튕김 사다리가 감쇠하지 않는다는 이 관측(`outputs/depth_axis_verdict_0816.json : bounce_ladder` — combo `R1D1E1F1` · 앙각 −30° · 거리 15 m · 자세 8,192 개 **한 칸**, `decaying_series = false`)은 원인이 아직 안 밝혀졌는데 항구 기록에 없다. ⛔**「가장 무거운」 은 내린다**(2026-09-06) — 미해결 이상들의 경중을 같은 잣대로 재 본 적이 없어 무게를 매길 근거가 없다. ⚠이것은 「우리 커널이 맞고 PathSolver 가 틀렸다」는 뜻이 아니다 — 둘 다 근사이고 실측 대조는 0 건이다.
