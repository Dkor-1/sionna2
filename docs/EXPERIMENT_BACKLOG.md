...
> ⭐**2026-08-28 갱신 — §0-A 의 Q1~Q9(125 줄)는 전부 ✅소진됐다.**
> 디스크 실측으로 확인: Q1 25 샤드 · Q2 8+8 · Q3 8 · Q4 8 · Q5 8 · Q6 24 · Q7 16 · Q8 8 · Q9 24.
> 아래 Q 블록은 **이력으로 남긴다**(같은 것을 두 번 사지 않기 위해). 새로 넣을 줄은 **§0-C** 에 있다.
...
### 0-C. ⭐지금 넣을 줄 (2026-08-28 신설)

⛔**모든 줄에 `--inmem` 을 붙인다.** 안 붙이면 자세마다 정점을 텍스트 OBJ 로 썼다가
되읽는다 — 자세당 **62 ms** 낭비(샤드당 4.2 분). 정점·면이 비트 동일이라 파일 이름이
안 갈리고 기존 샤드와 그대로 이어진다(`benchmark/mesh_inmem.py` 81 건 대조 완료 ·
적대 검증 `adv_refute_hashlottery_0824.json` 통과, 9 판 전부 npaths=41).
⚠2026-08-27~28 에 만든 잡 파일 7 개가 이걸 **전부 빠뜨렸다**. 만든 뒤
`grep -c inmem <잡파일>` 이 줄 수와 같은지 확인한다.

**⭐비교 축은 다섯 팔뿐이다**(사용자 확정): ①다 끔 `R0D0E0F1` ②굴절 `R1D0E0F1`
③회절 `R0D1E1F1` ④굴절+회절 `R1D1E1F1` ⑤우리 커널 `--engine ours`.
**확산 F 는 항상 켠다** — 「다 끔」은 «확산만 켜고 나머지를 끈 것»이다.
⛔모서리만 켜는 칸은 무의미(E 는 D=0 이면 무동작, 축차이 0.01 dB 로 확인).

| 후보 | 무엇을 묻나 | 규모 | 상태 |
|---|---|---|---|
| **R1. 거리 축** | 스위치 순위가 거리에도 안 바뀌나. ⭐다섯 팔이 **15 m 에만** 있었다(r30/60/120/240 은 ours 뿐) | 160 줄 (거리 30·45·60·90 × 앙각 4 × 다섯 팔 × 샤드 2) | ⏳**도는 중** `runners/jobs_0828_range_sweep.txt` |
| **R2. PRF k=8** | 사다리 네 번째 단. ②굴절 el −90 이 p=+0.500 으로 문턱 한가운데라 판정이 안 된다 | 40 줄 | ⏳**대기** `runners/jobs_0828_prf_k8.txt` |
| **R3. PRF × 기체** | 격자 산물 판정이 기체 무관인가 · ⭐우리 커널 θ_half 가 **개구 한계를 따라가나**(프롭 지름 152~381 mm, 로브폭 32.2~12.9°) | 60 줄 | ⏳**도는 중** `runners/jobs_0828_prf_drones.txt` |
| **R4. 거리 × PRF** | 원거리에서도 회절 팔이 격자 산물인가 | 미설계 | 후보 |
| **R5. 방위 × 다섯 팔** | 방위 곡선이 ours 에만 있다(15/30/60/75°) | 미설계 | 후보 |

⚠**거리 축을 읽을 때** — `RESUME.md:386` 이 못박아 뒀다: **레벨 불변은 설계상 당연하다**
(1/R⁴ 은 링크버짓 몫). 거리가 실제로 바꾸는 것은 **파면 곡률**이고, ours 로는 이미
「30~480 m 에서 리듬 몫이 거리에 거의 안 움직이고, 빗살 대비 43.4~43.7 dB 로 평평」이
나와 있다. ⛔**리듬 몫의 절대 크기(82.9~83.1 %)는 인용하지 않는다** — 창 반폭 hw·f_above
가 지배한다(`RETRACTION_LOG` R29). ⚠출처로 적혀 있던 `RESUME.md:386` 은 지금 그 문서에
없다(옛 판 `.bak-0825` 에만 남아 있다).
⇒ R1 이 새로 알려 주는 것은 **«스위치 순위가 거리에 따라 바뀌나»** 뿐이다.
⛔**120·240 m 정면 칸은 `near_numeric_floor` 라 물리로 읽지 말 것.**
...
### 3-B. ⭐속도 최적화 후보 (2026-08-28 조사) — ⛔**GPU 를 사기 전에 «재는 것» 부터**

자세 단가 실측(샤드 원장 `meta[5]` 직독, spp=4e9 · mavic4pro):

| 팔 | 자세당 | 조립 몫 | **솔버 몫** |
|---|---:|---:|---:|
| 회절 켠 두 팔 | 1455~1470 ms | 10 % | **90 %** |
| 회절 끈 두 팔 | 718~869 ms | 17~20 % | 80~83 % |
| 큐 가중 | **1128 ms** | 12.9 % | **87 %** |

⇒ **장면 최적화의 이론 상한이 13 % 다.** 이걸 모르고 크게 기대하면 안 된다.

| 후보 | 상태 | 이득 | 다음 한 걸음 |
|---|---|---|---|
| **`--inmem`** | ✅**적용 완료**(2026-08-28) | 62 ms/자세 = **5.5 %** | 없음. 새 잡 줄에 항상 붙인다 |
| **씬-params 갱신** | ⏸**조건부 — 아직 사지 마라** | ⚠**모른다.** 상한 7.3 %(build_scene 전부) 인데 저장소 옛 감사는 같은 항목을 **−4.3 ms(0.4 %)** 로 적었다(`docs/RESUME_0820.md:114`) — **20 배 어긋난다** | ⭐**GPU 0 으로 `sp.update()` 실비용부터 잰다.** 그 숫자 없이는 살지 말지 못 정한다 |
| 카드당 워커 2 → 3 | ⏸미검증 | 미상 | GPU 메모리·CPU 는 남는다(3.1/97.9 GB · 5.5/16 CPU). 다만 카드 sm 이 이미 85~97 % 라는 관측이 있다 — **재고 나서** |
| CUDA MPS | ⏸미검토 | 미상 | ⛔켜려면 워커를 다시 띄워야 한다. **큐가 빈 때만** |
| `prop` 그룹만 갱신 | ⛔**값어치 없음** | 0.57 ms/자세 = **0.05 %** | 넣지 마라 |
| 솔버 손잡이(spp·max_depth) | ⛔**불가** | — | 물리 예산이라 값이 바뀐다 |

⛔**씬-params 갱신의 치명 함정** — 지금 코드의 `mesh_inmem.update_vertices` 는
`mi.traverse(mesh)` + **메쉬 자신의** `params.update()` 라 **부모 씬에 안 알린다.**
지금 멀쩡한 유일한 이유가 그 직후 `build_scene` 이 `mi.Scene` 을 새로 만들기 때문이다.
**재조립만 빼면 원장이 조용히 오염된다** — 실측 대조:

| | 재조립(생산) 대비 |
|---|---|
| **씬**-params (`scene.mi_scene_params[...]` → `sp.update()`) | **6/6 비트 동일** ✅ |
| **메쉬**-params (지금 코드) | **1/6 만 일치** ⛔ 경로 수까지 갈림 |

⇒ 반드시 **씬 레벨**로 해야 한다. 그리고 착수 전 게이트는 `docs/EQUIVALENCE_GATES.md` **B 층**이다
(옛↔옛 산포를 먼저 재고 옛↔새가 그 안인지 본다). 선례: `benchmark/adv_refute_hashlottery_0824.py`.

⚠**「비트 동일」을 잘못 요구하지 마라.** `--det` 가 큐 어디에도 없고, 코드 주석이
「PathSolver 가 돌려주는 경로 **순서**가 매번 달라 같은 코드로도 합이 갈린다(실측 4 판 4 종류)」
라고 적어 뒀다(`elevation_sweep_md.py:555-560`). ⇒ **4,500 샤드는 옛 코드로 다시 돌려도
비트로 재현되지 않는다.** 진짜 위험은 «값이 바뀐다» 가 아니라 **«옛/새 경로가 서로 다른
확률로 뽑아 칸 사이에 물리가 아닌 계통차가 생긴다»** 이다.
...
| ⛔**프로펠러 단독 팔**(`--parts=prop` · `--parts=-prop`) | **사용자 지시(2026-08-28)** — 「프로펠러 단독은 더 이상 하지마 · 필요없음」. ⭐**이미 있는 데이터를 «인용» 하는 것은 별개다**(el 0·−30·−60·−90 의 \|상관\| 0.019/0.991/0.964/0.350 은 그대로 쓴다). 금지되는 것은 **새로 사는 것**이다. ⇒ `clutter_removal_verdict_0825.json` 의 「프로펠러 단독 팔은 el 0·−30·−60·−90 에만 있다」는 **앞으로도 남는 한계**로 인용한다 |
| ⛔**동체 단독 phy on**(`phys_partsnoprop`) | 동체 단독은 **완전한 상수**다(상대산포 7.10e-17, 굴절 켠 판도 9.24e-17). 상수는 AC 분해에 정확히 0 을 기여하므로 새로 알려 줄 것이 **DC 레벨 한 숫자뿐**이다. 「교차항 100.00 %」는 엔진 섞임의 산물이 아니라 **산술적으로 강제된 값**이다(B 가 상수라 ac(B)≈0, P 가 B 보다 77.9 dB 아래). ⇒ 고칠 것은 팔이 아니라 `crossterm_anatomy_0825.py:8` **한 줄**이다 |
| ⛔**`--n-poses` 사다리**(8192 → 32768 로 «촘촘하게») | **잘못된 손잡이다.** `elevation_sweep_md.py:250` 이 PRF 를 원장에서 고정하고 `:290` 이 자세를 t=i/PRF 로 찍으므로 **n 은 자세 간격에 개입하지 않는다** — 4 배는 «4 배 긴 기록»일 뿐이고 블레이드 통과당 자세 수는 155.5 로 **불변**이다. 촘촘히 보려면 **`--prf`** 를 쓴다(2026-08-27 신설). ⚠0827 덱 14 장의 Future work 문면이 이 잘못된 전제로 쓰여 있다 |
| ⛔⛔**자세 «배치»**(자세 N 개를 한 장면에 묶어 솔버 한 번에) | **영구 제외.** 벽이 셋이고 서로 독립이라 하나를 고쳐도 나머지가 남는다. ①`sb_candidate_generator.py:106,110` 이 `samples_per_src × num_sources` 를 `sampler.seed()` 에 넘기는데 그 wavefront 는 **uint32**(상한 4,294,967,295)다. 생산 `--spp 4000000000` 이 이미 상한의 93 % 라 **N=2 면 8e9 로 넘쳐 `TypeError`**(실측 재현). ②사본 k>0 이 **다른 난수 레인**을 받는다 — 확산이 켜져 있어(`F1`) 그 난수가 산란 방향을 직접 정하므로 **사본을 멀리 떼어놔도 남는다**. ③대상 루프가 **N²**(`:443` 이 `active` 와 무관). ⇒ 그리고 애초에 **솔버가 자세 단가의 87~90 %** 라 장면 최적화의 이론 상한이 **13 %** 다 |
...
### ⛔ 광선 예산은 **uint32 천장(2³² = 4,294,967,296)** 에 막혀 있다 (2026-08-31)

`--spp` 를 4e9 보다 크게 주면 **아예 안 돈다.**
```
sionna/rt/path_solvers/sb_candidate_generator.py:110
  self._sampler.seed(seed, num_samples)
TypeError: seed(): wavefront_size: int = 4294967295
```
지금 쓰는 **4e9 가 이미 천장의 0.9 배**다. 36e9 는 8.4 배, 144e9 는 33.5 배라 즉시 예외다.

**무엇이 막히나** — 「거리 실험의 교란을 가른다」가 막힌다. PathSolver 팔은 전 거리에
`--spp 4e9` 고정이라 표적에 닿는 광선이 1/r² 로 준다(15 m 214,134 → 90 m 5,948).
그래서 「멀어지면 박자가 흐려진다」가 물리인지 광선 부족인지 못 가르는데, **광선을 r² 로
올리는 방법은 천장 때문에 못 쓴다.**

**대신 쓸 수 있는 길**
- 거리를 좁혀 비교한다(15 → 30 m 는 4e9 → 16e9 라 여전히 넘친다. 15 → 21 m 면 8e9 로 넘친다).
  ⇒ **사실상 이 축으로는 못 늘린다.**
- ⭐**반대로 15 m 쪽 광선을 줄여** 90 m 와 같은 «표적당 광선 수» 로 맞춘다.
  4e9 × (15/90)² = **111M** — 천장 안이다. 이쪽이 유일하게 열린 길이다.
- 또는 표적 입체각을 키운다(더 큰 기체·더 가까운 거리).

⚠이 천장은 자세 배치 최적화가 막혔던 이유와 **같은 것**이다(2026-08-28).
