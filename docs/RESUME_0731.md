...
⛔**정정(2026-09-04) — 아래 대비는 `RETRACTION_LOG` R14 가 무효화했다**(2026-08-03,
φ 0~355° 72 점 실측). 「23.17 dB · 200배」는 φ 의 성질이 아니라 **스윕하지 않은 고도차
Δz = 35 m** 의 성질이고 단일 d 칸 값이다. φ 를 실제로 쓸어 보니 **R90 span 0.48 %
(≤0.083 dB)** 이고 **φ=90° 가 세 팔 모두 최소**다 — 방향까지 반대였다.
...
```
report13·report05 의 R90·커버리지·블라인드가 전부 이 한 방위에서 풀린 것은 맞다. 다만
남는 것은 **«한 컷으로만 보고했다» 는 보고 범위**이지 위험의 크기가 아니다.
