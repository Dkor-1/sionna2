# ⭐⭐부호 규약 라운드 — 2026-08-11 08:xx KST

## 끝낸 것

| # | 무엇 | 원장 |
|---|---|---|
| 1 | ⭐⭐**구면파 도플러 부호 결함**을 찾아 고쳤다. `sbr_field(range_m=)` 가 **거리**에 `+j` 를 걸어 평면파 갈래(**투영**에 `+j`)와 정확히 −1 배였다 | `verify_phase_sign.json` 게이트 **3/3** · 철회기록 **R28** |
| 2 | 옛 자료 **59 파일 · 70 배열**을 `np.conj` 로 정정(재계산 불필요·멱등) | `benchmark/fix_phase_sign_legacy.py` |
| 3 | ⭐⭐**PO 커널 운동학 검증** — 해석 점산란체와 수렴격자에서 `ρ = 0.9999~1.0000`. 사용자가 지적한 «위상 변동 비단조» 는 **커널이 아니라 잣대**였다 | `verify_po_elev_unit.json` · `verify_po_elev_rebuttal.json` |
| 4 | 생산 격자 **λ/12 사면** — 관심 대역 안에서 조화선당 ≤ 0.14 dB. 격자 재계산 계획 취소 | 위 원장 |
| 5 | ⭐⭐`md_range_sweep` 재생성 완료(**37 dB 반영**, `g_mf_db = 36.99`, 규약 `v2_2026-08`) | `outputs/md_range_sweep.json` (14 셀) |

### ⭐ 3 번의 진짜 원인 — 앙각마다 **동체 대 로터 세기비가 뒤집힌다**

| el | 0 | −15 | −30 | −45 | **−60** | −75 | −90 |
|---|---|---|---|---|---|---|---|
| DC(0 Hz) 전력 비중 | 97.2 % | 98.9 % | 72.4 % | 86.4 % | **5.1 %** | 91.8 % | 99.99 % |
| \|E\|min/max | 0.535 | 0.562 | 0.055 | 0.278 | **0.0049** | 0.488 | 0.967 |

−60° 에서 정지 동체 항이 로터 항과 맞먹어 합성 페이저가 원점을 **10 바퀴** 감는다.
`np.unwrap` 은 그걸 누적해 4,217° 를 낸다. 상관 **−0.981**.
⇒ 그 잣대는 도플러가 아니라 **동체 대 로터 비**를 재고 있었다. 산란 물리를 0 으로 만든
기하 기준에 같은 잣대를 걸면 우리 커널보다 **더 심하게** 널뛴다.

## 지금 도는 것

| 무엇 | 어디 | 상태 |
|---|---|---|
| ⭐**250M 앙각 스윕**(`--engine sionna --spp 250000000`) | GPU 2, 8 샤드 | 7 앙각 중 6 완료. 규칙값 11.1M 은 경로 6~13 개라 «대역밖이 가장 심한 구간» 이었다 |
| 광선 사다리 마지막 계단 | GPU 2 | 2 프로세스 |
| 선행연구 조사 · 문서 정합 | CPU | 계속 |

## ⚠미해결 선언

1. ⚠**구면파로 낸 그림들의 도플러 축이 뒤집혀 있다.** 수치는 안 바뀌지만 **그림은 재생성해야** 한다.
   8/11 덱 거리 그림이 여기 해당한다 — 덱이 인용한 수치는 전부 크기 대칭이라 **주장은 그대로 선다**.
2. ⚠**Sionna el=0 자료 결함** — σ = 2,931 m² 인데 물리 상한이 5.8 m² 다(27 dB 초과). 기전 미확인.
3. ⚠**mavic4pro `dc_ac` −30 dB 이동** — 원장 두 판이 37.18 ↔ 5.33 으로 갈린다. 미설명.
4. ⚠**el −60 의 최대 위상 스텝 176.4°** — π 모호성 경계다. 그 표본은 **어떤 잣대로도** 판정 불가이고,
   진짜 한계는 잣대가 아니라 **슬로타임 표본화**다.
5. ⚠**우리 σ 는 여전히 `NOT_VALIDATED`** — 이 라운드는 거기 손대지 않았다. 운동학만 판정했다.
6. ⚠`build_part07_microdoppler.py:740·799·837` 이 얼린 팔 절대 레벨을 리포트에 **다시 써넣는다**
   (노트북만 고치면 되돌아온다). 리포트 08_3 자기모순(:1119 ↔ :1100·:1228·:1304)도 남아 있다.

## 할 것 (상류부터)

1. 250M 앙각 스윕 끝나면 **Chapter 1 사과-대-사과 비교** — 이제 부호도 맞고 예산도 맞는다.
2. 구면파 그림 재생성(도플러 축).
3. `md_range_sweep` 수확 — `docs/MD_RANGE_SWEEP_CITATIONS.md` 지도대로 그림 4 장 · «거리 대 정확도» 곡선.
4. 1층 GPU 대기 — `hover_long` 순수 PO 심판 팔 · `das_fleet_sigma` 재실행.
5. 3층 — 딥러닝 분류 착수(결정트리 버리고 CNN).
