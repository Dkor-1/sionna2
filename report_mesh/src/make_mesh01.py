...
"(예: body / arm / motor / prop ...)에 속하는지 이름표를 답니다.",
"```",
"",
...
"← 출처: `src/drone_cad.py:9` (\"내부 면이 그대로 남아 PO/SBR 이 헛세는 원인\").",
"",
...
"← 출처: `src/drones.py:94~113` DroneSpec `note` 필드·`docs/SPECS.md` (각 기종 dji.com 스펙페이지 URL 포함).",
...
"",
f"공식 외형치수와의 오차는 5종 최악이 **{worst_dim:.1f}%** (자세한 검증은 mesh04)",
...
"  '인피니티 볼 짐벌'을 반영했다 ← 출처: `src/drone_cad.py` 머리말 실루엣 목록(15~16행).",
...
"형상이 충분히 매끄럽다(엣지 길이 통계는 mesh06 에서 자세히)",
...
"| 자료 | 정체 | 라이선스 | 용도 |",
"|---|---|---|---|",
f"| Phantom 4 실기체 3D 스캔 | {G['source']['thing']} (저작자 {G['source']['author']}) | {G['source']['license']} | 표면 오차 채점 (mesh07) |",
"| Typhoon H480 / 3DR Solo CAD | ethz-asl/rotors_simulator 실물 모델 | Apache-2.0 | 실물 CAD 갤러리 대조 (mesh07) |",
"| Holybro X500 부품(프롭·모터) | PX4/PX4-gazebo-models | BSD-3-Clause | 부품 형상 대조 (mesh07) |",
"",
...
"자세한 방법(chamfer 거리 — 두 표면 사이 최근접점 거리 분포)과 그림은 mesh07 에서.",
"",
...
"물리의 요구다 ← 출처: 재질 배정 원본 `src/materials.py`(ITU-R P.2040 + 커스텀), 상세는 mesh05.",
"",
...
f"  **{META['mesh_engine']}** 엔진으로 깎는다 ← 출처: `mesh_verify.json` `meta.mesh_engine`·`src/drones.py:243` `MESH_ENGINE = \"cad\"`.",
"- **왜 trimesh + manifold3d 인가?** 처음엔 의존성 없는 자작 도구(`src/geom.py`)만 썼는데,",
"  프리미티브를 겹쳐 쌓는 방식이라 \"**불리언이 없어 겹친 파트의 내부 면이 그대로 남아",
"  PO/SBR 이 헛세는 원인**\"이 됐다 ← 출처: `src/drone_cad.py:9` (그대로 인용). manifold3d 의",
"  불리언 합집합으로 내부 면을 아예 녹여 없앤 것이 지금 엔진이다 — 더 정확하고 더 빠르다",
"  ← 출처: `src/drone_cad.py:23~27` \"왜 이게 RCS 에 중요한가\". (자작 `geom.Mesh` 는 여전히",
"  최종 컨테이너·OBJ 저장 담당으로 남아 있다 ← 출처: `src/geom.py:41`.)",
...
"### 4.3 검증 9종 — 각각 한 질문",
"",
"검증 스위트의 자기소개를 그대로 옮긴다 ← 출처: `report_mesh/src/verify_mesh_suite.py:9~18`",
"모듈 docstring (그대로 인용):",
"",
"| 검사 | 질문 |",
"|---|---|",
...
"| 편 | 주제 (한 줄) |",
"|---|---|",
"| **mesh02** | 몸체를 코드로 깎는 법 — 로프트·스윕·불리언으로 눈물방울 동체가 조립되는 과정 |",
"| **mesh03** | 프로펠러 — NACA 익형 단면을 피치·테이퍼·스큐로 비틀어 진짜 블레이드를 만들기 |",
"| **mesh04** | 치수·대칭 검증 — 공식 제원 대조(오차 %)와 좌우 미러 chamfer 거리 |",
"| **mesh05** | 재질 배정 — 색 = 부위 = 전파재질(ITU-R P.2040) 규칙과 금속/플라스틱의 밝기 차 |",
"| **mesh06** | 삼각형 품질과 부위 겹침 — 엣지 길이 vs 파장, 의도된 겹침의 정직한 공개 |",
"| **mesh07** | 실물과 대조 — Phantom 4 실기체 스캔 오버레이·실물 CAD 갤러리 채점 |",
"| **mesh08** | 수치 수렴 — 점 간격(PO)·세분화(SBR)를 바꿔도 RCS 가 안 흔들리는지 증명 |",
"",
...
"---",
"",
"**다음 편** → [mesh02 — 몸체를 코드로 깎는 법](mesh02_build.ipynb) : 로프트가 뭔지,",
"불리언 합집합이 왜 내부 면을 없애 주는지, 빌드 단계 그림과 함께."),
