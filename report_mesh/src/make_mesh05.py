...
"",
f"**한 줄 요약** — 우리 드론 5종의 프로펠러는 납작한 판이 아니라, **NACA 익형 단면을 "
...
f"C_T 법은 3950~4410, 공식 최대 rpm(7500, C2 인증) 앵커링은 4740~5300 을 준다. 우리는 어느 쪽이 "
f"맞는지 **모른다**. 플래시·f_tip 이 이 값에 선형으로 걸린다 ← 출처: src/drones.py:139-144 note.",
"",
"그래서 프로펠러의 **모양**이 틀리면 이 지문 전체가 틀린다. 날의 폭(시위)·비틀림(피치)·휨(스큐)이 "
"각도별 반짝임의 세기와 타이밍을 정하기 때문이다. 마이크로도플러 결과는 **report08** "
"([report08.ipynb](../report08.ipynb) §4)에서 다루고, 이 편은 그 입력이 되는 **날개 기하**를 만든다.",
"",
...
"- **(b)** 기하 피치각 θ(r)=atan(P/2πr) 을 5기종 스펙 피치로 그린 곡선 — §4. "
...
"",
"날개 단면을 어떻게 그릴까? 우리는 1933년 NACA 보고서 이래 항공계 표준인 **NACA 4자리 대칭 익형**의 "
...
"",
"5기종의 피치와, 그로부터 계산한 허브 쪽(0.15R)·팁의 피치각:",
"",
PITCH,
"",
...
"",
f"그림 (b)의 다섯 곡선이 전부 **왼쪽(허브)에서 높고 오른쪽(팁)에서 낮은** 이유는 공식이 말해 준다. "
...
f"이어지고, 팁 피치각 {ROWS['s1000plus']['th_tip']:.1f}° 로 다섯 중 가장 완만하다 — 큰 프롭을 천천히 "
f"돌리는 리프터의 전형(P/D 비가 가장 작다).",
f"- **Mavic 4 Pro·Matrice 4E·Phantom 4** 는 P 가 5.0~5.8″ 로 비슷해 곡선이 거의 겹친다 — 팁 "
f"{ROWS['mavic4pro']['th_tip']:.1f}~{ROWS['phantom4']['th_tip']:.1f}° 대.",
"",
...
"|---|---|---|",
"| `root_frac` | 0.14 | 날이 시작하는 반경 비율 — 안쪽 14% 는 허브(클램프)가 차지한다. 실물 접이식 프롭의 뿌리 위치 |",
"| `chord_max` | 0.26 (**비율!**) | 최대시위 = 0.26·R. 코드 주석: \"실물 1345 는 0.30R — 소비자용 프롭은 더 슬림\" ← src/drone_cad.py:349 |",
...
"| `n_sec`·`n_pts` | 22·36 | 스팬 단면 수·단면당 점 수 — 곡면 매끈함과 삼각형 수의 절충(§1: 프롭이 이미 전체 면의 절반) |",
),
...
"",
f"1. **프롭 지름** — 완성 메쉬에서 실측한 Mavic 4 Pro 프롭 지름은 공식값(266.7 mm) 대비 "
f"오차 **{PROP_ERR:+.2f}%** 다. 시위·스큐·로프트를 다 거친 뒤에도 반경이 스펙에서 벗어나지 "
...
f"3. **시위** — 최대시위는 0.26·R(Mavic 기준 **{C_MAX_MM:.1f} mm**). 실물 1345 프롭의 0.30R "
f"보다 슬림한데, 이는 소비자용 프롭의 날씬한 평면형을 반영한 선택이다 ← 출처: "
f"src/drone_cad.py:349 주석 \"실물 1345 는 0.30R — 소비자용 프롭은 더 슬림\".",
"",
...
for key in {ORDER!r}:
    ...
    print(f"{{s.name:16s}}  R={{R*1000:5.1f}} mm  →  최대시위 {{max_chord_mm(R):5.1f}} mm  (= 0.26·R = {{0.26*R*1000:5.1f}} mm)")""",
),
...
"포함하면(full) p95 가 수십 mm 로 **수십 배** 뛴다. 결함일까? 아니다 — 검증 코드의 docstring 이 "
"먼저 답을 적어 두었다:",
"",
...
f"최대시위 0.26R={C_MAX_MM:.1f} mm(실물 1345 의 0.30R 대비 슬림 — 소비자용) (§6).",
...
"이어진다. 마이크로도플러 **결과**가 궁금하면 [report08.ipynb](../report08.ipynb) §4 로.",
),
]
