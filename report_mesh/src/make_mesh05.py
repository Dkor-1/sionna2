...
"---",
"",
"## ⏳ 이 편의 형상 절은 기체별 프로펠러 정본화 라운드가 채운다",
"",
"> ⏳ **날 시위 분포 · 기종별 두께 · 팁 형상 · 파장 대비 삼각형 크기**는 별도 라운드가 정본이다.",
"> 그 라운드가 법칙 자체를 다시 정하는 중이라, 이 편에 값을 적으면 두 곳이 갈린다.",
"> **그래서 이 편은 지금 «방법과 원리»만 싣고, 형상 상수의 판정은 비워 둔다.**",
"",
"지금 확실한 것만 적는다:",
"",
...
"- **회전 원반 지름은 정규화돼 있다** — 공표 프롭 지름과의 오차가 사실상 0 이다(mesh08 §1).",
"  즉 **지름은 맞춰 놓았고, 아직 정본화가 남은 것은 «날의 모양»** 이다.",
"",
"**비워 둔 칸**(그 라운드가 채운다):",
"",
"| 축 | 지금 상태 | 누가 정하나 |",
"|---|---|---|",
"| 날 시위 분포(평면형) | ⏳ | 기체별 프로펠러 정본화 라운드 |",
"| 기종별 날 두께 | ⏳ | 같은 라운드 |",
"| 팁 형상 | ⏳ | 같은 라운드 |",
"| 파장 대비 삼각형 크기 | ⏳ — 로터 1개당 삼각형 수가 고정인데 프롭 지름은 기체마다 크게 다르다 | 같은 라운드 |",
"| 기하 피치 기준 반경 | ⏳ | 같은 라운드 |",
"",
"아래 §1~§8 은 **그 라운드의 결과와 무관하게 유효한 부분**이다 —",
"왜 익형 단면인가, 피치가 무엇인가, 로프트가 무엇을 만드는가, 카이럴성이 왜 물리인가.",
"",
"---",
"",
...
# ------------------------------------------------------- 11. §6 현재 검증
md(
"## 6. 지금 검증돼 있는 것과, 아직 정본화 중인 것",
"",
"§3~5 의 유도가 코드에서 실제로 지켜지는지 확인한다. **확정된 축과 진행 중인 축을 나눠 적는다.**",
"",
"### 확정 — 지금 믿어도 되는 것",
"",
f"1. **회전 원반 지름** — 완성 메쉬에서 잰 프롭 지름은 공표값 대비 오차 **{PROP_ERR:+.5f} %** 다.",
"   빌드가 **날을 지은 뒤 실제 최대반경을 재서 되돌리기** 때문이다. ⚠ 이 값은 «맞다» 가 아니라",
"   **«맞춰 놓았다»** 로 읽어야 한다 ← 출처: mesh_verify.json `C_dims.*.checks.prop_dia`.",
f"2. **피치 곡선** — 비틀림 θ(r) 이 공표 피치 P 하나에서 θ(r)=atan(P/2πr) 로 유도된다(§4).",
f"   허브 쪽 피치각이 Mini {ROWS['mini5pro']['th_hub']:.0f}° ~ Mavic "
f"{ROWS['mavic4pro']['th_hub']:.0f}° 로 갈리는 **기종별 개성**이 메쉬에 그대로 실린다.",
"3. **카이럴성** — CW/CCW 가 서로 거울상으로 지어지고, 로터별 비틀림 방향이 회전방향과 맞는지를",
"   출하 게이트가 검사한다(**손대칭성 검사** — 좌우 뒤집힌 기체를 잡는 장치, mesh07 §2).",
"4. **프롭 어셈블리의 기하 건강** — 전 기종 수밀, 중복면 0, 안쪽 법선 0",
"   ← 출처: `docs/MESH_AUDIT_0816.md` §③.",
"",
"### ⏳ 정본화 진행 중 — 이 편이 값을 적지 않는 축",
"",
"| 축 | 왜 여기서 안 적나 |",
"|---|---|",
"| 최대시위 비율 · 시위 분포 | 기체별 프로펠러 정본화 라운드가 법칙 자체를 다시 정하는 중 |",
"| 기종별 날 두께 | 같은 라운드. 두께는 **재질 계산의 손잡이**라 σ 에 크게 실린다 |",
"| 팁 형상 | 같은 라운드 |",
"| 파장 대비 삼각형 크기 | 로터 1개당 삼각형 수가 고정인데 프롭 지름은 기체마다 크게 다르다 |",
"| 기하 피치의 기준 반경 | 같은 라운드 |",
"",
...
# ⏳ chord_max 의 **값**은 이 편이 정하지 않는다(정본화 라운드의 몫) — 코드에서 읽어 온다.
import inspect
from drone_cad import _blade
CMAX = inspect.signature(_blade).parameters["chord_max"].default

def max_chord_mm(R, chord_max=CMAX):
...
print(f"현재 코드의 chord_max = {{CMAX}} (⏳ 정본화 라운드가 정한다)\\n")
for key in {ORDER!r}:
    s = DRONES[key]
    R = s.prop_dia_mm / 2000.0                        # 반경[m] ← 스펙
    print(f"{{s.name:16s}}  R={{R*1000:5.1f}} mm  →  최대시위 {{max_chord_mm(R):5.1f}} mm"
          f"  (= chord_max·R = {{CMAX*R*1000:5.1f}} mm)")""",
),
...
"⏳ 그 동작에 넣는 **상수 값**은 기체별 프로펠러 정본화 라운드가 정한다 (§5·§6).",
f"4. 지금 확정된 것: 회전 원반 지름 오차 {PROP_ERR:+.5f} %(맞춰 놓은 값), 피치 곡선은 공표 P 에서 유도, "
"카이럴성은 출하 게이트가 검사 (§6).",
