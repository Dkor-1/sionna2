...
cells.append(md(
    ...
    "",
    "**왜 자작 검사가 아니라 trimesh 인가.** 우리 메쉬는 자작 라이브러리 `src/geom.py` 로 쌓는다.",
    "그런데 \"쌓는 도구\"에게 \"네가 쌓은 게 옳은지\"를 물으면 같은 버그를 두 번 믿게 된다.",
    "mesh_check.py 의 docstring 이 이유를 그대로 적어 놨다:",
    "",
    "> geom.py 는 의존성 없이 삼각형을 쌓는 도구다. 대신 **메쉬가 옳은지 검사할 능력이 없다.**",
    "> 실제로 조용히 살아 있던 버그: prop_blade 의 **캡 2장이 뒤집혀 있었다**(법선 안쪽).",
    "> 주석엔 \"outward\" 라고 적혀 있었다. (…) trimesh 는 이걸 1초 만에 잡는다.",
    "",
    "← 출처: src/mesh_check.py 5~12행 docstring 원문.",
    "",
    "도구별 채택 이유와 대안:",
    "",
    "| 도구 | 무엇에 쓰나 | 왜 이것인가 (대안은 왜 아닌가) |",
    "|---|---|---|",
    "| **trimesh** | watertight·winding·부호부피 판정 | 파이썬 메쉬 검증의 사실상 표준. `is_watertight`·`is_winding_consistent`·부호있는 `volume` 이 내장 — 직접 짜면 검증기 자체를 또 검증해야 한다. geom.py 주석의 \"설치 권한 없음\"은 낡은 이유였다(py312 는 사용자 소유 venv) ← 출처: mesh_check.py 11~12행 |",
...
cells.append(md(
    ...
    "   ← 출처: verify_mesh_suite.py sec_F_overlap() 240~243행 주석 \"watertight 단일컴포넌트만 불리언 대상(견고성)\".",
...
cells.append(md(
    ...
    "",
    "**반면교사 — 이 프로젝트에서 실제로 겪은 안쪽법선 버그 2건:**",
    "",
    "| 버그 | 어디서 | 무엇이 뒤집혔나 | 왜 아팠나 |",
    "|---|---|---|---|",
    "| ① uv_sphere | 구(짐벌 카메라 공 등) 생성기 | 감는 방향 규약 위반으로 법선이 안쪽을 향함 + 극점 밴드에서 0-넓이 퇴화 삼각형 | 카메라 공처럼 잘 보이는 곡면의 반사가 왜곡. 수정 후 감기 규약 명문화 + 극점을 삼각형 팬으로 바꿔 퇴화면 제거 ← 출처: src/geom.py uv_sphere() 289~292행 주석 |",
    "| ② prop_blade 캡 | 프로펠러 날개 뿌리/끝 마감 2장 | 두 캡의 감기가 서로 반대 → 한쪽은 항상 안쪽 법선. **주석에는 \"outward\"라고 적혀 있었다** | 프로펠러는 마이크로도플러 신호 그 자체라 특히 민감 ← 출처: src/geom.py 368~376행 \"⚠ 2026-07-14 수정\" 주석, src/mesh_check.py 7~10행 |",
    "",
    "②가 무서운 이유는 **주석과 코드가 달랐다**는 점이다. 사람 눈은 주석을 믿는다. 그래서 지금은",
    "믿음 대신 기계 검사를 파이프라인에 넣었다: 닫힌 부품의 부호있는 부피가 음수면(=안팎 뒤집힘)",
    "`mesh_check.assert_ok()` 가 예외를 던져 **빌드 자체가 실패**한다 — 재발 방지 회귀 장치다",
    "← 출처: src/mesh_check.py assert_ok() 90~98행.",
    "",
...
cells.append(md(
    ...
    "  퍼뜨린다. 기준은 넓이 1e-14 m² 미만 ← 출처: src/mesh_check.py 57행. 과거 uv_sphere 극점이",
    "  바로 이 유형이었다(§4 표 ①).",
...
cells.append(md(
    ...
    *[f"| {NAME[k]} | {A[k]['edge_mm']['p50']:.1f} mm | {A[k]['edge_mm']['p95']:.1f} mm | "
    ...
    "",
    f"**정직한 공개 — 최장 엣지는 λ 를 넘는다** (예: S1000+ 최장 {A['s1000plus']['edge_mm']['max']:.0f} mm"
    ...
    "",
    f"**최소각도 정직하게**: 1퍼센타일이 {min(A[k]['tri_min_angle_deg']['p1'] for k in ORDER):.1f}°까지",
...
cells.append(md(
    ...
    *[f"| {NAME[k]} | {fr95[k]:.2f} mm | {fu95[k]:.1f} mm | ×{fu95[k]/fr95[k]:.0f} |" for k in ORDER],
    ...
    "로터마다 dir=+1/−1 을 교대로 준다 ← 출처: src/drones.py rotor_layout() 373~374행 docstring",
    "\"dir:+1/-1(CCW/CW) — 인접 로터가 반대로 도는 멀티로터 관례\"·386행. 날개 장착 위상도 로터마다",
...
cells.append(md(
    ...
    *[f"| {NAME[k]} | {Fo[k]['total_overlap_cm3']:.0f} cm³ | {ovp[k]:.2f}% | "
    ...
    "   몸체와 한 덩어리면 관절이 죽는다 ← 출처: src/drones.py _build_propeller_legacy() 361~363행.",
...
cells.append(md(
    ...
    f"| 안쪽 법선 | {tot_inward}건 (과거 버그 2건은 수정·회귀장치 가동) | 통과 |",
...
cells.append(md(
    ...
    "",
    "**다음 리포트 → mesh08 — 검증 ② (공식 제원 치수 대조·부피/암시밀도·재질 커버리지,",
