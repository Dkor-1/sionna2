...
md("## 0. 왜 출처만 다루는 리포트가 따로 있나",
   ...
   "정밀하게 계산해도, 기체 치수가 틀렸으면 결과도 틀린다. 그래서 이 시리즈는 검증(mesh05~)에 앞서",
...
md("## 1. DJI 공식 제원 — 조사 → 독립검증의 2단계",
   ...
   "그 이유를 `note` 필드에 남겼다(§1.2). ← 출처: `docs/drone_research.json` 구조 · `src/drones.py:37`(DroneSpec 정의)·`:91`(DRONES)",
   ),
...
md("## 1.3 사례 2·3 — Mavic 4 Pro 대각, Matrice 4E 프로펠러",
   ...
   "",
   "**세 사례의 교훈**: 웹에서 한 번 긁은 값은 믿지 않는다. (1) 독립 재조사로 대조하고,",
   "(2) 공식 외형과의 **기하 일관성**으로 다시 검산하고, (3) 정정 이력을 지우지 않고 남긴다.",
   "이렇게 채택된 최종 치수가 실제 메쉬와 일치하는지는 mesh05(치수 검증)에서 자로 잰다.",
   ),
...
md("## 2. 실기체 3D 스캔 — DJI Phantom 4 (Thingiverse thing:1456295)",
   ...
   "Phantom 4 한 기체로 방법을 확립하고(→ mesh07 스캔 대조) 나머지는 공식 외형 치수로 검증한다.",
   ),
...
md("## 2.2 스캔 데이터의 한계 — 알고 쓰는 것",
   ...
   f" {G['scan_to_cad_mm']['p50']:.1f} mm)는 **mesh07(스캔 대조 편)** 에서 그림과 함께 다룬다 —",
...
md("## 3.1 원칙: 다운로드 모델은 '검증 전용', 표적은 스펙에서 생성",
   ...
   "**잣대로만** 쓴다(→ mesh07). ← 출처: `assets/meshes/reference/SOURCES.md` · `src/drone_cad.py`",
   ),
...
md("## 5. 라이선스를 어떻게 지켰나",
   ...
   " `report_mesh/src/viz_mesh_reports.py:229-246` (그림 제목 문자열)",
   ),
...
md("---",
   ...
   "```bash",
   "# 이 노트북 재생성",
   "/home/yunjung/.venvs/py312/bin/python report_mesh/src/make_mesh03.py",
   "",
   "# 본문 수치의 원천 재생성(검증 스위트 → mesh_verify.json, 그림 포함)",
   "/home/yunjung/.venvs/py312/bin/python report_mesh/src/verify_mesh_suite.py",
   "/home/yunjung/.venvs/py312/bin/python report_mesh/src/viz_mesh_reports.py",
   "",
