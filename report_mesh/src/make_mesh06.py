...
CAM_MISMATCH_DB = db(GM["camera"]) - db(G_PLASTIC_BULK)   # 카메라 버그의 어긋남 크기
...
MAT_KO = {"plastic": "플라스틱(ABS/PC)", "carbon": "탄소섬유(CFRP)", "metal": "금속(ITU)",
          "prop_plastic": "프로펠러 플라스틱", "camera_assembly": "카메라 조립품", "pcb": "PCB(FR-4+구리)"}
...
cells.append(md(
    ...
    "근거, 그리고 실제로 있었던 버그(카메라 10.9 dB 어긋남)까지 다룬다.",
    "",
    "## 용어풀이",
    "",
    "| 용어 | 한 줄 뜻 |",
...
cells.append(md(
    ...
    "> ← 출처: `src/drones.py:281-285` (`_build_frame_legacy` 내부 주석)",
    "",
...
cells.append(md(
    ...
    *rows,
    ...
    "  금속 하우징+유리 렌즈+짐벌 모터의 복합체라서인데, 이 항목이 실제 버그를 고친",
    "  이야기는 §5 에서.",
))
...
cells.append(md(
    ...
    "",
    f"직관 하나: 우리 주파수 {FC_GHZ:g} GHz 의 파장은 약 {LAM_MM:.1f} mm",
    "← 출처: `mesh_verify.json` §meta lam_hi_mm. 드론 셸 두께(1~3 mm)는 파장의 수십분의 1이라",
...
color_rows = []
COLOR_STORY = {
    "metal":           "강청(steel-blue) — 모터·배터리 포일. '차가운 금속' 연상색",
    "plastic":         "밝은 회색 — 동체 셸·캐노피·착륙장치·식별색. 가장 넓은 면적이라 중립색",
    "carbon":          "검정 — 탄소섬유 암. 실물 CFRP 도 검다",
    "prop_plastic":    "주황 — 프로펠러. 마이크로도플러의 주인공이라 **일부러 잘 보이게**",
    "camera_assembly": "청록 — 짐벌 카메라(금속 하우징+유리 렌즈). 렌즈 유리 연상색",
    "pcb":             "초록 — ESC/메인보드. 실물 FR-4 솔더마스크가 초록",
}
...
cells.append(md(
    ...
    "",
    "렌더 색은 `src/drones.py:513-520` 의 `MATERIAL_COLOR` 6색이 전부다",
    "(주석 원문: \"재질 → 표시색 (모든 드론 공통; 색만 보면 재질을 안다) — 2026-07-16 사용자",
    "지시로 재질별 매핑\").",
    "",
    ...
    *color_rows,
    ...
    "이 리포트의 해설)",
    "",
    "**원래는 이렇지 않았다.** 초기 버전은 기종마다 개성 색(Phantom=흰색, S1000=검정,",
    "Mavic=실버 …)을 입혔다 — 보기엔 실물 같지만, 그림에서 **어느 부위가 금속인지 알 수",
    "없었다**. 그래서 '드론별 개성 색'을 폐기하고 재질색으로 통일했다. 결정의 이유는",
    "`drone_colors()` docstring 에 그대로 남아 있다:",
    "",
...
cells.append(md(
    ...
    "",
    "- 회색 동체·캐노피(플라스틱), 그 앞 아래 **청록** 짐벌 카메라, 로터 위 **주황** 프로펠러,",
    "  로터 허브의 **강청** 모터. 동체 속에는 렌더로는 잘 안 보이는 **강청 배터리·초록 PCB** 가",
    "  들어 있다(§1 의 내부 산란체).",
    f"- `arm` 그룹이 없다 — Matrice 4E 의 접이식 암은 셸형이라 `body`(플라스틱)로 흡수되고,",
    "  `accent` 식별색도 이 기종엔 없다(§2.1 표의 '실재하는 기종' 열과 일치).",
    "",
    "같은 규칙의 나머지 4종 그림(`wireframe_mini5pro/mavic4pro/s1000plus/phantom4.png`)은",
    "형상 편(mesh02)에서 다룬다 — 색 규칙은 거기서도 동일하다.",
))
...
cells.append(md(
    ...
    "| **커스텀 (문헌 기반)** | `plastic`(ABS/PC, εr=2.7), `prop_plastic`(나일론/PC), `carbon`(CFRP, σ=3×10³ S/m), `pcb`(FR-4+구리) | ITU 표에 이 재질들이 **없다**. 문헌값으로 정의하고 근거를 각 `note` 에 기록 ← `src/materials.py:69-90` |",
    "",
...
cells.append(md(
    ...
    f"| `prop_plastic` | {G_PLASTIC_BULK:.3f} | {GM['prop']:.2f} | \"셸보다 더 얇아 실효 \\|Γ\\| 0.25\" ← `materials.py:79-80` |",
...
cells.append(code(
    ...
    "print(f'camera 버그의 크기: 20*log10(0.85/{gp:.4f}) = '\n"
    "      f'{20*math.log10(0.85/gp):.1f} dB  # §5 참조')\n"
))

# --- 12. §5 camera 버그 사례 -----------------------------------------------------------
cells.append(md(
    "## 5. 사례 연구 — 카메라가 두 엔진에서 10.9 dB 어긋났던 버그",
    "",
    "이 체계가 왜 '한 곳 정의'를 고집하는지 보여주는 실제 사고다. 2026-07-14 감사 전에는",
    "재질 체계가 **두 개**였다 — Sionna 용 `make_material()` 과, PO 용으로 손으로 적어둔",
    "`GROUP_GAMMA` 표. 그리고 두 표가 조용히 어긋나 있었다. `drones.py` 주석 원문:",
    "",
    "> \"⚠ 2026-07-14 수정: camera 는 Sionna 에서 **plastic**(\\|Γ\\|=0.244)이었는데 PO 에선",
    "> 0.85 였다 — 같은 부품을 두 엔진이 **10.9 dB** 다르게 본 버그. 이제",
    "> 'camera_assembly'(ITU metal + PO 실효 0.85)로 통합되어 그런 어긋남이 구조적으로",
    "> 불가능하다. prop 도 'prop_plastic'(PO 실효 0.25)으로 분리 — 예전 손표의 값을 재질",
    "> 정의로 옮겼다.\"",
    "> ← 출처: `src/drones.py:191-194`",
    "",
    f"어긋남의 크기를 지금 값으로 재계산해도 20·log10({GM['camera']:.2f}/{G_PLASTIC_BULK:.3f}) =",
    f"**{CAM_MISMATCH_DB:.1f} dB** 다(위 코드 셀에서 확인 가능). 같은 카메라를 RT 는 반투명",
    "플라스틱으로, RCS 는 금속 덩어리로 보고 있었던 셈 — 두 결과를 비교·교차검증하는 순간",
    "그 비교 자체가 무의미해진다.",
    "",
    "**고친 방법**이 중요하다. \"어느 쪽 숫자가 맞나\"를 골라 한쪽 표를 고치는 대신,",
    "**표 자체를 하나로 합쳤다**: `camera_assembly` 라는 재질을 신설하고(Sionna 쪽=ITU metal,",
    f"PO 쪽=실효 {GM['camera']:.2f}, 근거는 note 에 ← `src/materials.py:52-58`), 두 엔진이 그 한",
    "항목을 읽게 했다. 사람이 같은 값을 두 번 적는 구조가 남아 있는 한 같은 사고는 재발하기",
    "때문이다(materials.py docstring \"■ 왜 이렇게 바꿨나\" ← `src/materials.py:7-13`).",
))
...
cells.append(md(
    ...
    f"{db(GM['body']):.1f} dB)가 금속(0 dB)으로 계산되는 — §5 와 같은 종류의 — 조용한 과대평가를",
