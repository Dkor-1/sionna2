...
"+ `scipy`(스플라인 보간)다. 왜 이 조합인가 — 의존성 없는 자작 엔진(geom.py)에는",
"**불리언·로프트·스무딩·검증이 전부 없어서** \"프로펠러 캡 법선이 뒤집힌 걸 몇 달간 못 잡았\"기",
"때문이다 ← 출처: src/cadkit.py:5-17 모듈 docstring (대안이었던 legacy 엔진은",
"src/drones.py:246-290 `_build_frame_legacy` 로 비교·회귀용으로만 남아 있다).",
"",
"조립 흐름 한 눈에:",
"",
"```",
"DroneSpec(공식 숫자, src/drones.py:36)          §1",
"   → build_frame_cad(부위 조립, src/drone_cad.py:226)   §2",
"   → 불리언 합집합(내부 면 제거, src/drone_cad.py:324)   §2",
"   → frame_fit_scale(공식 외형 맞춤, src/drones.py:323)  §3",
...
"",
"`DroneSpec`(src/drones.py:36-81)은 드론 한 종의 모든 정보를 담는 데이터클래스인데,",
...
"",
"프레임 조립은 `build_frame_cad(spec)`(src/drone_cad.py:226-330) 한 함수가 담당한다.",
"S1000+(옥토콥터, 원형 카본 센터프레임)만 별도 분기이고, 나머지 4종은 공통 순서를 따른다.",
"",
"**1단계. 동체 — 초타원 단면의 로프트** (`_body_folding`, src/drone_cad.py:107-115)",
"",
...
"접이식 3종은 n_pow=2.9, Phantom 은 더 각진 셸이라 n_pow=3.4 를 준다(← src/drone_cad.py:267-269).",
"코 처짐(nose_drop)도 접이식 0.22 vs Phantom 0.05 로 다르다 — Phantom 셸은 앞뒤가 거의 대칭이다.",
"",
"**2단계. 캐노피 — 등에 얹힌 배터리 돔** (`_canopy`, src/drone_cad.py:118-126)",
"",
"실물은 배터리가 동체 등에 얹힌 낮고 평평한 돔 모양이라, 동체보다 작은 로프트를 하나 더 만들어",
"z 위로 올려 붙인다. 그룹 이름은 `canopy` — 재질 배정(플라스틱)이 그룹 단위로 따라온다.",
"",
"**3단계. 암 — 베지에 경로의 스윕** (`_arm_folding`, src/drone_cad.py:129-142)",
"",
...
"vs 고정암 0.085·diag (← src/drone_cad.py:273-274).",
"",
"**4단계. 모터 벨 — 회전체** (`_motor_bell`, src/drone_cad.py:44-51)",
"",
"(r,z) 프로파일 9개 점을 z축으로 한 바퀴 돌린 회전체다. \"아래가 잘록하고 위가 부푼 실제",
"아웃러너(outrunner: 겉통이 도는 드론 모터) 형상\"(← 해당 함수 docstring). 회전체를 만들 때",
"r=0 인 점을 꼭짓점으로 접지 않으면 면적 0 짜리 퇴화 삼각형이 쏟아진다는 함정이 있었다",
"(← 출처: src/cadkit.py:262-264 주석 — trimesh 검증이 모터에서 288개를 실제로 잡아냈다).",
),
...
"",
"짐벌과 착륙장치는 **드론 실루엣의 핵심 식별 특징**이라(← src/drone_cad.py:146 절 주석) 기종별로",
"함수를 나눴다. 스펙의 `gimbal_style`/`gear` 필드가 어느 함수를 쓸지 정한다:",
"",
"| 함수 | 형태 | 쓰는 기종 | 왜 |",
"|---|---|---|---|",
"| `_gimbal_infinity` (drone_cad.py:148-158) | 구(볼) + 전면 렌즈 3개 + 롤 요크 | Mavic 4 Pro | 실물이 기수와 일직선인 **구형 Infinity 짐벌**(360° 회전, 3렌즈)이라서 — 매달린 상자로 만들면 실루엣이 틀린다 (← drone_cad.py:16-17 주석, docs/SPECS.md Mavic 4 Pro) |",
"| `_gimbal_hanging` (drone_cad.py:161-170) | 방진판 + 요크 + 카메라 상자 + 렌즈 | Mini 5 Pro·Phantom 4·S1000+ | 코 아래 **매달린** 전통 짐벌. Phantom 은 함몰(recessed)이라 동체에 더 붙여 배치 (← drone_cad.py:303) |",
"| `_gimbal_sensor` (drone_cad.py:173-180) | 3축 마운트 + 렌즈 3 + 레이저 측거 | Matrice 4E | 측량 페이로드(카메라 클러스터 + 레이저 거리계)가 공식 구성이라서 (← docs/SPECS.md Matrice 4E) — RTK 돔도 캐노피 위에 추가된다 (← drone_cad.py:299-301) |",
"| `_gear_skids` (drone_cad.py:186-196) | 다리 4 + 좌우 스키드 바 | Phantom 4 | 일체형 흰 셸에 붙은 고정 착륙다리가 Phantom 정체성 (← docs/SPECS.md) |",
"| `_gear_tall` (drone_cad.py:199-211) | 길게 벌어지는 카본 봉 + 발 바 | S1000+ | 벨리 짐벌 공간을 확보하는 긴 접이식 다리 (← docs/SPECS.md S1000+, 랜딩기어 460×511×305 mm) |",
"| `_gear_feet` (drone_cad.py:214-220) | 작은 발 4개 | Matrice 4E | 전용 스키드 없이 낮은 발로 앉는 기종 |",
...
"",
"조립 마지막에 렌더에선 절대 안 보이는 상자 두 개가 셸 **안에** 들어간다(← src/drone_cad.py:320-322):",
"",
"> \"내부 금속 산란체 (RCS 지배) — 셸 안이라 렌더엔 안 보이지만 PO/SBR 이 센다\"",
"",
"이유: 드론 셸은 플라스틱이라 GHz 전파에 **반투명**하다(진폭 반사계수 |Γ|≈0.3 수준 — 즉 전파",
"대부분이 셸을 뚫고 들어간다). 그래서 실물 드론의 레이더 반사는 셸이 아니라 **안에 있는",
"배터리팩·ESC/메인보드·모터 금속**이 지배한다(← 출처: src/drones.py:281-285 주석). 재질 배정도",
"이에 맞춰 battery=metal(\"GHz 에서 파우치 포일은 사실상 금속\"), pcb=FR-4+구리 그라운드플레인이다",
"(← src/drones.py:188-189, DRONE_GROUP_MAT). 치수는 기종별 실물이 비공개라 동체 대비 대표",
"비율(파라메트릭)로 뒀다 — 이것도 추정임을 주석에 남겼다(← src/drones.py:285-287).",
"",
"### §2.3 마지막 손질 — 불리언 합집합",
"",
"부위를 겹쳐 쌓기만 하면 '동체 속에 파묻힌 암 뿌리' 같은 **내부 면**이 메쉬에 그대로 남아,",
"레이더 계산(PO/SBR)이 존재하지 않는 면을 헛세게 된다. 그래서 그룹별로 불리언 합집합을 돌려",
"겹친 파트를 한 껍질로 녹인다 — \"그런 면이 **애초에 존재하지 않는다**\"(← src/drone_cad.py:324-329).",
...
"같아진다는 보장이 없다. 실제로 2026-07-14 감사에서 파라메트릭 실루엣만으로 만든 프레임은",
"**높이가 전 기종 −25~−47%**, Mavic 폭이 −36% 어긋나 있었다(← 출처: src/drones.py:301-309 주석).",
"",
"이게 왜 치명적인가 — 챔버 기하(낮은 앙각 el≈15°)에서는 **높이가 측면 투영면적을 지배**하고,",
"평판 극한에서 RCS 는 σ ∝ (투영면적)² 이므로, 높이 −44% 는 σ 를 ~5 dB 과소평가한다. 즉 모양이",
"예뻐도 크기가 틀리면 탐지 확률 계산이 통째로 틀린다(← 같은 주석, src/drones.py:306-309).",
"",
"**해법** (`frame_fit_scale`, src/drones.py:323-340): 실루엣은 그대로 두고, 완성된 프레임의",
...
"(← src/drones.py:379 — \"프레임과 **같은** 외형보정 배율\").",
"",
"**대가도 정직하게** 적혀 있다(← src/drones.py:312-314): 축마다 배율이 다르므로(비등방)",
"모터 원통이 약간 타원이 된다. \"RCS 가 보는 것은 투영면적과 외형이므로 이쪽을 맞추는 것이",
"옳다는 판단.\" 그리고 **프로펠러는 스케일하지 않는다** — 프롭 지름(prop_dia_mm)은 별도 공식",
"스펙이 있기 때문이다(← src/drones.py:310).",
"",
...
"(외형이 우선 ← src/drones.py:315-319 주석). 프로펠러 지름이 전 기종 일관되게 +0.84% 인 것은",
...
"",
"색 규칙(모든 기종 공통): 프로펠러=주황, 모터=강청(금속), 카메라=청록, 플라스틱=밝은 회색,",
"PCB=초록 — **색이 곧 재질**이라 그림만 봐도 전파 물성이 읽힌다(← src/drones.py:512-529",
...
"   접이식과 달리 암이 굵고(0.085·diag ← src/drone_cad.py:273) 덜 휜다(bend 0.02 vs 0.06 ←",
"   src/drone_cad.py:281). 암 그룹도 카본이 아니라 동체와 같은 흰 셸(plastic)이다 —",
"   `arm_style='body'` 면 암을 body 그룹에 넣는다(← src/drone_cad.py:279).",
...
"   붙인다(cx 를 0.62 배로 ← src/drone_cad.py:303-304). 기수 아래 작은 비전센서 2개도 붙는다",
"   (← src/drone_cad.py:305-307).",
"",
...
"— 버그가 아니다.** 프로펠러는 장착 위상(+12° 오프셋 ← src/drones.py:386)과 교대 회전 방향으로",
...
f"   ({NAME[worst_key]} 대각, 종속 값). 감사에서 잡힌 높이 −25~−47% 문제가 이렇게 닫혔다(§3).",
f"4. **좌우대칭은 규약 + 실측으로 보증한다** — 프레임 p95 ≤ 2 mm 전 기종 통과(§5).",
"",
"**한계(정직하게)** — 내부 battery/pcb 치수는 기종별 실물 비공개라 동체 비율 추정이고,",
