...
f"Phantom 4 스캔점의 절반이 CAD 표면 {_scan_p50:.1f} mm 이내(→ mesh08 §3), 커뮤니티/실물 CAD 와 RCS 순위 일치"
...
f"| `_gear_motor_legs` | 앞 모터 포드 밑 두 갈래 다리 2개 | {_gear_users('motor_legs')} | 접이식 소형기는 전용 스키드 대신 **앞쪽 모터 포드 아래에만** 다리가 달린다(뒤쪽 포드는 매끈하다) — 제품사진 계측, 밴드 ±15 % (← src/drones.py 각 기체 note) |",
f"| (없음) | — | {_gear_users('none')} | 착륙장치를 아예 선언하지 않는 기종. 지금 함대에는 해당 기체가 없다 |",
...
"(외형이 우선 ← src/drones.py:266-268 주석).",
...
f"   피한다. 와이어프레임 패널에서 앞쪽 프롭 두 개가 뒤쪽보다 {mini.rotor_z_mm[1] - mini.rotor_z_mm[0]:g} mm 낮게 앉은 게 보인다(← src/drones.py:106-107",
...
f"| 암/착륙장치 | 접이식 · 모터 포드 아래 다리({mini.gear_h_mm:g} mm) | 고정암 · 스키드 다리 | src/drones.py:103,170 |",
