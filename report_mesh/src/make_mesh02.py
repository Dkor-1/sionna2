...
# ── §6.1 매끈함의 대가 ─────────────────────────────────────────────────────
md(
"### 6.1 매끈함에는 대가가 있다 — 두 가지, 부호가 서로 반대다",
"",
"«매끈해서 좋다» 만 적으면 절반이다. 지금 상태에서 실제로 재 본 두 가지를 적는다.",
"①은 형상을 **더 크게**(= 전파에 «밝게») 틀리고, ②는 끝단 캡을 **더 작게**(= «어둡게») 틀린다 —",
"부호가 서로 반대다. ⛔ 옛 판 제목은 «둘 다 «밝게» 쪽으로 틀린다» 였다 — ②가 반대 부호라 2026-09-06 에 내렸다.",
"",
...
"",
"⭐ **잰 세 칸에서는 모두 «더 크게» 쪽으로 벗어났다** — matrice4e·mini5pro·mini2 셋 다",
"형상표보다 크게 나왔다. 다만 표본은 이 셋뿐이고, 원장 `outputs/mesh_inspect_body_arms_0816.json`",
f"의 B8 확신도도 «{[x for x in F.BODY['findings'] if x.get('id') == 'B8'][0]['confidence']}» 다 — 부호가 늘 이쪽인지는 아직 모른다.",
"3차 스플라인 넘침의 방향은 제어점 배열에 달려 있어 안쪽으로도 벗어날 수 있다.",
"⛔ 옛 판은 «부호가 늘 같은 쪽이다 — 우연이 아니라 스플라인 넘침의 성질이다» 로 적었다 —",
"본 것이 아니라 추론이라 2026-09-06 에 내렸다.",
"고치는 길은 둘이다: 제어점을 6 → 8~10 으로 늘리거나, 넘치지 않는 보간",
...
"| 잣대 | 어떻게 검증했나 |",
"|---|---|",
"| 자기교차(삼각형-모서리 관통) | 양성 대조 4종(교차 삼각형·관통 삼각형·회전 상자쌍 18 hits"
"·구쌍 84 hits)에서 전부 검출, 음성 대조 5종에서 0 |",
"| 부품 부양(안 닿는 파트) | **양방향** 거리 + 내부 판정 — 단방향 정점→면 거리는 큰 상자가"
" 가는 봉을 감쌀 때 «떠 있다» 로 읽는다 |",
"",
