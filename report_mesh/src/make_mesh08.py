...
FORCED = {k: tuple(a is not None for a in (DRONES[k].envelope_mm or (None, None, None)))
          for k in ORDER}
...
#  ⭐ 개별 각도의 흔들림은 **두 기체 중 나쁜 쪽**으로 인용한다(한 기체만 쓰면 최악을 숨긴다).
...
"",
"옷을 다 만든 뒤 **줄자로 다시 재보는** 단계다. 설계에 넣은 숫자를 그대로 믿지 않고, **완성된 메쉬를 독립적으로 실측**해 공식 스펙과 비교한다: 외형 L×W×H 는 `frame_envelope_mm()`(빌드된 프레임의 바운딩박스를 실측 ← `src/drones.py`), 프로펠러 지름은 빌드된 프롭 메쉬의 xy 최대 반경×2, 대각(모터-모터 거리)은 실제 로터 배치 좌표에서 잰다. ← 출처: `report_mesh/src/verify_mesh_suite.py` `sec_C_dims()`",
"",
...
"| 강제 범위 | 기체 |",
"|---|---|",
f"| **높이만** 강제(L/W 는 자유) | {', '.join(NAME[k] for k in lw_free if any(FORCED[k]))} |",
f"| **세 축 전부** 강제 | {', '.join(NAME[k] for k in lw_forced)} |",
f"| 강제 없음 | {', '.join(NAME[k] for k in lw_free if not any(FORCED[k])) or '—'} |",
"",
...
"실물 충실도의 진짜 증거는 §3(실기체 스캔 대조)과, 공식 CAD 가 있는 기체의 대조다(§1.5).",
),
...
f"{len(MF.M4EFIX['fixes'])}건 정정했고, 지금 그 정정이 전부 메쉬에 반영돼 있다:",
"",
f"> {MF.BODY['meshfix_matrice4e_landed']['verdict']}",
"",
"← 출처: `outputs/meshfix_matrice4e.json`(정정 명세) · "
"`outputs/mesh_inspect_body_arms_0816.json` `meshfix_matrice4e_landed`(착지 검증).",
"",
...
for key in ORDER:
    for chk, v in C[key]["checks"].items():
        if "official" not in v:      # prop_disc_L/W 는 공표값이 없는 실측 전용 항목이다
            print(f"{NAME[key]:<12} {chk:<9} {'—':>9} {v['measured']:>9.1f} {'—':>8}")
            continue
...
"| 드론 | 대각 기준값 | 등급 | 근거 |",
"|---|---|---|---|",
f"| Mini 5 Pro | {C['mini5pro']['checks']['diagonal']['official']:.0f} mm | ⚠ **추정 (±20 mm)** | DJI 는 Mini 시리즈 대각을 공개하지 않는다. 공개 치수·프롭 규격과 조사된 로터 배치 좌표에서 유도한 값으로, 독립 검증자도 \"±~20 mm 근사로 취급하고 실기체·삼면도로 확인하라\" 고 명시 ← 출처: docs/SPECS.md 'Mini 5 Pro' 주의·검증 절(dji.com/mini-5-pro/specs), `src/drones.py` note·로터좌표 주석 |",
f"| Mavic 4 Pro | {C['mavic4pro']['checks']['diagonal']['official']:.0f} mm | 공식 외형에서 **유도** | DJI 는 대각을 공개하지 않고, 흔히 도는 추정 400 mm 는 공식 외형 328.7×390.5 와 기하학적으로 **모순**이다(400 으로는 그 상자를 걸칠 수 없다) → 공식 외형이 함의하는 440.9 mm 를 기준으로 쓴다 ← 출처: `src/drones.py` note, docs/SPECS.md 'Mavic 4 Pro' |",
...
"",
"택배 상자를 들어보고 \"이 무게면 안에 뭐가 들었겠구나\" 가늠하는 것과 같은 스모크 테스트다. 무게는 DJI 공식값이고 부피는 우리 메쉬에서 나오므로, 둘을 나눈 **암시밀도 = 공식 무게 ÷ 메쉬 부피** 가 상식적인 범위에 있는지 보면 \"메쉬가 크게 잘못 만들어지진 않았는가\"를 빠르게 걸러낼 수 있다. 부피는 그룹(부위)별 watertight 컴포넌트의 닫힌 부피 합이다. ← 출처: `report_mesh/src/verify_mesh_suite.py` `sec_D_volume()`",
"",
...
"",
"1. **binary STL 을 numpy 로 직접 파싱** — 3.09M 삼각형에 무거운 라이브러리 대신 50바이트 레코드 규격을 그대로 읽는 편이 빠르고 의존성이 없다 (`prep_cad_scan.py`).",
"2. **스케일 보정 ×1.0125** — 스캔의 모터 허브 대각이 345.7 mm 로 공식 350 mm 보다 1.25 % 작게 나와, 공식 대각에 맞춰 늘린다 (`prep_cad_scan.py`). ⛔ **차이의 원인은 특정하지 못했다** — 스캐너 보정인지, 개체차인지, 공표값 쪽인지 가른 계산이 없다(옛 판은 «스캐너 보정 오차» 로 단정했고 2026-09-06 에 내렸다). 크기를 이렇게 맞춰 두었으므로 **§3 대조가 독립인 축은 «표면 형상» 이고 «절대 크기» 는 아니다.** 실물 스캔도 '측정'이라 오차가 있다는 좋은 예다.",
"3. **PCA 로 z-up 정렬** — 스캔 좌표축은 임의라, 면적가중 분산이 최소인 축을 z 로 삼고 랜딩기어(긴 꼬리)가 아래로 가게 부호를 정한다 (`prep_cad_scan.py`).",
f"4. **3 mm 복셀 클러스터** — 3.09M 삼각형 → {G['n_scan']:,} 점으로 압축. 3 mm 는 최고 대역(5.2 GHz) 파장 {LAM52:.0f} mm 의 λ/10(≈5.8 mm)보다 조밀해 PO 용으로 충분하다. 셀 안에서 법선이 상쇄된(양면 접힘) 점은 버리고 유효면적을 법선 벡터합 크기로 줄여 **PO 물리와 정합**시킨다 (`prep_cad_scan.py, 84-96`).",
"",
f"**비교 방법** (`verify_mesh_suite.py` `sec_G_scan()`) — 스캔에는 **프로펠러와 짐벌 카메라가 없다**(탈거 후 스캔) → 공정하게 CAD 쪽도 prop/camera/gimbal 그룹을 제외하고 3 mm 간격으로 표면 점 {G['n_cad']:,}개를 뽑는다. 정렬은 수평 중심 맞춤 + **z 오프셋만 ±30 mm 그리드 탐색**(2 mm 스텝): 회전까지 맞추는 ICP 같은 방법은 잘못 수렴하면 오차를 몰래 숨기므로, 일부러 자유도를 z 하나로 묶어 **보수적으로** 잰다. 탐색 결과 z-shift = {G['z_shift_mm']:+.0f} mm — 스캔(몸체만)의 무게중심 높이와 CAD(랜딩기어 포함 전체) 중심 높이가 달라 생기는 자연스러운 어긋남이다.",
),
...
"",
f"왼쪽: CAD(파랑)와 실기체 스캔(빨강) 오버레이 — 실루엣이 겹친다. 가운데: 스캔의 각 점을 \"CAD 까지 거리\"로 색칠한 지도. **어디가 다른지**가 보인다: 동체 셸의 매끈한 중앙부는 어둡고(수 mm), 암 뿌리·셸 곡률이 급히 변하는 모서리·랜딩기어 접합부가 밝다(수십 mm) — 파라메트릭 CAD 가 뭉뚱그린 디테일이 정확히 그 자리들이다. 오른쪽: 거리 히스토그램과 p50/p90. ← 출처: 그림 `report_mesh/src/viz_mesh_reports.py` `fig_scan_overlay()`",
"",
"### 두 방향의 비대칭이 말해주는 것",
"",
...
"",
f"(a) Mavic 4 Pro 의 방위 RCS 패턴 — λ/10(파랑)과 λ/20(빨강)이 로브(봉우리) 영역에서는 거의 포개진다. 방위평균 이동은 {hm['azavg_dbsm']['diff']:+.2f} dB(Phantom 4 는 {hp['azavg_dbsm']['diff']:+.2f} dB) — **둘 다 ±{max(abs(hm['azavg_dbsm']['diff']), abs(hp['azavg_dbsm']['diff'])):.1f} dB 이내**다. (b) 는 다음 절의 SBR 검사. ← 출처: 그림 `report_mesh/src/viz_mesh_reports.py` `fig_convergence()`",
"",
"### 왜 개별 널은 " f"{PO_NULL_MAX:.0f} dB 씩 튀는데 괜찮다고 하는가",
"",
...
"",
"**그리고 검출 문제에서 중요한 것은 평균이다.** 실제 드론은 자세가 계속 변하고 프로펠러가 돌아 널 위치가 쉼 없이 이동한다 — 수신기가 겪는 것은 특정 널 하나가 아니라 각도 분포의 **평균적 에너지**다. 그래서 이 시리즈의 RCS 결론(순서·규모)은 전부 방위평균 통계로 말하고, 개별 널 깊이는 애초에 주장하지 않는다. 이는 실측 문헌들의 RCS 도 세팅에 따라 ±수 dB 씩 흩어진다는 앵커 편의 논거와 같은 이유다. ← 출처: 본편 권 3 «σ 를 무엇에 붙들어 매나»(`reports/03_anchor.ipynb`)",
),
...
"",
"SBR(`src/rcs_sbr.py` `rcs_sbr_batch`)은 PO 와 독립인 두 번째 계산 엔진이라 자기만의 수렴 놉이 있다. 검사를 **두 겹**으로 설계한 이유가 중요하다 (← 출처: `report_mesh/src/verify_mesh_suite.py` `sec_I_sbr_subdiv()` docstring):",
"",
...
"### 조심할 것",
"",
f"- **개별 널·개별 각도의 dB 값** — PO 점간격에 최대 {PO_NULL_MAX:.0f} dB(Mavic 4 Pro {hm['per_angle_absdiff_db']['max']:.1f} · Phantom 4 {hp['per_angle_absdiff_db']['max']:.1f}), SBR 광선 밀도에 최대 {RAY['per_angle_absdiff_db']['max']:.0f} dB 까지 민감하다. \"방위 137° 에서 −38 dBsm\" 같은 문장은 이 파이프라인이 보증하지 않는다.",
f"- **절대 dBsm 은 ±2~3 dB 로 읽어라** — 수치 수렴(±0.5 dB 급)에 재질 |Γ| 불확실성과 형상 단순화가 얹힌다. 이는 실측 문헌 자체가 세팅(자세·대역·편파)에 따라 ±수 dB 흩어진다는 앵커 편의 논거와 정확히 맞물리는 폭이다 — 우리 절대값 주장도 그 이상 정밀한 척하지 않는다.",
...
f"- **«{NAME['mavic4pro']}·{NAME['mini5pro']} 의 세로 치수»** — 두 기체는 공표 높이를 형상이 아니라",
...
"### 기존 리포트와의 연결",
"",
"- **별편 2-3 «표적을 짓는다 — 메쉬와 재질»**(`reports/02_3_target-mesh.ipynb`) — 같은 주제의 본편 쪽 편이다. "
"여기서 검증한 스캔 점구름·사진·CAD 세 자로 «메쉬가 실물과 얼마나 맞나» 의 **결론**을 쓴다. "
"이 시리즈(report_mesh)는 그 결론을 **어떻게 만들고 어떻게 채점했는지**를 쓴다.",
"- **권 3 «σ 를 무엇에 붙들어 매나»**(`reports/03_anchor.ipynb`) — 이 편이 세운 신뢰 경계"
"(±2~3 dB, 평균 중심) 위에서 문헌 실측치와 앵커링한다.",
),
...
"---",
"",
"**시리즈 완결.** 이 시리즈는 `README.md` 편성에서 **부록 8편**이고, 메쉬가 실제 탐지에 쓰이는 "
"모습은 **본편 11권 + 별편 8편**(`reports/`)에 있다 — 특히 별편 2-3(메쉬·재질)·권 3(σ 앵커)·"
"권 6(마이크로도플러)이 이 검증 위에 서 있다.",
),
]
