...
f"  로터 거리는 {MF.BODY['per_drone']['mini5pro']['wheelbase_mm']:.2f} mm 다"
f"({diag_err['mini5pro']:+.2f} %). 스펙 `note` 가 이미 그렇게 선언한다.",
"",
...
f"- **CAD→스캔: p50 {c2s['p50']:.1f} mm 로 2배쯤 크다** — 이건 모델이 나빠서가 아니라 **스캔에 구멍이 있어서**다. 실기체 스캔은 기체를 세워 놓고 돌려 찍기 때문에 **바닥면(동체 하부)이 스캔되지 않았고**, 그 자리의 CAD 점들은 짝을 찾아 멀리 헤매게 된다. 비다양체·부유 링 아티팩트(~0.8%)도 스캔 쪽 잡음이다. ← 출처: `src/prep_cad_scan.py:18-19` 주의 절, mesh_verify.json `G_scan.cad_to_scan_mm`",
"",
